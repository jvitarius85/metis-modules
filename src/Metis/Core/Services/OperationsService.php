...
final class OperationsService {
    ...
    public function queueSummary(): array {
        ...
        $rows = $this->db->fetchAll(
            ...
            [ 'system.cron.task', self::JOB_TYPE ]
        );
        ...
        foreach ( $rows as $row ) {
            $bucket = ( (string) ( $row['job_type'] ?? '' ) === 'system.cron.task' ) ? 'cron' : 'operations';
    ...
    public function recentJobs( int $limit = 20, array $jobTypes = [ 'system.cron.task', self::JOB_TYPE ] ): array {
