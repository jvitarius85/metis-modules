...
final class HelpSearchStore {
    ...
    public function runSeeder( bool $force = false ): array {
        $this->ensureSchema();
        $seedFile = dirname( __DIR__, 3 ) . '/database/seeders/help_documents_seed.php';
        if ( ! is_file( $seedFile ) ) {
    ...
    private function encodeTags( array $tags ): string {
        $encoded = function_exists( 'wp_json_encode' )
            ? wp_json_encode( array_values( array_unique( array_map( 'strval', $tags ) ) ) )
            : json_encode( array_values( array_unique( array_map( 'strval', $tags ) ) ), JSON_UNESCAPED_SLASHES | JSON_UNESCAPED_UNICODE );
