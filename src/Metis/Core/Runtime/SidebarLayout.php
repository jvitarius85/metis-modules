...
if ( ! function_exists( 'metis_render_sidebar_layout' ) ) {
    function metis_render_sidebar_layout( array $args ): void {
        ...
        echo '<div class="mw-sidebar-layout ' . metis_escape_attr( $class ) . '">';
        echo '<aside class="mw-sidebar-layout-sidebar ' . metis_escape_attr( $shell_class ) . '">';
        echo '<div class="mw-sidebar-layout-sidebar-inner ' . metis_escape_attr( $sidebar_class ) . '">';
        if ( $sidebar ) {
            $sidebar();
        }
        echo '</div>';
        echo '</aside>';
        echo '<div class="mw-sidebar-layout-content ' . metis_escape_attr( $content_class ) . '">';
        if ( $content ) {
            $content();
        }
        echo '</div>';
