...
if ( ! function_exists( 'metis_help_topic_response' ) ) {
    function metis_help_topic_response(): void {
        ...
        if ( ! $service instanceof Metis_Help_Service ) {
            metis_runtime_send_json_error( [ 'message' => 'Help service is unavailable.' ], 500 );
        }

        $topic_id = metis_help_normalize_topic_id( $_POST['topic'] ?? '' );
        if ( $topic_id === '' ) {
            metis_runtime_send_json_error( [ 'message' => 'Help topic is required.' ], 400 );
        }

        $topic = $service->topic( $topic_id );
        if ( ! is_array( $topic ) ) {
            metis_runtime_send_json_error( [ 'message' => 'Help topic not found.' ], 404 );
        }
...
if ( ! function_exists( 'metis_help_search_response' ) ) {
    function metis_help_search_response(): void {
        ...
        try {
            ...
            metis_runtime_send_json_error( [ 'message' => 'Help search failed: ' . $e->getMessage() ], 500 );
        }
    }
}
