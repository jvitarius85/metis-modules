...
final class JobQueue {
    ...
    private function claimJobs( int $limit ): array {
        ...
        $claimed = [];
        $lease_until = $this->formatTimestamp( \current_time( 'timestamp' ) + self::DEFAULT_LEASE_TTL );
    ...
    private function markFailed( array $job, \Throwable $e ): string {
        ...
        if ( $attempts < $max_attempts ) {
            $retry_at = $this->formatTimestamp( \current_time( 'timestamp' ) + min( 3600, max( 60, $attempts * 60 ) ) );
            $wpdb->update(
                $table,
                [
                    'status'         => self::STATUS_QUEUED,
    ...
    private function normalizeDatetime( mixed $value ): string {
        ...
        return $timestamp === false ? \current_time( 'mysql' ) : $this->formatTimestamp( $timestamp );
    }
