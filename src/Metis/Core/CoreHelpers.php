...
function metis_navigation_svg_icon_directory(): string {
    ...
    $candidates = [
        METIS_ROOT . '/assets/Images/icons',
    ];
...
function metis_sidebar_nav( array $modules, string $active_domain ): string {
    ...
    foreach ( $items as $item ) {
        ...
        if ( $children === [] && $module_view_children === [] ) {
            ...
            $html .= '<a href="' . $url . '" class="mw-sidebar-item' . $active_cls . '" data-tooltip="' . metis_escape_attr( $label ) . '" aria-label="' . metis_escape_attr( $label ) . '"' . $aria_current . '>'
                . '<span class="mw-sidebar-icon' . $icon_class . '">' . $icon . '</span>'
                . '<span class="mw-sidebar-label">' . metis_escape_html( $label ) . '</span>'
                . '</a>';
            continue;
        }

        if ( $children === [] && $module_view_children !== [] ) {
            $submenu_id = 'mw-sidebar-submenu-' . metis_key_clean( (string) $module_key . '-' . (string) ( $item['id'] ?? 0 ) );
            $active_view = metis_key_clean( (string) metis_get_query_var( 'metis_view' ) );
            $group_active = $is_active;

            $html .= '<div class="mw-sidebar-group' . ( $group_active ? ' is-active' : '' ) . '">';
            $group_url = $route !== '' ? metis_escape_url( $route ) : '#';
            $html .= '<a href="' . $group_url . '" class="mw-sidebar-item mw-sidebar-group-link mw-sidebar-has-submenu' . ( $group_active ? ' is-active' : '' ) . '" data-tooltip="' . metis_escape_attr( $label ) . '" aria-label="' . metis_escape_attr( $label ) . '" aria-haspopup="true" aria-expanded="false" aria-controls="' . metis_escape_attr( $submenu_id ) . '"' . ( $is_active ? ' aria-current="page"' : '' ) . '>'
                . '<span class="mw-sidebar-icon' . $icon_class . '">' . $icon . '</span>'
                . '<span class="mw-sidebar-label">' . metis_escape_html( $label ) . '</span>'
                . '</a>';
            $html .= '<div class="mw-sidebar-submenu" id="' . metis_escape_attr( $submenu_id ) . '" aria-label="' . metis_escape_attr( $label . ' navigation' ) . '" aria-hidden="true">';
            $html .= '<div class="mw-sidebar-submenu-title">' . metis_escape_html( $label ) . '</div>';
            ...
            foreach ( $module_view_children as $child ) {
                ...
                $html .= '<a class="mw-sidebar-subitem' . ( $childActive ? ' is-active' : '' ) . '" href="' . metis_escape_url( $childRoute ) . '" data-tooltip="' . metis_escape_attr( $childLabel ) . '" aria-label="' . metis_escape_attr( $childLabel ) . '"' . ( $childActive ? ' aria-current="page"' : '' ) . '>'
        ...
        foreach ( $children as $child ) {
            ...
            $subsubmenu_id = 'mw-sidebar-subsubmenu-' . metis_key_clean( (string) $module_key . '-' . (string) $childModule . '-' . (string) $childLabel );
            $html .= '<div class="mw-sidebar-subitem-group">';
            $html .= '<a class="mw-sidebar-subitem mw-sidebar-subitem-link' . ( $childActive ? ' is-active' : '' ) . '" href="' . metis_escape_url( $childRoute ) . '" data-tooltip="' . metis_escape_attr( $childLabel ) . '" aria-label="' . metis_escape_attr( $childLabel ) . '" aria-haspopup="true" aria-expanded="false" aria-controls="' . metis_escape_attr( $subsubmenu_id ) . '"' . ( $childActive ? ' aria-current="page"' : '' ) . '>'
                . '<span>' . metis_escape_html( $childLabel ) . '</span>'
                . '<span class="mw-sidebar-subitem-arrow" aria-hidden="true">›</span>'
                . '</a>';
            $html .= '<div class="mw-sidebar-subsubmenu" id="' . metis_escape_attr( $subsubmenu_id ) . '" aria-label="' . metis_escape_attr( $childLabel . ' views' ) . '" aria-hidden="true">';
            foreach ( $childViewChildren as $viewChild ) {
                ...
                $html .= '<a class="mw-sidebar-subitem' . ( $viewActive ? ' is-active' : '' ) . '" href="' . metis_escape_url( $viewRoute ) . '" data-nav-nested-link="1" data-tooltip="' . metis_escape_attr( $viewLabel ) . '" aria-label="' . metis_escape_attr( $viewLabel ) . '"' . ( $viewActive ? ' aria-current="page"' : '' ) . '>'
...
function metis_sidebar_logout_nav( string $active_domain = '' ): string {
    ...
    return '<a href="' . $url . '" class="mw-sidebar-item mw-sidebar-logout' . $active_cls . '" data-tooltip="' . metis_escape_attr( $label ) . '" aria-label="' . metis_escape_attr( $label ) . '"' . $aria_current . '>'
        . '<span class="mw-sidebar-icon' . $icon_class . '">' . $icon . '</span>'
        . '<span class="mw-sidebar-label">' . metis_escape_html( $label ) . '</span>'
        . '</a>';
}
