...
function metis_auth_normalize_identifier( string $identifier ): array {
    ...
    $email = strtolower( trim( sanitize_email( $identifier ) ) );
    $login = sanitize_key( $identifier );
...
function metis_auth_find_user_by_identifier( string $identifier ): ?array {
    ...
    foreach ( $candidates as $row ) {
        $login = sanitize_key( (string) ( $row['user_login'] ?? '' ) );
...
function metis_auth_person_row_to_auth_payload( array $person, ?array $auth_user = null ): array {
    ...
    if ( is_array( $auth_user ) ) {
        $login = sanitize_key( (string) ( $auth_user['user_login'] ?? '' ) );
    }
    if ( $login === '' ) {
...
function metis_auth_current_method(): string {
    return sanitize_key( (string) ( $_SESSION['metis_auth_method'] ?? '' ) );
}

function metis_auth_refresh_session(): void {
    $user_id = (int) ( $_SESSION['metis_auth_user_id'] ?? 0 );
    if ( $user_id < 1 ) {
        unset( $_SESSION['metis_user'], $_SESSION['metis_person_id'], $_SESSION['metis_session_integrity'], $_SESSION['metis_auth_issued_at'] );
        return;
    }
...
function metis_auth_finalize_login( array $row, string $auth_method = 'password' ): void {
    ...
    $_SESSION['metis_user'] = metis_auth_user_row_to_session( $row );
    $_SESSION['metis_auth_method'] = sanitize_key( $auth_method );
    metis_auth_refresh_session_integrity();
...
function metis_auth_logout(): void {
    ...
    unset( $_SESSION['metis_auth_user_id'], $_SESSION['metis_person_id'], $_SESSION['metis_auth_issued_at'], $_SESSION['metis_session_token'], $_SESSION['metis_session_integrity'], $_SESSION['metis_user'], $_SESSION['metis_pending_auth'], $_SESSION['metis_auth_password_verified_at'] );
    if ( \Metis\Core\Application::has_service( 'session_security' ) ) {
        \Metis\Core\Application::service( 'session_security' )->regenerateId();
    }
}
...
function metis_auth_login_customization(): array {
    ...
    $config_path = ( defined( 'METIS_PATH' ) ? rtrim( (string) METIS_PATH, '/\\' ) : dirname( __DIR__, 4 ) ) . '/config/login.php';
    ...
    $resolved = [
        ...
        'background_color' => sanitize_hex_color( (string) ( $defaults['background_color'] ?? '#edf2f7' ) ) ?: '#edf2f7',
    ...
    if ( class_exists( 'Core_Settings_Service' ) ) {
        ...
        $resolved['background_color'] = sanitize_hex_color( (string) Core_Settings_Service::get( 'login_background_color', $resolved['background_color'] ) ) ?: $resolved['background_color'];
...
function metis_auth_render_shell( string $title, string $body, int $status = 200 ): never {
    ...
    $background_style = 'background:' . esc_attr( (string) $custom['background_color'] ) . ';';
    if ( ! empty( $custom['background_image'] ) ) {
        $background_style = 'background:' . esc_attr( (string) $custom['background_color'] ) . ' url(' . esc_url( (string) $custom['background_image'] ) . ') center/cover no-repeat;';
    }
    ...
    echo '<link rel="stylesheet" href="' . esc_url( metis_site_url( '/assets/css/auth-shell.css' ) ) . '">';
    echo '<script src="' . esc_url( metis_site_url( '/assets/js/auth-passkey-client.js' ) ) . '" defer></script>';
    echo '</head><body class="metis-auth-shell" style="' . $background_style . '"><div class="wrap">';
    echo '<div class="auth-brand">';
    if ( $logo !== '' ) {
        echo '<img src="' . esc_url( $logo ) . '" alt="' . esc_attr( $brand ) . '">';
    }
    if ( $brand !== '' ) {
        echo '<div class="auth-brand-name">' . esc_html( $brand ) . '</div>';
    }
    if ( $welcome !== '' ) {
        echo '<p class="muted">' . esc_html( $welcome ) . '</p>';
    }
    echo '</div>';
    echo $body;
    if ( $footer !== '' ) {
        echo '<div class="auth-footer">' . esc_html( $footer ) . '</div>';
    }
    echo '</div></body></html>';
    exit;
}
...
function metis_auth_build_passkey_client( string $redirect ): string {
    $complete_url = esc_url_raw( metis_home_url( '/api/auth/passkeys/complete' ) );
    $resolve_url = esc_url_raw( metis_home_url( '/api/auth/resolve' ) );
    ...
    return ' data-resolve-url="' . esc_attr( $resolve_url ) . '"'
        . ' data-complete-url="' . esc_attr( $complete_url ) . '"'
        . ' data-resolve-nonce="' . esc_attr( $resolve_nonce ) . '"'
        . ' data-begin-nonce="' . esc_attr( $begin_nonce ) . '"'
        . ' data-complete-nonce="' . esc_attr( $complete_nonce ) . '"'
        . ' data-complete-action="' . esc_attr( metis_auth_passkey_complete_nonce_action() ) . '"'
...
function metis_router_handle_auth_resolve_request( Metis_Http_Request $request ): Metis_Http_Response {
    try {
        ...
        $redirect = (string) ( $input['redirect_to'] ?? metis_portal_url() );
        ...
        return Metis_Http_Response::json( [ 'success' => false, 'data' => [ 'message' => $e->getMessage() ] ], 400 );
    }
}
...
function metis_router_handle_auth_passkey_complete_request( Metis_Http_Request $request ): Metis_Http_Response {
    try {
        ...
        ], (string) ( $input['redirect_to'] ?? metis_portal_url() ) );
...
function metis_auth_rate_limit_check( string $bucket, int $limit = 10, int $window = 300 ): void {
    metis_auth_protection_service()->assertRequestRateLimit( sanitize_key( $bucket ) );
}
...
function metis_auth_handle_request( Metis_Http_Request $request ): bool {
    ...
    if ( metis_auth_has_users() || metis_auth_legacy_people_without_auth_count() > 0 ) {
        ...
        $body .= '<button type="button" id="metis-google-sso-button" class="mw-btn-secondary">Sign in with Google</button>';
        $body .= '</div></form>';
