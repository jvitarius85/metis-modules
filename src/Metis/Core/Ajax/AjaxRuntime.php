...
function metis_ajax_legacy_hook_name( string $action ): string {
    return 'legacy_ajax_' . metis_key_clean( $action );
}

function metis_ajax_legacy_handler( string $action ): callable|null {
    $hook = metis_ajax_legacy_hook_name( $action );

    $has_hook = function_exists( 'metis_runtime_has_action' ) && metis_runtime_has_action( $hook );
    $has_hook = $has_hook || ( function_exists( 'metis_has_action' ) && metis_has_action( $hook ) );
    if ( ! $has_hook ) {
        return null;
    }

    return static function () use ( $hook ): void {
        if ( function_exists( 'metis_runtime_do_action' ) ) {
            metis_runtime_do_action( $hook );
            return;
        }
...
function metis_ajax_dispatch_handler( string $action ): array {
    ...
    if ( $handler === null ) {
        $handler = metis_ajax_legacy_handler( $action );
    }
    if ( $handler === null ) {
...
function metis_ajax_dispatch_legacy_action( string $action ): array {
    return metis_ajax_dispatch_handler( metis_key_clean( $action ) );
}
