...
final class TemplateService {
    ...
    private const DIRECTORY_ALIASES = [
        'image_overlay_banner' => 'default',
    ];
    ...
    public static function getActiveTemplateSlug(): string {
        ...
        if ( $all === [] ) {
            return StructuredWebsiteBuilderService::defaultTemplateForPageType( 'page' );
        }

        return metis_key_clean(
            (string) ( $all[0]['slug'] ?? StructuredWebsiteBuilderService::defaultTemplateForPageType( 'page' ) )
        );
    }
    ...
    public static function discoverTemplates(): array {
        ...
        $template_keys = StructuredWebsiteBuilderService::templateKeys();
        $templates_by_slug = [];
        foreach ( $template_keys as $key ) {
            ...
            $folder_slug = $slug;
            if ( ! is_dir( $dir ) ) {
                $alias_dir_name = self::DIRECTORY_ALIASES[ $slug ] ?? '';
                if ( $alias_dir_name !== '' ) {
                    $alias_dir = $base_dir . '/' . $alias_dir_name;
                    if ( is_dir( $alias_dir ) ) {
                        $dir = $alias_dir;
                        $folder_slug = metis_key_clean( $alias_dir_name );
                    }
                }
            }
            if ( ! is_dir( $dir ) ) {
        ...
        $order = array_flip( StructuredWebsiteBuilderService::templateKeys() );
        usort(
            $templates,
    ...
    /**
     * Use a single canonical template directory and ignore legacy alternates.
     */
    private static function templatesBaseDir(): string {
        $root = dirname( __DIR__, 5 ) . '/modules/website';
