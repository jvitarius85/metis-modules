...
final class WebsiteRenderer {
    ...
    public static function renderHomepagePlaceholder(): string {
        ...
        $asset_base = dirname( (string) ( $_SERVER['SCRIPT_NAME'] ?? '/index.php' ) ) . '/assets/error-pages/';
        $css_href   = $asset_base . 'metis-errors.css';
        $logo_src   = $asset_base . 'metis-shield-logo.png';
        ...
        $html .= '<link rel="stylesheet" href="' . metis_escape_attr( $css_href ) . '">';
    ...
    private static function tokenValuesForRender( array $input, array $context, array $page_data ): array {
        ...
        if ( $content_type === 'post' ) {
            ...
            $post_excerpt = trim( (string) ( $page_data['excerpt'] ?? $input['description'] ?? '' ) );
        }
    ...
    private static function buildPostHeaderHtml( array $page_data ): string {
        ...
        $excerpt = trim( (string) ( $page_data['excerpt'] ?? '' ) );

        if ( $title === '' && $date_raw === '' && $author === '' && $excerpt === '' ) {
            return '';
        }
        ...
        if ( $excerpt !== '' ) {
            $html .= '<p class="metis-template-post-excerpt">' . metis_escape_html( $excerpt ) . '</p>';
        }
        $html .= '</header>';
    ...
    private static function bodyClassList( array $context, string $site_layout_profile_key, string $menu_style, array $menu_config = [] ): array {
        ...
        $mobile = is_array( $menu_config['mobile'] ?? null ) ? $menu_config['mobile'] : [];
        $mobile_type = metis_key_clean( (string) ( $mobile['menu_type'] ?? '' ) );
        if ( in_array( $mobile_type, [ 'slide', 'overlay' ], true ) ) {
            $classes[] = 'metis-menu-mobile-' . self::safeHtmlClass( $mobile_type );
            if ( $mobile_type === 'overlay' ) {
                $classes[] = 'metis-menu-mobile-overlay';
            }
        }
        $mobile_button_style = metis_key_clean( (string) ( $mobile['button_style'] ?? '' ) );
        if ( in_array( $mobile_button_style, [ 'square', 'rounded' ], true ) ) {
            $classes[] = 'metis-menu-mobile-btn-' . self::safeHtmlClass( $mobile_button_style );
        }
    ...
    private static function menuBodyDataAttributes( array $menu_config ): string {
        $mobile = is_array( $menu_config['mobile'] ?? null ) ? $menu_config['mobile'] : [];
        $breakpoint = isset( $mobile['breakpoint'] ) ? (int) $mobile['breakpoint'] : 980;
        if ( $breakpoint < 480 ) {
            $breakpoint = 480;
        } elseif ( $breakpoint > 1600 ) {
            $breakpoint = 1600;
        }
        $menu_type = metis_key_clean( (string) ( $mobile['menu_type'] ?? 'slide' ) );
        if ( ! in_array( $menu_type, [ 'slide', 'overlay' ], true ) ) {
            $menu_type = 'slide';
        }
        $dropdown = is_array( $menu_config['dropdown'] ?? null ) ? $menu_config['dropdown'] : [];
        $dropdown_behavior = self::sanitizeMenuDropdownBehavior( (string) ( $dropdown['behavior'] ?? 'hover' ) );
        return ' data-metis-nav-breakpoint="' . metis_escape_attr( (string) $breakpoint ) . '" data-metis-nav-mobile-type="' . metis_escape_attr( $menu_type ) . '" data-metis-nav-dropdown-behavior="' . metis_escape_attr( $dropdown_behavior ) . '"';
    }
    ...
    private static function assetVersionToken( array $template_structure = [], bool $include_theme_state = false ): string {
        ...
        $candidates = [
            METIS_ROOT . '/src/Metis/Modules/Website/Services/WebsiteRenderer.php',
            METIS_ROOT . '/src/Metis/Modules/Website/Services/ThemeService.php',
            METIS_ROOT . '/modules/website/assets/public-navigation.js',
        ];
        $template_slug = self::templateVariantSlug( $template_structure );
        if ( $template_slug !== '' ) {
            $candidates[] = METIS_ROOT . '/modules/website/Templates/' . $template_slug . '/structure.css';
            $candidates[] = METIS_ROOT . '/modules/website/Templates/' . $template_slug . '/menu.css';
        }
        $latest = 0;
    ...
    private static function renderShellPartial( string $name, array $vars ): string {
        ...
        $path = dirname( __DIR__, 5 ) . '/modules/website/Templates/shell/' . $slug . '.php';
        if ( ! is_file( $path ) ) {
            return '';
        }
    ...
    public static function renderThemeMenuPreviewHtml(): string {
        ...
        if ( trim( $menu_html ) === '' ) {
            $menu_html = '<ul class="metis-shell-menu-list"><li class="metis-shell-menu-item"><a href="#" class="metis-shell-menu-link">About</a></li><li class="metis-shell-menu-item"><a href="#" class="metis-shell-menu-link">Programs</a></li><li class="metis-shell-menu-item"><a href="#" class="metis-shell-menu-link metis-shell-menu-btn metis-shell-menu-btn--mw_primary" data-metis-nav-url="#">Join Us</a></li></ul>';
        }
    ...
    private static function renderShellMenuItems( array $items, string $current_path = '' ): string {
        ...
        foreach ( $items as $item ) {
            ...
            $button_color_key = metis_key_clean( (string) ( $item['button_color_key'] ?? 'mw_primary' ) );
            if ( ! in_array( $button_color_key, [ 'mw_primary', 'mw_accent', 'mw_text', 'mw_surface' ], true ) ) {
                $button_color_key = 'mw_primary';
            }
            $children = is_array( $item['children'] ?? null ) ? $item['children'] : [];
            ...
            if ( $as_button ) {
                if ( $has_navigable_url ) {
                    ...
                    $button_has_popup = $has_children ? ' aria-haspopup="true"' : '';
                    $html .= '<a href="' . metis_escape_attr( $url ) . '" class="metis-shell-menu-link metis-shell-menu-btn metis-shell-menu-btn--'
                        . metis_escape_attr( $button_color_key ) . '" data-metis-nav-url="' . metis_escape_attr( $url ) . '"' . $button_aria . $button_has_popup . '>'
                        . metis_escape_html( $label ) . $submenu_indicator . '</a>';
                } else {
                    $label_focus = $has_children ? ' tabindex="0" aria-haspopup="true"' : '';
                    $html .= '<span class="' . metis_escape_attr( $label_class ) . '"' . $label_focus . '>'
                        . metis_escape_html( $label ) . $submenu_indicator . '</span>';
                }
            } else {
                if ( $has_navigable_url ) {
                    $link_attrs = ' class="metis-shell-menu-link"' . ( $has_children ? ' aria-haspopup="true"' : '' ) . ( $self_active ? ' aria-current="page"' : '' );
    ...
    private static function templatePreviewIsolationCss(): string {
        return '.metis-template-preview-content-surface{background:#fff;border:1px solid #dbe4f2;border-radius:12px;padding:24px;max-width:920px;margin:24px auto;}'
            ...
            . '.metis-template-preview-hero{border:1px solid #cfdbef;border-radius:16px;min-height:170px;padding:18px;margin:18px 0;background:linear-gradient(135deg,#f7faff 0%,#eef3ff 100%);}'
    ...
    private static function publicBaseCss(): string {
        return implode( "\n", [
            ...
            '.metis-shell-nav-toggle{display:none;align-items:center;justify-content:center;padding:8px 12px;border:1px solid var(--metis-color-border,#d8deea);border-radius:8px;background:#fff;color:var(--metis-color-text,#1a1f2b);font-weight:600;cursor:pointer;}',
            ...
            '.metis-shell-menu-item{position:relative;}',
            ...
            '.metis-shell-menu-item>.metis-shell-menu-btn--mw_primary,.metis-shell-menu-item>.metis-shell-menu-btn--mw_accent,.metis-shell-menu-item>.metis-shell-menu-btn--mw_text,.metis-shell-menu-item>.metis-shell-menu-btn--mw_surface{background:var(--metis-menu-button-bg,var(--metis-color-primary,#485bc7));color:var(--metis-menu-button-text,#fff) !important;border:1px solid var(--metis-menu-button-border,transparent);}',
            ...
            '.metis-public-content .metis-transcript__row--intro .metis-transcript__body,.metis-public-content .metis-transcript__row--lead .metis-transcript__body,.metis-structured-section__content .metis-transcript__row--intro .metis-transcript__body,.metis-structured-section__content .metis-transcript__row--lead .metis-transcript__body{max-width:min(100%,66ch);text-align:left;border-radius:28px;border-color:color-mix(in srgb,var(--metis-color-primary,#485bc7) 14%,transparent);background:linear-gradient(160deg,color-mix(in srgb,var(--metis-color-surface,#ffffff) 30%,transparent),color-mix(in srgb,var(--metis-color-primary,#485bc7) 6%,transparent));box-shadow:0 18px 36px rgba(15,23,42,.05);padding:1.2rem 1.25rem 1.25rem;}',
            ...
            'body.metis-menu-style-h_pill .metis-shell-nav .metis-shell-menu-item>.metis-shell-menu-link{padding:8px 12px;border-radius:999px;background:var(--metis-color-surface,#fff);border:1px solid var(--metis-color-border,#d8deea);}',
            'body.metis-menu-style-h_underline .metis-shell-nav .metis-shell-menu-item>.metis-shell-menu-link{padding-bottom:8px;border-bottom:2px solid transparent;}',
            'body.metis-menu-style-h_underline .metis-shell-nav .metis-shell-menu-item>.metis-shell-menu-link:hover{border-bottom-color:var(--metis-color-primary,#485bc7);}',
            'body.metis-menu-style-v_sidebar_clean .metis-shell-nav-primary .metis-shell-menu-list,body.metis-menu-style-v_sidebar_cards .metis-shell-nav-primary .metis-shell-menu-list,body.metis-menu-style-v_sidebar_compact .metis-shell-nav-primary .metis-shell-menu-list{flex-direction:column;align-items:stretch;}',
            'body.metis-menu-style-v_sidebar_clean .metis-shell-nav-primary,body.metis-menu-style-v_sidebar_cards .metis-shell-nav-primary,body.metis-menu-style-v_sidebar_compact .metis-shell-nav-primary{position:fixed;right:0;top:96px;z-index:1100;width:min(300px,27vw);max-height:calc(100vh - 120px);overflow:auto;padding:14px 12px;border-left:1px solid var(--metis-color-border,#d8deea);background:var(--metis-color-surface,#fff);box-shadow:-10px 0 26px rgba(2,8,23,.10);}',
            'body.metis-menu-style-v_sidebar_clean .metis-shell-body,body.metis-menu-style-v_sidebar_cards .metis-shell-body,body.metis-menu-style-v_sidebar_compact .metis-shell-body,body.metis-menu-style-v_sidebar_clean .metis-shell-footer,body.metis-menu-style-v_sidebar_cards .metis-shell-footer,body.metis-menu-style-v_sidebar_compact .metis-shell-footer{padding-right:calc(max(16px,3vw) + min(300px,27vw));}',
            'body.metis-menu-style-v_sidebar_clean .metis-shell-nav-primary .metis-shell-menu-item>.metis-shell-menu-link{padding:8px 10px;border-left:2px solid transparent;}',
            'body.metis-menu-style-v_sidebar_clean .metis-shell-nav-primary .metis-shell-menu-item>.metis-shell-menu-link:hover{border-left-color:var(--metis-color-primary,#485bc7);}',
            'body.metis-menu-style-v_sidebar_cards .metis-shell-nav-primary{padding:10px;border:1px solid var(--metis-color-border,#d8deea);border-radius:12px;background:var(--metis-color-surface,#fff);}',
            'body.metis-menu-style-v_sidebar_cards .metis-shell-nav-primary .metis-shell-menu-item>.metis-shell-menu-link{padding:10px 12px;border-radius:10px;background:var(--metis-color-surface_alt,#f8fafc);}',
            'body.metis-menu-style-v_sidebar_compact .metis-shell-nav-primary .metis-shell-menu-item>.metis-shell-menu-link{padding:6px 8px;font-size:12px;letter-spacing:.03em;text-transform:uppercase;}',
            ...
            'body.metis-layout-centered_editorial .metis-shell-header{position:relative;background:linear-gradient(180deg,#f9fbff,#ffffff 66%);}',
            ...
            'body.metis-layout-impact_campaign .metis-shell-header{background:linear-gradient(180deg,var(--metis-color-surface,#fff),rgba(255,255,255,.9));backdrop-filter:blur(8px);}',
            ...
            'body.metis-layout-showcase_grid .metis-shell-header{position:relative;border-bottom:0;background:linear-gradient(120deg,rgba(255,255,255,.95),rgba(255,255,255,.78));}',
            ...
            'body.metis-layout-showcase_grid .metis-shell-main::before{content:"";position:absolute;inset:-22px -18px auto auto;width:220px;height:220px;background:radial-gradient(circle,var(--metis-color-accent,#ff7542) 0%,rgba(255,255,255,0) 70%);opacity:.14;pointer-events:none;}',
            ...
            '.metis-structured-section__inner{max-width:1200px;margin:0 auto;padding:48px 0;}',
            ...
            '.metis-structured-text .metis-inline-image{margin:18px auto;display:grid;justify-items:center;gap:10px;}',
            ...
            '.metis-structured-text .metis-inline-image img{display:block;max-width:min(720px,100%);height:auto;border-radius:16px;border:1px solid var(--metis-color-border,#dbe3ef);box-shadow:0 18px 40px rgba(15,23,42,.08);}',
            ...
            '.metis-structured-text .metis-text-color-mw_primary{color:var(--mw-primary,#485bc7);}',
            '.metis-structured-text .metis-text-color-mw_primary_dark{color:var(--mw-primary-dark,#3246a7);}',
            '.metis-structured-text .metis-text-color-mw_accent{color:var(--mw-accent,#ff7542);}',
            '.metis-structured-text .metis-text-color-mw_bg{color:var(--mw-bg,#f5f6fa);}',
            '.metis-structured-text .metis-text-color-mw_surface{color:var(--mw-surface,#ffffff);}',
            '.metis-structured-text .metis-text-color-mw_border{color:var(--mw-border,#e0e2ea);}',
            '.metis-structured-text .metis-text-color-mw_text{color:var(--mw-text,#1f2330);}',
            '.metis-structured-text .metis-text-color-mw_text_muted{color:var(--mw-text-muted,#6d7485);}',
            '.metis-structured-text .metis-text-color-mw_header_bg{color:var(--mw-header-bg,#eceeff);}',
            '.metis-structured-text .metis-text-color-mw_row_odd_bg{color:var(--mw-row-odd-bg,#ffffff);}',
            '.metis-structured-text .metis-text-color-mw_row_even_bg{color:var(--mw-row-even-bg,#f8f9fd);}',
            '.metis-structured-text .metis-text-color-mw_row_hover_bg{color:var(--mw-row-hover-bg,#eef2ff);}',
            '.metis-structured-text .metis-text-color-mw_sidebar_bg{color:var(--mw-sidebar-bg,#16192b);}',
            '.metis-structured-text .metis-text-color-mw_sidebar_icon_color{color:var(--mw-sidebar-icon-color,#7a82a6);}',
            '.metis-structured-text .metis-text-color-mw_sidebar_active_color{color:var(--mw-sidebar-active-color,#a8b4ff);}',
            ...
            '.metis-structured-events__item,.metis-structured-posts-list__item{background:linear-gradient(180deg,color-mix(in srgb,var(--metis-color-surface_alt,#f8fafc) 82%, transparent),color-mix(in srgb,var(--metis-color-accent,#ff7542) 8%, transparent));border:1px solid color-mix(in srgb,var(--metis-color-accent,#ff7542) 42%, var(--metis-color-border,#dbe3ef));border-radius:22px;padding:20px;box-shadow:0 14px 28px rgba(15,23,42,.06);backdrop-filter:blur(8px);}',
            ...
            '.metis-structured-events__title,.metis-structured-posts-list__title{margin:0 0 8px;font-size:1.08rem;line-height:1.3;}',
            '.metis-structured-posts-list__media{display:block;margin:-20px -20px 16px;}',
            '.metis-structured-posts-list__media img{display:block;width:100%;aspect-ratio:16/9;object-fit:cover;border-radius:22px 22px 0 0;border-bottom:1px solid color-mix(in srgb,var(--metis-color-border,#dbe3ef) 70%, transparent);}',
            '.metis-structured-posts-list__byline,.metis-structured-posts-list__categories{margin:0 0 10px;color:var(--metis-color-muted,#64748b);font-size:.95rem;line-height:1.5;}',
            ...
            '.metis-structured-posts-list__excerpt p{margin:0 0 10px;}',
            '.metis-structured-posts-list__cta{margin:0;}',
    ...
    private static function readMenuCssAsset( string $variant ): string {
        ...
        $path = dirname( __DIR__, 6 ) . '/assets/css/website/menu-' . $name . '.css';
    ...
    private static function templateBaseCss(): string {
        return implode( "\n", [
            ...
            '.metis-template-hero-style-overlay{position:relative;overflow:hidden;min-height:240px;border-radius:18px;padding:24px;background:linear-gradient(140deg,var(--metis-color-surface,#fff),var(--metis-color-surface_alt,#f8fafc));}',
            ...
            '.metis-template select{appearance:none;-webkit-appearance:none;-moz-appearance:none;background-image:linear-gradient(45deg,transparent 50%, #64748b 50%),linear-gradient(135deg,#64748b 50%,transparent 50%);background-position:calc(100% - 14px) calc(50% - 2px),calc(100% - 9px) calc(50% - 2px);background-size:5px 5px,5px 5px;background-repeat:no-repeat;padding-right:30px;}',
            ...
            '.metis-template-post-excerpt{color:var(--metis-color-muted,#64748b);margin:0;}',
            ...
            '.metis-template-hero-region-glass{background:linear-gradient(135deg,rgba(255,255,255,.95),rgba(248,251,255,.9));border-radius:18px;padding:20px;box-shadow:0 20px 60px rgba(15,23,42,.08);}',
            '.metis-template-header-glass::before{background:linear-gradient(140deg,rgba(255,255,255,.92),rgba(238,244,255,.96));border-bottom:1px solid rgba(148,163,184,.25);}',
            ...
            '.metis-template-banner-inner{border-radius:18px;padding:22px;background:linear-gradient(140deg,rgba(15,23,42,.06),rgba(99,102,241,.10));display:grid;gap:16px;align-items:center;}',
    ...
    private static function themeTemplateMenuCss(): string {
        return implode( "\n", [
            ...
            '.metis-template .metis-template-menu{display:flex !important;align-items:center !important;justify-content:flex-end !important;min-width:0;}',
            '.metis-template .metis-template-menu .metis-shell-menu-list{display:flex !important;justify-content:flex-end !important;align-items:var(--metis-menu-item-align,center) !important;flex-wrap:wrap !important;gap:10px var(--metis-menu-item-gap,16px) !important;list-style:none;margin:0;padding:0;}',
            ...
            '.metis-template .metis-template-menu .metis-shell-menu-item>.metis-shell-menu-btn--mw_primary,.metis-template .metis-template-menu .metis-shell-menu-item>.metis-shell-menu-btn--mw_accent,.metis-template .metis-template-menu .metis-shell-menu-item>.metis-shell-menu-btn--mw_text,.metis-template .metis-template-menu .metis-shell-menu-item>.metis-shell-menu-btn--mw_surface{background:var(--metis-menu-button-bg,var(--metis-color-primary,#485bc7));color:var(--metis-menu-button-text,#fff);border:1px solid var(--metis-menu-button-border,transparent);}',
            ...
            '.metis-template .metis-template-menu .metis-shell-menu-sub{padding:6px;border-radius:var(--metis-menu-dropdown-radius,10px);}',
            ...
            'body.metis-menu-style-h_pill .metis-template .metis-template-menu .metis-shell-menu-item>.metis-shell-menu-link{padding:8px 12px;border-radius:999px;background:var(--metis-color-surface,#fff);border:1px solid var(--metis-color-border,#d8deea);}',
            'body.metis-menu-style-h_underline .metis-template .metis-template-menu .metis-shell-menu-item>.metis-shell-menu-link{padding-bottom:8px;border-bottom:2px solid transparent;}',
            'body.metis-menu-style-h_underline .metis-template .metis-template-menu .metis-shell-menu-item>.metis-shell-menu-link:hover{border-bottom-color:var(--metis-color-primary,#485bc7);}',
            'body.metis-menu-style-v_sidebar_clean .metis-template .metis-template-menu .metis-shell-menu-list,body.metis-menu-style-v_sidebar_cards .metis-template .metis-template-menu .metis-shell-menu-list,body.metis-menu-style-v_sidebar_compact .metis-template .metis-template-menu .metis-shell-menu-list{flex-direction:column;align-items:stretch;}',
            'body.metis-menu-style-v_sidebar_clean .metis-template .metis-template-menu,body.metis-menu-style-v_sidebar_cards .metis-template .metis-template-menu,body.metis-menu-style-v_sidebar_compact .metis-template .metis-template-menu{position:fixed;right:0;top:96px;z-index:1100;width:min(300px,27vw);max-height:calc(100vh - 120px);overflow:auto;padding:14px 12px;border-left:1px solid var(--metis-color-border,#d8deea);background:var(--metis-color-surface,#fff);box-shadow:-10px 0 26px rgba(2,8,23,.10);}',
            'body.metis-menu-style-v_sidebar_clean .metis-template .metis-template-main,body.metis-menu-style-v_sidebar_cards .metis-template .metis-template-main,body.metis-menu-style-v_sidebar_compact .metis-template .metis-template-main,body.metis-menu-style-v_sidebar_clean .metis-template .metis-template-footer,body.metis-menu-style-v_sidebar_cards .metis-template .metis-template-footer,body.metis-menu-style-v_sidebar_compact .metis-template .metis-template-footer{padding-right:min(300px,27vw);}',
            'body.metis-menu-style-v_sidebar_clean .metis-template .metis-template-menu .metis-shell-menu-item>.metis-shell-menu-link{padding:8px 10px;border-left:2px solid transparent;}',
            'body.metis-menu-style-v_sidebar_clean .metis-template .metis-template-menu .metis-shell-menu-item>.metis-shell-menu-link:hover{border-left-color:var(--metis-color-primary,#485bc7);}',
            'body.metis-menu-style-v_sidebar_cards .metis-template .metis-template-menu{padding:10px;border:1px solid var(--metis-color-border,#d8deea);border-radius:12px;background:var(--metis-color-surface,#fff);}',
            'body.metis-menu-style-v_sidebar_cards .metis-template .metis-template-menu .metis-shell-menu-item>.metis-shell-menu-link{padding:10px 12px;border-radius:10px;background:var(--metis-color-surface_alt,#f8fafc);}',
            'body.metis-menu-style-v_sidebar_compact .metis-template .metis-template-menu .metis-shell-menu-item>.metis-shell-menu-link{padding:6px 8px;font-size:12px;letter-spacing:.03em;text-transform:uppercase;}',
            '@media (max-width: 980px){.metis-template .metis-template-header-inner{display:flex !important;flex-direction:column !important;align-items:flex-start !important;}.metis-template .metis-template-menu{justify-content:flex-start !important;width:100%;}.metis-template .metis-template-menu .metis-shell-menu-list{justify-content:flex-start !important;gap:8px 12px !important;}body.metis-menu-style-v_sidebar_clean .metis-template .metis-template-menu,body.metis-menu-style-v_sidebar_cards .metis-template .metis-template-menu,body.metis-menu-style-v_sidebar_compact .metis-template .metis-template-menu{position:static;width:100%;max-height:none;box-shadow:none;border-left:0;padding:0;}body.metis-menu-style-v_sidebar_clean .metis-template .metis-template-main,body.metis-menu-style-v_sidebar_cards .metis-template .metis-template-main,body.metis-menu-style-v_sidebar_compact .metis-template .metis-template-main,body.metis-menu-style-v_sidebar_clean .metis-template .metis-template-footer,body.metis-menu-style-v_sidebar_cards .metis-template .metis-template-footer,body.metis-menu-style-v_sidebar_compact .metis-template .metis-template-footer{padding-right:0;}}',
        ] );
    }
