...
final class StructuredWebsiteBuilderService {
    ...
    /** @var array<string,string> */
    private const LEGACY_TEMPLATE_ALIASES = [
        'default' => 'image_overlay_banner',
        'home_default' => 'hero_split_glass',
        'page_default' => 'centered_stack_marketing',
        'post_default' => 'editorial_focus',
        'hero_focus_wide' => 'hero_split_glass',
        'center_nav_minimal' => 'centered_stack_marketing',
        'clean_nav_left' => 'image_overlay_banner',
        'visual_grid' => 'modular_grid_dash',
        'content_sidebar' => 'editorial_focus',
        'clean_minimal' => 'compact_app_style',
    ];

    /** @var array<string,string> */
    private const PAGE_TYPE_DEFAULT_TEMPLATE = [
        'homepage' => 'hero_split_glass',
        'page' => 'centered_stack_marketing',
        'post' => 'editorial_focus',
    ];
    ...
    public static function templateLabel( string $template_key ): string {
        $normalized = self::normalizeTemplateAlias( $template_key );
    ...
    public static function isStructuredTemplateKey( string $template_key ): bool {
        return in_array( self::normalizeTemplateAlias( $template_key ), self::templateKeys(), true );
    }
    ...
    public static function defaultTemplateForPageType( string $page_type ): string {
        ...
        return self::PAGE_TYPE_DEFAULT_TEMPLATE[ $type ] ?? 'centered_stack_marketing';
    }

    public static function resolveTemplateKey( string $raw_template_key, string $page_type = 'page' ): string {
        $template = self::normalizeTemplateAlias( $raw_template_key );
    ...
    public static function sectionsFromLayout( mixed $raw_layout, bool $is_post = false ): array {
        ...
        $sections = isset( $meta['sections'] ) && is_array( $meta['sections'] )
            ...
            : [];
        if ( $sections === [] ) {
            $legacy_html = self::legacyHtmlFromDecodedLayout( $decoded );
            if ( $legacy_html !== '' ) {
                $sections = [
                    [
                        'id' => 'section_0',
    ...
    public static function isLegacySimpleLayout( mixed $raw_layout ): bool {
        $decoded = self::decodeLayout( $raw_layout );
        $meta = self::structuredMetaFromDecodedLayout( $decoded );
        $sections = isset( $meta['sections'] ) && is_array( $meta['sections'] )
            ? $meta['sections']
        ...
        return self::legacyHtmlFromDecodedLayout( $decoded ) !== '';
    }

    public static function migrateLegacySimpleLayout( mixed $raw_layout, array $options = [] ): ?string {
        if ( ! self::isLegacySimpleLayout( $raw_layout ) ) {
            return null;
        }
        ...
        $sections = self::sectionsFromLayout( $raw_layout, $is_post );
        $layout = self::buildLayoutArray( $sections, $page_type, $template_key, $hero );

        return self::encodeJson( $layout );
    }
    ...
    private static function legacyHtmlFromDecodedLayout( array $decoded ): string {
        $meta = isset( $decoded['editor_meta'] ) && is_array( $decoded['editor_meta'] ) ? $decoded['editor_meta'] : [];
        $candidates = [
            isset( $meta['simple_html'] ) ? (string) $meta['simple_html'] : '',
            isset( $meta['html'] ) ? (string) $meta['html'] : '',
            isset( $decoded['html'] ) ? (string) $decoded['html'] : '',
            isset( $decoded['content_html'] ) ? (string) $decoded['content_html'] : '',
        ];

        foreach ( $candidates as $candidate ) {
            $html = self::sanitizeRichText( $candidate );
    ...
    private static function normalizeTemplateAlias( string $template_key ): string {
        $candidate = metis_key_clean( $template_key );
        if ( $candidate === '' ) {
            return '';
        }
        if ( isset( self::LEGACY_TEMPLATE_ALIASES[ $candidate ] ) ) {
            return (string) self::LEGACY_TEMPLATE_ALIASES[ $candidate ];
        }
        return $candidate;
    }
