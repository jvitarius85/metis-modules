...
final class SchemaManager {
    ...
    public static function dropIndexIfExists( string $table, string $index_name ): void {
        ...
        $index = $wpdb->get_var( $wpdb->prepare( "SHOW INDEX FROM {$table} WHERE Key_name = %s", $index_name ) );
        if ( $index === null ) {
            return;
        }
    ...
    public static function ensureSchema(): void {
        ...
        $wpdb->query( "UPDATE {$people_table} SET auth_provider = 'metis' WHERE auth_provider IS NULL OR auth_provider = '' OR auth_provider NOT IN ('metis', 'workspace')" );
        ...
        $wpdb->query( "UPDATE {$roles_table} SET role_domain = 'metis' WHERE role_domain IS NULL OR role_domain = ''" );
        ...
        \dbDelta( $workspace_user_roles_sql );
        ...
        \dbDelta( $workspace_group_members_sql );
        ...
        \dbDelta( $workspace_security_actions_sql );
        ...
        \dbDelta( $workspace_sync_jobs_sql );

        if ( ! self::tableExists( $people_table ) ) { $wpdb->query( $people_sql ); }
        if ( ! self::tableExists( $roles_table ) ) { $wpdb->query( $roles_sql ); }
        if ( ! self::tableExists( $perms_table ) ) { $wpdb->query( $perms_sql ); }
        if ( ! self::tableExists( $role_perms_table ) ) { $wpdb->query( $role_perms_sql ); }
        if ( ! self::tableExists( $user_roles_table ) ) { $wpdb->query( $user_roles_sql ); }
        if ( ! self::tableExists( $activity_table ) ) { $wpdb->query( $activity_sql ); }
        if ( ! self::tableExists( $requests_table ) ) { $wpdb->query( $requests_sql ); }
        if ( ! self::tableExists( $templates_table ) ) { $wpdb->query( $templates_sql ); }
        if ( ! self::tableExists( $template_roles_table ) ) { $wpdb->query( $template_roles_sql ); }
        if ( ! self::tableExists( $documents_table ) ) { $wpdb->query( $documents_sql ); }
        if ( ! self::tableExists( $emergency_access_table ) ) { $wpdb->query( $emergency_access_sql ); }
        if ( ! self::tableExists( $passkeys_table ) ) { $wpdb->query( $passkeys_sql ); }
        if ( ! self::tableExists( $challenges_table ) ) { $wpdb->query( $challenges_sql ); }
        if ( ! self::tableExists( $lifecycle_tasks_table ) ) { $wpdb->query( $lifecycle_tasks_sql ); }
        if ( ! self::tableExists( $workspace_users_table ) ) { $wpdb->query( $workspace_users_sql ); }
        if ( ! self::tableExists( $workspace_user_roles_table ) ) { $wpdb->query( $workspace_user_roles_sql ); }
        if ( ! self::tableExists( $workspace_groups_table ) ) { $wpdb->query( $workspace_groups_sql ); }
        if ( ! self::tableExists( $workspace_group_members_table ) ) { $wpdb->query( $workspace_group_members_sql ); }
        if ( ! self::tableExists( $workspace_security_actions_table ) ) { $wpdb->query( $workspace_security_actions_sql ); }
        if ( ! self::tableExists( $workspace_sync_jobs_table ) ) { $wpdb->query( $workspace_sync_jobs_sql ); }
