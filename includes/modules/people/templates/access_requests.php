...
$metis_roles = $wpdb->get_results("SELECT role_key, role_name FROM {$roles_table} WHERE role_domain='metis' ORDER BY role_name ASC", ARRAY_A) ?: [];
?>

<div class="metis-people-ops">
    <h1 class="mw-page-title">Access Requests</h1>
            ...
            <div class="metis-contact-field metis-contact-field-half">
                ...
                <select id="metis-access-role-key" class="mw-select" required>
                    ...
                    <?php foreach ($metis_roles as $role): ?>
                        <option value="<?php echo esc_attr((string) $role['role_key']); ?>"><?php echo esc_html((string) $role['role_name']); ?></option>
                    <?php endforeach; ?>
                </select>
            </div>
    ...
    <section class="mw-premium-table" style="margin-top:14px;">
        ...
        <?php foreach ($rows as $row): ?>
            <div class="mw-premium-row" style="display:grid;grid-template-columns:120px 120px 130px 170px 160px 130px 200px 1fr 170px;align-items:center;">
                <div class="mw-premium-cell"><?php echo esc_html((string) $row['request_code']); ?></div>
                <div class="mw-premium-cell"><?php echo esc_html((string) $row['status']); ?></div>
                <div class="mw-premium-cell"><?php echo esc_html((string) $row['target_pid']); ?></div>
                <div class="mw-premium-cell"><?php echo esc_html((string) $row['target_name']); ?></div>
                <div class="mw-premium-cell"><?php echo esc_html((string) $row['role_name']); ?></div>
                <div class="mw-premium-cell"><?php echo esc_html((string) ((int) ($row['approval_count'] ?? 0) . '/' . (int) ($row['required_approvals'] ?? 0))); ?></div>
                <div class="mw-premium-cell">
                    ...
                    echo esc_html(!empty($window) ? implode(' | ', $window) : '—');
                    ?>
                </div>
                <div class="mw-premium-cell">
                    <div><?php echo esc_html((string) $row['reason']); ?></div>
                    <?php if (!empty($row['decision_note'])): ?>
                        <div class="mw-muted">Decision: <?php echo esc_html((string) $row['decision_note']); ?></div>
                    <?php endif; ?>
                </div>
                <div class="mw-premium-cell">
                    <?php if ($can_manage && (string) $row['status'] === 'pending'): ?>
                        <button type="button" class="mw-btn-xs metis-access-resolve" data-id="<?php echo esc_attr((string) $row['id']); ?>" data-decision="approved">Approve</button>
                        <button type="button" class="mw-btn-xs mw-btn-danger metis-access-resolve" data-id="<?php echo esc_attr((string) $row['id']); ?>" data-decision="rejected">Reject</button>
                    <?php else: ?>
