...
$role_key_param = isset( $_GET['role'] ) ? sanitize_key( metis_unslash( $_GET['role'] ) ) : '';
$role_domain_param = isset( $_GET['domain'] ) ? sanitize_key( metis_unslash( $_GET['domain'] ) ) : 'metis';
...
if ( ! $is_new && $role_key_param !== '' ) {
    $role = $wpdb->get_row( $wpdb->prepare( "SELECT * FROM {$roles_table} WHERE role_key = %s AND role_domain = %s LIMIT 1", $role_key_param, $role_domain_param ), ARRAY_A );
    if ( ! $role ) {
...
$permissions_rows = $wpdb->get_results( "SELECT * FROM {$perms_table} ORDER BY module_slug ASC, action_key ASC", ARRAY_A ) ?: [];
...
if ( $role ) {
    $selected_permission_keys = $wpdb->get_col( $wpdb->prepare(
    ...
    $assigned_people = (int) $wpdb->get_var( $wpdb->prepare(
        "SELECT COUNT(*) FROM {$user_roles_table} WHERE role_id = %d",
        (int) $role['id']
    ) );

    $people_table = Metis_Tables::get( 'people' );
    $assigned_people_rows = $wpdb->get_results( $wpdb->prepare(
...
<div class="metis-people-role-detail" data-can-manage="<?php echo esc_attr( $can_manage ? '1' : '0' ); ?>">
    <div id="metis-people-alert" class="mw-alert" style="display:none;"></div>

    <div class="metis-people-detail-header">
        <div>
            <h1 class="mw-page-title"><?php echo esc_html( $is_new ? 'Add Role' : (string) ( $role['role_name'] ?? 'Role' ) ); ?></h1>
            <p class="mw-subtitle"><?php echo esc_html( $is_new ? 'Define permissions for a new Metis role.' : 'Role Key: ' . (string) ( $role['role_key'] ?? '' ) ); ?></p>
        </div>
        <div class="metis-top-actions">
            <a href="<?php echo esc_url( metis_people_roles_list_url() ); ?>" class="mw-btn mw-btn-ghost metis-top-action-btn">Back to Roles</a>
        </div>
    </div>
    ...
    <div class="metis-people-role-grid">
        <section class="mw-premium-wrap">
            ...
            <div class="metis-role-summary-stats">
                <div class="metis-role-summary-stat">
                    ...
                    <div class="metis-role-summary-stat-value"><?php echo esc_html( (string) $assigned_people ); ?></div>
                </div>
                <div class="metis-role-summary-stat">
                    <div class="metis-role-summary-stat-label">Permissions</div>
                    <div class="metis-role-summary-stat-value"><?php echo esc_html( (string) count( $selected_permission_keys ) ); ?></div>
                </div>
                <div class="metis-role-summary-stat">
                    <div class="metis-role-summary-stat-label">Domain</div>
                    <div class="metis-role-summary-stat-value" style="font-size:16px;padding-top:4px;"><?php echo esc_html( ucfirst( (string) ( $role['role_domain'] ?? 'metis' ) ) ); ?></div>
                </div>
                <div class="metis-role-summary-stat">
            ...
            <?php if ( ! empty( $assigned_people_rows ) ) : ?>
                ...
                <div class="metis-role-members-list">
                    ...
                    ?>
                        <a class="metis-role-member-chip" href="<?php echo esc_url( metis_people_person_url( (string) ( $member['pid'] ?? '' ) ) ); ?>">
                            <?php echo esc_html( $member_name !== '' ? $member_name : (string) ( $member['email'] ?? 'Person' ) ); ?>
                            <span class="mw-muted"><?php echo esc_html( (string) ( $member['pid'] ?? '' ) ); ?></span>
                        </a>
                    <?php endforeach; ?>
                </div>
            <?php else : ?>
        ...
        <section class="mw-premium-wrap">
            ...
            <form id="metis-role-detail-form" class="metis-contact-form">
                <input id="metis-role-id" type="hidden" value="<?php echo esc_attr( (string) ( $role['id'] ?? 0 ) ); ?>">
                <input id="metis-role-domain" type="hidden" value="<?php echo esc_attr( (string) ( $role['role_domain'] ?? 'metis' ) ); ?>">

                <div class="metis-contact-field metis-contact-field-half">
                    <label for="metis-role-key">Role Key</label>
                    <input id="metis-role-key" class="mw-input" type="text" value="<?php echo esc_attr( (string) ( $role['role_key'] ?? '' ) ); ?>" <?php disabled( ! $can_manage || ! empty( $role['is_system'] ) ); ?> required>
                </div>

                <div class="metis-contact-field metis-contact-field-half">
                    <label for="metis-role-name">Role Name</label>
                    <input id="metis-role-name" class="mw-input" type="text" value="<?php echo esc_attr( (string) ( $role['role_name'] ?? '' ) ); ?>" <?php disabled( ! $can_manage ); ?> required>
                </div>

                <div class="metis-contact-field metis-contact-field-full">
                    <label for="metis-role-description">Description</label>
                    <input id="metis-role-description" class="mw-input" type="text" value="<?php echo esc_attr( (string) ( $role['description'] ?? '' ) ); ?>" <?php disabled( ! $can_manage ); ?>>
                </div>
                    ...
                    <div id="metis-role-permissions" class="metis-people-perm-groups">
                        <?php foreach ( $permissions_by_module as $module_slug => $perms ) : ?>
                            <div class="metis-perm-group">
                                <div class="metis-perm-group-title"><?php echo esc_html( (string) $module_slug ); ?></div>
                                <div class="metis-people-check-grid">
                                    <?php foreach ( $perms as $perm ) :
                                        $perm_key = (string) ( $perm['permission_key'] ?? '' );
                                    ?>
                                        <label class="metis-people-check">
                                            <input type="checkbox" class="metis-role-perm-toggle" value="<?php echo esc_attr( $perm_key ); ?>" <?php checked( in_array( $perm_key, $selected_permission_keys, true ) ); ?> <?php disabled( ! $can_manage ); ?>>
                                            <?php echo esc_html( (string) ( $perm['permission_name'] ?? $perm_key ) ); ?>
                                        </label>
                                    <?php endforeach; ?>
                                </div>
                            </div>
                ...
                <?php if ( $can_manage ) : ?>
                    <div class="metis-contact-actions">
                        <a href="<?php echo esc_url( metis_people_base_url() ); ?>" class="mw-btn mw-btn-ghost">Cancel</a>
