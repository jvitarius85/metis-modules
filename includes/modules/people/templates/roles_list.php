...
$assign_rows = $wpdb->get_results("SELECT role_id FROM {$user_roles_table}", ARRAY_A) ?: [];
...
<div class="metis-people"
    ...
    <h1 class="mw-page-title">Roles</h1>
        ...
        <aside class="mw-list-sidebar">
            ...
            <div class="mw-list-sidebar-section">
                ...
                <div class="mw-list-sidebar-actions">
                    ...
                    <a href="<?php echo esc_url(metis_people_people_list_url()); ?>" class="mw-btn mw-btn-secondary">People</a>
                </div>
            </div>
            <?php if ($can_manage) : ?>
            <div class="mw-list-sidebar-section">
    ...
    <section class="mw-premium-table metis-roles-wrap">
        <div id="metis-role-rows">
            ...
            ?>
                <div class="metis-roles-domain-heading"><?php echo esc_html($domain_label); ?></div>
                ...
                <?php foreach ($domain_rows as $i => $r):
                    $rid = (int) ($r['id'] ?? 0);
                    $perm_count = (int) $wpdb->get_var( $wpdb->prepare("SELECT COUNT(*) FROM {$role_perms_table} WHERE role_id = %d AND allow_access = 1", $rid) );
                    $members_count = (int) ($role_member_count[$rid] ?? 0);
                    $is_last = ($i === count($domain_rows) - 1);
                ?>
                    <div class="mw-premium-row metis-role-row<?php echo $is_last ? ' is-last' : ''; ?>" data-href="<?php echo esc_url( metis_people_role_url( (string) ( $r['role_key'] ?? '' ), (string) ( $r['role_domain'] ?? 'metis' ) ) ); ?>">
                        <div class="mw-premium-cell metis-role-col-key"><?php echo esc_html((string) ($r['role_key'] ?? '')); ?></div>
                        <div class="mw-premium-cell metis-role-col-name"><?php echo esc_html((string) ($r['role_name'] ?? '')); ?></div>
                        <div class="mw-premium-cell metis-role-col-perms"><?php echo esc_html((string) $perm_count); ?></div>
                        <div class="mw-premium-cell metis-role-col-members"><?php echo esc_html((string) $members_count); ?></div>
                        <div class="mw-premium-cell metis-role-col-system"><?php echo !empty($r['is_system']) ? 'Yes' : 'No'; ?></div>
                        <?php if ($can_manage) : ?><div class="mw-premium-cell metis-role-col-actions"><a href="<?php echo esc_url( metis_people_role_url( (string) ( $r['role_key'] ?? '' ), (string) ( $r['role_domain'] ?? 'metis' ) ) ); ?>" class="mw-btn-xs">Edit</a></div><?php endif; ?>
                    </div>
                <?php endforeach; ?>
                </div>
            <?php endforeach; ?>
