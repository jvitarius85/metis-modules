...
$is_new = isset( $_GET['new'] ) && (string) metis_unslash( $_GET['new'] ) === '1';
$pid = isset( $_GET['pid'] ) ? sanitize_text_field( metis_unslash( $_GET['pid'] ) ) : '';

$person = null;
if ( ! $is_new && $pid !== '' ) {
    $person = $wpdb->get_row( $wpdb->prepare( "SELECT * FROM {$people_table} WHERE pid = %s LIMIT 1", $pid ), ARRAY_A );
    if ( ! $person ) {
        echo '<div class="mw-alert mw-alert-error">Person not found.</div>';
        return;
    }
...
$roles_rows = $wpdb->get_results( "SELECT * FROM {$roles_table} ORDER BY role_domain ASC, role_name ASC", ARRAY_A ) ?: [];
$metis_roles = [];
...
$avatar_src = metis_avatar_fallback_url( $person_email, 160 );
...
if ( $linked_donor_id !== '' ) {
    ...
    $linked_row = $wpdb->get_row(
        $wpdb->prepare( "SELECT first_name, last_name, email FROM {$contacts_table} WHERE did = %s LIMIT 1", $linked_donor_id ),
        ARRAY_A
    );
    if ( $linked_row ) {
...
if ( $workspace_groups_table ) {
    $workspace_group_options = $wpdb->get_results(
    ...
    if ( ! empty( $drive_cfg['ok'] ) ) {
        ...
        if ( $drive_user_folders_table && $drive_shared_id !== '' ) {
            ...
            if ( $drive_folder_id !== '' && function_exists( 'metis_portal_url' ) ) {
                $drive_folder_url = add_query_arg(
...
if ( $person && ! empty( $person['id'] ) ) {
    ...
    $effective_permissions = $wpdb->get_results( $wpdb->prepare(
    ...
    $person_activity_rows = $wpdb->get_results( $wpdb->prepare(
    ...
    $person_request_rows = $wpdb->get_results( $wpdb->prepare(
    ...
    $person_document_rows = $wpdb->get_results( $wpdb->prepare(
    ...
    $person_emergency_rows = $wpdb->get_results( $wpdb->prepare(
    ...
    $person_passkey_rows = $wpdb->get_results( $wpdb->prepare(
    ...
    $person_lifecycle_tasks = $wpdb->get_results( $wpdb->prepare(
    ...
    $perms_table = Metis_Tables::get( 'people_permissions' );
    $permissions_catalog = $wpdb->get_results(
    ...
    $workspace_row = $wpdb->get_row( $wpdb->prepare(
...
<div class="metis-people-detail" data-can-manage="<?php echo esc_attr( $can_manage ? '1' : '0' ); ?>">
    <div id="metis-people-alert" class="mw-alert" style="display:none;"></div>

    <div class="metis-people-detail-header">
        <div>
            <h1 class="mw-page-title"><?php echo esc_html( $is_new ? 'Add Person' : ( $full_name !== '' ? $full_name : $display_name ) ); ?></h1>
            <p class="mw-subtitle"><?php echo esc_html( $is_new ? 'Create a new Metis person profile and assign access.' : 'PID: ' . (string) ( $person['pid'] ?? '' ) ); ?></p>
        </div>
        <div class="metis-top-actions">
            <a href="<?php echo esc_url( metis_people_people_list_url() ); ?>" class="mw-btn mw-btn-ghost">Back to People</a>
        </div>
    </div>
    ...
    <div class="metis-people-person-grid">
        ...
        <div class="metis-people-profile-card">
            <div class="metis-people-profile-avatar-wrap">
                <img class="metis-people-profile-avatar" src="<?php echo esc_url( $avatar_src ?: '' ); ?>" alt="Profile photo">
            </div>
            <?php if ( $can_manage && ! $is_new && ! empty( $person['id'] ) ) : ?>
                <button type="button" class="mw-btn-xs" id="metis-people-avatar-edit-open">Change Photo</button>
            <?php endif; ?>
            <h3><?php echo esc_html( $full_name !== '' ? $full_name : ( $display_name !== '' ? $display_name : 'New Person' ) ); ?></h3>

            <div class="metis-people-profile-meta">
                <?php if ( $person_email !== '' ) : ?>
                    <div class="metis-people-profile-meta-row"><?php echo esc_html( $person_email ); ?></div>
                <?php endif; ?>
                <?php if ( ! empty( $person['pid'] ) ) : ?>
                    <div class="metis-people-profile-meta-row">
                        <span class="metis-people-profile-meta-label">PID</span>
                        <span><?php echo esc_html( (string) $person['pid'] ); ?></span>
                    </div>
                <?php endif; ?>
                <?php if ( $linked_donor_id !== '' ) : ?>
                    <div class="metis-people-profile-meta-row">
                        <span class="metis-people-profile-meta-label">Donor</span>
                        <span><?php echo esc_html( $linked_donor_id ); ?></span>
                    </div>
                <?php endif; ?>
                <div class="metis-people-profile-meta-row">
                    <span class="metis-people-profile-meta-label">Auth</span>
                    <span><?php echo esc_html( ucfirst( (string) ( $person['auth_provider'] ?? 'metis' ) ) ); ?></span>
                </div>
            </div>
            ...
            if ( ! empty( $profile_tags ) ) : ?>
                <div class="metis-people-profile-tags">
                    <?php foreach ( $profile_tags as [$tag_label, $tag_class] ) : ?>
                        <span class="metis-people-profile-tag <?php echo esc_attr( $tag_class ); ?>"><?php echo esc_html( $tag_label ); ?></span>
                    <?php endforeach; ?>
                </div>
            <?php endif; ?>
            ...
            <div class="metis-people-profile-actions">
                <?php if ( $linked_donor_id !== '' ) : ?>
                    <a class="mw-btn mw-btn-ghost" style="font-size:13px;padding:6px 10px;" href="<?php echo esc_url( $donor_profile_url ); ?>">Open Donor Profile</a>
                <?php endif; ?>
                <?php if ( $can_manage && ! $is_new && ! empty( $person['pid'] ) ) : ?>
                    <button type="button" class="mw-btn mw-btn-danger" style="font-size:13px;padding:6px 10px;" id="metis-people-offboard-btn" data-pid="<?php echo esc_attr( (string) $person['pid'] ); ?>">Run Offboarding</button>
                <?php endif; ?>
            </div>
        </div>
        ...
        <div class="metis-people-tab-content">
            ...
            <form id="metis-people-detail-form" class="metis-contact-form">
                <input id="metis-people-id" type="hidden" value="<?php echo esc_attr( (string) ( $person['id'] ?? 0 ) ); ?>">
                <input id="metis-people-pid" type="hidden" value="<?php echo esc_attr( (string) ( $person['pid'] ?? '' ) ); ?>">
                ...
                <div class="metis-contact-field metis-contact-field-half">
                    ...
                    <input id="metis-people-first-name" class="mw-input" type="text" value="<?php echo esc_attr( $first_name ); ?>" <?php disabled( ! $can_manage ); ?>>
                </div>

                <div class="metis-contact-field metis-contact-field-half">
                    <label for="metis-people-last-name">Last Name</label>
                    <input id="metis-people-last-name" class="mw-input" type="text" value="<?php echo esc_attr( $last_name ); ?>" <?php disabled( ! $can_manage ); ?>>
                </div>

                <div class="metis-contact-field metis-contact-field-half">
                    <label for="metis-people-name">Display Name</label>
                    <input id="metis-people-name" class="mw-input" type="text" value="<?php echo esc_attr( $display_name ); ?>" <?php disabled( ! $can_manage ); ?> required>
                </div>

                <div class="metis-contact-field metis-contact-field-half">
                    <label for="metis-people-email">Email</label>
                    <input id="metis-people-email" class="mw-input" type="email" value="<?php echo esc_attr( $person_email ); ?>" <?php disabled( ! $can_manage ); ?> required>
                </div>
                ...
                <div class="metis-contact-field metis-contact-field-half">
                    ...
                    <select id="metis-people-provider" class="mw-select" <?php disabled( ! $can_manage ); ?>>
                        <option value="workspace" <?php selected( (string) ( $person['auth_provider'] ?? 'metis' ), 'workspace' ); ?>>Google Workspace SSO</option>
                        <option value="metis" <?php selected( (string) ( $person['auth_provider'] ?? 'metis' ), 'metis' ); ?>>Metis Native</option>
                    </select>
                </div>
                ...
                <div class="metis-contact-field metis-contact-field-half">
                    ...
                    <input id="metis-people-linked-donor-id" class="mw-input" type="text" value="<?php echo esc_attr( $linked_donor_id ); ?>" placeholder="Search donor by name, email, or ID" list="metis-people-donor-list" autocomplete="off" <?php disabled( ! $can_manage ); ?>>
                    <datalist id="metis-people-donor-list"></datalist>
                    <div id="metis-people-linked-donor-name" class="mw-muted"><?php echo esc_html( $linked_donor_name ); ?></div>
                </div>
                ...
                <div class="metis-contact-field metis-contact-field-half">
                    ...
                    <select id="metis-people-status" class="mw-select" <?php disabled( ! $can_manage ); ?>>
                        <option value="active" <?php selected( (string) ( $person['status'] ?? 'active' ), 'active' ); ?>>Active</option>
                        <option value="inactive" <?php selected( (string) ( $person['status'] ?? 'active' ), 'inactive' ); ?>>Inactive</option>
                    </select>
                </div>

                <div class="metis-contact-field metis-contact-field-half">
                    <label for="metis-people-lifecycle-status">Lifecycle Status</label>
                    <select id="metis-people-lifecycle-status" class="mw-select" <?php disabled( ! $can_manage ); ?>>
                        <option value="candidate" <?php selected( $lifecycle_status, 'candidate' ); ?>>Candidate</option>
                        <option value="active" <?php selected( $lifecycle_status, 'active' ); ?>>Active</option>
                        <option value="leave" <?php selected( $lifecycle_status, 'leave' ); ?>>On Leave</option>
                        <option value="alumni" <?php selected( $lifecycle_status, 'alumni' ); ?>>Alumni</option>
                    </select>
                </div>

                <div class="metis-contact-field metis-contact-field-half">
                    <label for="metis-people-manager-pid">Manager PID</label>
                    <input id="metis-people-manager-pid" class="mw-input" type="text" value="<?php echo esc_attr( $manager_pid ); ?>" <?php disabled( ! $can_manage ); ?>>
                </div>

                <div class="metis-contact-field metis-contact-field-half">
                    <label for="metis-people-department">Department</label>
                    <input id="metis-people-department" class="mw-input" type="text" value="<?php echo esc_attr( $department ); ?>" <?php disabled( ! $can_manage ); ?>>
                </div>

                <div class="metis-contact-field metis-contact-field-half">
                    <label for="metis-people-board-term-start">Board Term Start</label>
                    <input id="metis-people-board-term-start" class="mw-input" type="date" value="<?php echo esc_attr( $board_term_start ); ?>" <?php disabled( ! $can_manage ); ?>>
                </div>

                <div class="metis-contact-field metis-contact-field-half">
                    <label for="metis-people-board-term-end">Board Term End</label>
                    <input id="metis-people-board-term-end" class="mw-input" type="date" value="<?php echo esc_attr( $board_term_end ); ?>" <?php disabled( ! $can_manage ); ?>>
                </div>

                <div class="metis-contact-field metis-contact-field-half">
                    <label for="metis-people-volunteer-area">Volunteer Area</label>
                    <input id="metis-people-volunteer-area" class="mw-input" type="text" value="<?php echo esc_attr( $volunteer_area ); ?>" <?php disabled( ! $can_manage ); ?>>
                </div>
                ...
                <div class="metis-contact-field metis-contact-field-full">
                    ...
                    <div class="metis-people-check-grid">
                        <label class="metis-people-check"><input type="checkbox" id="metis-people-staff" <?php checked( ! empty( $person['is_staff'] ) ); ?> <?php disabled( ! $can_manage ); ?>> Staff</label>
                        <label class="metis-people-check"><input type="checkbox" id="metis-people-board" <?php checked( ! empty( $person['is_board'] ) ); ?> <?php disabled( ! $can_manage ); ?>> Board Member</label>
                        <label class="metis-people-check"><input type="checkbox" id="metis-people-volunteer" <?php checked( ! empty( $person['is_volunteer'] ) ); ?> <?php disabled( ! $can_manage ); ?>> Volunteer</label>
                    </div>
                </div>

                <?php if ( $can_manage ) : ?>
                    <div class="metis-contact-actions">
                        <a href="<?php echo esc_url( metis_people_people_list_url() ); ?>" class="mw-btn mw-btn-ghost">Cancel</a>
                        <button type="submit" class="mw-btn">Save Person</button>
                    </div>
                <?php endif; ?>
            </form>
                </div>
                ...
                <div class="metis-tab-panel" data-tab-panel="access">
                    ...
                    <div class="metis-people-access-shell">
                                ...
                                <?php foreach ( $metis_roles as $r ) : ?>
                                    ...
                                    <div class="metis-people-role-row">
                                        <label class="metis-people-check">
                                            <input type="checkbox" class="metis-role-toggle" data-role-key="<?php echo esc_attr( $role_key ); ?>" value="<?php echo esc_attr( $role_key ); ?>" <?php checked( in_array( $role_key, $selected_role_keys, true ) ); ?> <?php disabled( ! $can_manage ); ?>>
                                            <span class="metis-role-label-stack">
                                                <span><?php echo esc_html( (string) ( $r['role_name'] ?? $role_key ) ); ?></span>
                                                <span class="metis-role-subtitle"><?php echo esc_html( ucfirst( (string) ( $r['role_domain'] ?? 'metis' ) ) ); ?> role</span>
                                            </span>
                                        </label>
                                        <select class="mw-select metis-role-window-preset" data-role-key="<?php echo esc_attr( $role_key ); ?>" <?php disabled( ! $can_manage ); ?>>
                                            <option value="">Custom</option>
                                            <option value="always">Always</option>
                                            <option value="30d">30 days</option>
                                            <option value="90d">90 days</option>
                                        </select>
                                        <input type="datetime-local" class="mw-input metis-role-start" data-role-key="<?php echo esc_attr( $role_key ); ?>" value="<?php echo esc_attr( $start_val ); ?>" <?php disabled( ! $can_manage ); ?>>
                                        <input type="datetime-local" class="mw-input metis-role-end" data-role-key="<?php echo esc_attr( $role_key ); ?>" value="<?php echo esc_attr( $end_val ); ?>" <?php disabled( ! $can_manage ); ?>>
                                    </div>
                                <?php endforeach; ?>
                            </div>
                            <div class="mw-muted">Optional schedule window. Empty means always active.</div>
                        </div>
                            ...
                            <div class="metis-people-check-grid">
                                <?php if ( ! empty( $effective_permissions ) ) : ?>
                                    <?php foreach ( $effective_permissions as $perm ) : ?>
                                        <span class="mw-chip"><?php echo esc_html( (string) ( $perm['permission_key'] ?? '' ) ); ?></span>
                                    <?php endforeach; ?>
                                <?php else : ?>
                                    <span class="mw-muted">No active permissions found.</span>
                                <?php endif; ?>
                            </div>
                        </div>

                        <div class="metis-people-access-block">
                            <label>Permission Simulator</label>
                            <div class="metis-contact-form">
                                <div class="metis-contact-field metis-contact-field-half">
                                    <label for="metis-people-sim-module">Module</label>
                                    <select id="metis-people-sim-module" class="mw-select">
                                        ...
                                        ?>
                                            <option value="<?php echo esc_attr( $module_slug ); ?>"><?php echo esc_html( ucfirst( $module_slug ) ); ?></option>
                                        <?php endforeach; ?>
                                    </select>
                                </div>
                                <div class="metis-contact-field metis-contact-field-half">
                                    <label for="metis-people-sim-action">Action</label>
                                    <select id="metis-people-sim-action" class="mw-select">
                                ...
                                <div class="metis-contact-field metis-contact-field-full">
                                    <button type="button" class="mw-btn-xs" id="metis-people-sim-run">Run Simulation</button>
                                    <span id="metis-people-sim-result" class="mw-muted"></span>
                                </div>
                            </div>
                        </div>
                        ...
                        <?php if ( $can_manage ) : ?>
                            <div class="metis-contact-actions">
                                <button type="button" class="mw-btn" id="metis-people-save-access">Save Access</button>
                            </div>
                        <?php endif; ?>
                    </div>
                </div>
                ...
                <div class="metis-tab-panel" data-tab-panel="workspace">
                    ...
                    <div class="metis-contact-form">
                        <input type="hidden" id="metis-people-workspace-email-tab" value="<?php echo esc_attr( (string) ( $person['workspace_email'] ?: $workspace_linked_email ?: '' ) ); ?>">
                        <input type="hidden" id="metis-people-workspace-role-tab" value="<?php echo esc_attr( $current_workspace_role ); ?>">
                        <div class="metis-contact-field metis-contact-field-half">
                            <label><input type="checkbox" id="metis-people-workspace-user-tab" <?php checked( ! empty( $person['is_workspace_user'] ) ); ?> <?php disabled( ! $can_manage ); ?>> Google Workspace User</label>
                        </div>
                        <div class="metis-contact-field metis-contact-field-half">
                            <label><input type="checkbox" id="metis-people-workspace-protected-tab" <?php checked( $workspace_linked_protected ); ?> <?php disabled( ! $can_manage ); ?>> Protected (non-removable)</label>
                        </div>
                        <div class="metis-contact-field metis-contact-field-half">
                            <label for="metis-people-stripe-role-tab">Stripe Role (via Workspace SSO)</label>
                            <select id="metis-people-stripe-role-tab" class="mw-select" <?php disabled( ! $can_manage ); ?>>
                                <?php foreach ( $stripe_roles as $value => $label ) : ?>
                                    <option value="<?php echo esc_attr( (string) $value ); ?>" <?php selected( $current_stripe_role, (string) $value ); ?>><?php echo esc_html( (string) $label ); ?></option>
                                <?php endforeach; ?>
                            </select>
                            <div id="metis-people-stripe-role-help-tab" class="mw-muted"></div>
                        </div>
                        <div class="metis-contact-field metis-contact-field-full">
                            <label>Workspace Account</label>
                            <?php if ( $workspace_linked_user_id > 0 ) : ?>
                                <div class="mw-muted">
                                    Connected to: <?php echo esc_html( $workspace_linked_email ); ?>
                                </div>
                                <div class="metis-chip-wrap">
                                    <span class="mw-chip">Connected</span>
                            ...
                            <?php else : ?>
                                <div class="mw-muted">Not linked yet. Saving will create or link the Workspace user.</div>
                            <?php endif; ?>
                        </div>
                        <div class="metis-contact-field metis-contact-field-full">
                            ...
                            <div class="metis-chip-wrap" id="metis-people-drive-folder-wrap">
                                <?php if ( $drive_folder_id !== '' ) : ?>
                                    <span class="mw-chip" id="metis-people-drive-folder-name"><?php echo esc_html( $drive_folder_name !== '' ? $drive_folder_name : $drive_folder_id ); ?></span>
                                    <?php if ( $drive_folder_url !== '' ) : ?>
                                        <a id="metis-people-drive-folder-open" class="mw-btn-xs" href="<?php echo esc_url( $drive_folder_url ); ?>">Open in Drive</a>
                                    <?php endif; ?>
                                <?php else : ?>
                                    <span class="mw-muted" id="metis-people-drive-folder-name">No user folder attached yet.</span>
                                <?php endif; ?>
                                <?php if ( $can_attach_drive_folder ) : ?>
                                    <button
                                        ...
                                        data-shared-drive-id="<?php echo esc_attr( $drive_shared_id ); ?>"
                                        data-users-root-id="<?php echo esc_attr( $drive_users_root_id ); ?>"
                                        data-users-root-name="<?php echo esc_attr( $drive_users_root_name ); ?>">
                            ...
                            <div class="metis-chip-wrap">
                                <?php if ( ! empty( $workspace_linked_roles ) ) : ?>
                                    <?php foreach ( $workspace_linked_roles as $workspace_role_key ) : ?>
                                        ...
                                        ?>
                                        <span class="mw-chip"><?php echo esc_html( $workspace_role_label ); ?></span>
                                    <?php endforeach; ?>
                                <?php else : ?>
                                    <span class="mw-muted">No synced Workspace roles.</span>
                                <?php endif; ?>
                            </div>
                        </div>
                        <div class="metis-contact-field metis-contact-field-full">
                            <label>Workspace Groups</label>
                            <div class="mw-muted">Click a group chip to add/remove membership.</div>
                            <div class="metis-chip-wrap metis-people-workspace-group-toggle-wrap" id="metis-people-workspace-group-toggle-wrap" data-can-manage="<?php echo esc_attr( $can_manage ? '1' : '0' ); ?>">
                                <?php foreach ( $workspace_group_options as $workspace_group_option ) : ?>
                                    ...
                                    <button
                                        type="button"
                                        class="mw-chip metis-people-workspace-group-toggle<?php echo $is_active_group ? ' is-active' : ''; ?>"
                                        data-group-email="<?php echo esc_attr( $workspace_group_email ); ?>"
                                        <?php disabled( ! $can_manage ); ?>>
                                        <span><?php echo esc_html( $workspace_group_label ); ?></span>
                                    </button>
                                <?php endforeach; ?>
                            </div>
                            <input type="hidden" id="metis-people-workspace-groups-json" value="<?php echo esc_attr( metis_json_encode( array_values( array_map( 'strval', (array) $workspace_linked_groups ) ) ) ); ?>">
                        </div>
                        <?php if ( $can_manage ) : ?>
                            <div class="metis-contact-actions">
                                <button type="button" class="mw-btn" id="metis-people-save-workspace">Save Workspace Settings</button>
                            </div>
                        <?php endif; ?>
                    </div>
                </div>
                ...
                <div class="metis-tab-panel" data-tab-panel="security">
                    ...
                    <div class="metis-contact-form">
                        <div class="metis-contact-field metis-contact-field-half">
                            <label><input type="checkbox" id="metis-people-requires-2fa" <?php checked( $requires_2fa ); ?> <?php disabled( ! $can_manage ); ?>> Require MFA for login</label>
                        </div>
                        <div class="metis-contact-field metis-contact-field-half">
                            <label for="metis-people-mfa-method">MFA Method</label>
                            <select id="metis-people-mfa-method" class="mw-select" <?php disabled( ! $can_manage ); ?>>
                                <option value="none" <?php selected( $mfa_method, 'none' ); ?>>No MFA</option>
                                <option value="totp" <?php selected( $mfa_method, 'totp' ); ?>>Authenticator App</option>
                                <option value="passkey" <?php selected( $mfa_method, 'passkey' ); ?>>Passkey</option>
                                <option value="passkey_or_totp" <?php selected( $mfa_method, 'passkey_or_totp' ); ?>>Passkey or Authenticator App</option>
                                <option value="passkey_and_totp" <?php selected( $mfa_method, 'passkey_and_totp' ); ?>>Passkey and Authenticator App</option>
                            </select>
                        </div>
                        ...
                        <div class="metis-contact-field metis-contact-field-half">
                            ...
                            <div class="metis-people-security-row">
                                <span class="mw-chip <?php echo $totp_enabled ? 'mw-chip-success' : ''; ?>"><?php echo $totp_enabled ? 'Configured' : 'Not configured'; ?></span>
                                <?php if ( $can_manage && ! $is_new && ! empty( $person['id'] ) ) : ?>
                                    <button type="button" class="mw-btn-xs" id="metis-people-totp-setup-open"><?php echo $totp_enabled ? 'Reset App Code' : 'Set Up App Code'; ?></button>
                                <?php endif; ?>
                            </div>
                        </div>
                        ...
                        <div class="metis-contact-field metis-contact-field-half">
                            <label>Passkey</label>
                            <div class="metis-people-security-row">
                                <span class="mw-chip <?php echo $passkey_enabled ? 'mw-chip-success' : ''; ?>"><?php echo $passkey_enabled ? 'Configured' : 'Not configured'; ?></span>
                                <?php if ( $can_manage && ! $is_new && ! empty( $person['id'] ) ) : ?>
                                    <button type="button" class="mw-btn-xs" id="metis-people-passkey-register-open">Register Passkey</button>
                                <?php endif; ?>
                            </div>
                        </div>
                        <div class="metis-contact-field metis-contact-field-full">
                                ...
                                <?php if ( ! empty( $person_passkey_rows ) ) : ?>
                                    <?php foreach ( $person_passkey_rows as $pk ) : ?>
                                        <div class="metis-people-mini-item">
                                            <div><strong><?php echo esc_html( (string) ( $pk['label'] ?? 'Passkey' ) ); ?></strong></div>
                                            <div class="mw-muted">Created: <?php echo esc_html( (string) ( $pk['created_at'] ?? '' ) ); ?><?php echo ! empty( $pk['last_used_at'] ) ? ' | Last used: ' . esc_html( (string) $pk['last_used_at'] ) : ''; ?></div>
                                            <?php if ( $can_manage && empty( $pk['revoked_at'] ) ) : ?>
                                                <div class="metis-people-mini-actions">
                                                    <button type="button" class="mw-btn-xs mw-btn-danger metis-passkey-revoke" data-id="<?php echo esc_attr( (string) ( $pk['id'] ?? '' ) ); ?>">Revoke</button>
                                                </div>
                                            <?php endif; ?>
                                        </div>
                                    <?php endforeach; ?>
                                <?php else : ?>
                                    <div class="mw-muted">No passkeys registered.</div>
                                <?php endif; ?>
                            </div>
                        </div>
                        <?php if ( $can_manage && ! $is_new && ! empty( $person['id'] ) ) : ?>
                            <div class="metis-contact-field metis-contact-field-full">
                                ...
                                <div class="metis-people-security-row">
                                    <button
                                        type="button"
                                        class="mw-btn-xs mw-btn-danger"
                                        id="metis-people-reset-mfa"
                                        data-person-id="<?php echo esc_attr( (string) $person['id'] ); ?>"
                                        data-person-label="<?php echo esc_attr( trim( (string) ( $person['display_name'] ?: $person['email'] ?: $person['pid'] ?: 'this account' ) ) ); ?>">
                                        Reset MFA
                                    </button>
                                    <span class="mw-muted">Clears the authenticator secret and revokes all passkeys so the user can sign in and enroll again.</span>
                                </div>
                            </div>
                        <?php endif; ?>
                        <?php if ( $can_manage && $workspace_linked_user_id > 0 ) : ?>
                            <div class="metis-contact-field metis-contact-field-full">
                                <label>Workspace Security</label>
                                <div class="metis-people-security-row">
                                    <button type="button"
                                        class="mw-btn-xs mw-btn-danger metis-workspace-security-open"
                                        data-user-id="<?php echo esc_attr( (string) $workspace_linked_user_id ); ?>"
                                        data-user-email="<?php echo esc_attr( $workspace_linked_email ); ?>"
                    ...
                    <?php if ( $can_manage ) : ?>
                        <div class="metis-contact-actions">
                            <button type="button" class="mw-btn" id="metis-people-save-security">Save Security</button>
                        </div>
                    <?php endif; ?>
                </div>
                ...
                <div class="metis-tab-panel" data-tab-panel="notifications">
                    ...
                    <div class="metis-contact-form">
                        <div class="metis-contact-field metis-contact-field-half">
                            <label><input type="checkbox" id="metis-people-email-notifications" <?php checked( $email_notifications ); ?> <?php disabled( ! $can_manage ); ?>> Enable Email Notifications</label>
                        </div>
                        <div class="metis-contact-field metis-contact-field-full">
                            <label>Granular Notification Preferences</label>
                            <div class="metis-people-notify-grid">
                                ...
                                <?php foreach ( $notification_events as $event_key => $event_label ) : ?>
                                    ...
                                    ?>
                                    <div class="metis-people-notify-event"><?php echo esc_html( $event_label ); ?></div>
                                    <label class="metis-people-notify-cell"><input type="checkbox" class="metis-people-notify-pref" data-event="<?php echo esc_attr( $event_key ); ?>" data-channel="email" <?php checked( $pref_email ); ?> <?php disabled( ! $can_manage ); ?>></label>
                                    <label class="metis-people-notify-cell"><input type="checkbox" class="metis-people-notify-pref" data-event="<?php echo esc_attr( $event_key ); ?>" data-channel="in_app" <?php checked( $pref_in_app ); ?> <?php disabled( ! $can_manage ); ?>></label>
                                <?php endforeach; ?>
                            </div>
                        </div>
                    </div>
                    <?php if ( $can_manage ) : ?>
                        <div class="metis-contact-actions">
                            <button type="button" class="mw-btn" id="metis-people-save-notifications">Save Notifications</button>
                        </div>
                    <?php endif; ?>
                </div>
                ...
                <div class="metis-tab-panel" data-tab-panel="lifecycle">
                    ...
                    <?php if ( $can_manage && ! $is_new && ! empty( $person['id'] ) ) : ?>
                        <form id="metis-people-task-form" class="metis-contact-form">
                            <div class="metis-contact-field metis-contact-field-half">
                                <label for="metis-people-task-phase">Phase</label>
                                <select id="metis-people-task-phase" class="mw-select">
                            ...
                            <div class="metis-contact-field metis-contact-field-half">
                                ...
                                <input id="metis-people-task-due" class="mw-input" type="datetime-local">
                            </div>
                            <div class="metis-contact-field metis-contact-field-full">
                                <label for="metis-people-task-label">Task</label>
                                <input id="metis-people-task-label" class="mw-input" type="text" placeholder="Complete security orientation">
                            </div>
                            <div class="metis-contact-actions"><button type="submit" class="mw-btn-xs">Add Task</button></div>
                        </form>
                    <?php endif; ?>
                    <div class="metis-people-mini-list" id="metis-people-task-list">
                        <?php if ( ! empty( $person_lifecycle_tasks ) ) : ?>
                            <?php foreach ( $person_lifecycle_tasks as $task_row ) : ?>
                                <div class="metis-people-mini-item">
                                    <div><strong><?php echo esc_html( ucfirst( (string) ( $task_row['phase'] ?? 'onboarding' ) ) ); ?></strong> — <?php echo esc_html( (string) ( $task_row['task_label'] ?? '' ) ); ?></div>
                                    <div class="mw-muted">
                                        <?php echo esc_html( 'Status: ' . (string) ( $task_row['status'] ?? 'pending' ) ); ?>
                                        <?php if ( ! empty( $task_row['due_at'] ) ) : ?>
                                            <?php echo esc_html( ' | Due: ' . (string) $task_row['due_at'] ); ?>
                                        <?php endif; ?>
                                    </div>
                                    <?php if ( $can_manage && (string) ( $task_row['status'] ?? '' ) !== 'completed' ) : ?>
                                        <div class="metis-people-mini-actions">
                                            <button type="button" class="mw-btn-xs metis-task-complete" data-id="<?php echo esc_attr( (string) ( $task_row['id'] ?? '' ) ); ?>">Mark Complete</button>
                                        </div>
                                    <?php endif; ?>
                                </div>
                            <?php endforeach; ?>
                        <?php else : ?>
                            <div class="mw-muted">No lifecycle tasks yet.</div>
                        <?php endif; ?>
                    </div>
                </div>
                    ...
                    <div class="metis-people-mini-list">
                        <?php if ( ! empty( $person_activity_rows ) ) : ?>
                            <?php foreach ( $person_activity_rows as $row ) : ?>
                                <div class="metis-people-mini-item">
                                    <div><strong><?php echo esc_html( (string) ( $row['activity_type'] ?? '' ) ); ?></strong> — <?php echo esc_html( (string) ( $row['summary'] ?? '' ) ); ?></div>
                                    <div class="mw-muted"><?php echo esc_html( (string) ( $row['created_at'] ?? '' ) ); ?></div>
                                </div>
                            <?php endforeach; ?>
                        <?php else : ?>
                            <div class="mw-muted">No activity recorded.</div>
                        <?php endif; ?>
                    </div>
                </div>
                    ...
                    <div class="metis-people-mini-list">
                        <?php if ( ! empty( $person_request_rows ) ) : ?>
                            <?php foreach ( $person_request_rows as $row ) : ?>
                                <div class="metis-people-mini-item">
                                    <div><strong><?php echo esc_html( (string) ( $row['request_code'] ?? '' ) ); ?></strong> — <?php echo esc_html( (string) ( $row['role_name'] ?? '' ) ); ?> (<?php echo esc_html( (string) ( $row['status'] ?? '' ) ); ?>)</div>
                                    <div class="mw-muted"><?php echo esc_html( (string) ( $row['reason'] ?? '' ) ); ?></div>
                                </div>
                            <?php endforeach; ?>
                        <?php else : ?>
                            <div class="mw-muted">No requests recorded.</div>
                        <?php endif; ?>
                    </div>
                </div>
                ...
                <div class="metis-tab-panel" data-tab-panel="documents">
                    ...
                    <?php if ( $can_manage && ! $is_new && ! empty( $person['id'] ) ) : ?>
                        <form id="metis-people-doc-form" class="metis-contact-form">
                            <div class="metis-contact-field metis-contact-field-half"><label for="metis-people-doc-type">Type</label><input id="metis-people-doc-type" class="mw-input" type="text" placeholder="agreement"></div>
                            <div class="metis-contact-field metis-contact-field-half"><label for="metis-people-doc-label">Label</label><input id="metis-people-doc-label" class="mw-input" type="text" placeholder="NDA 2026"></div>
                            <div class="metis-contact-field metis-contact-field-half"><label for="metis-people-doc-remind">Remind At (optional)</label><input id="metis-people-doc-remind" class="mw-input" type="datetime-local"></div>
                            <div class="metis-contact-field metis-contact-field-half"><label for="metis-people-doc-expires">Expires At (optional)</label><input id="metis-people-doc-expires" class="mw-input" type="datetime-local"></div>
                            <div class="metis-contact-field metis-contact-field-full"><label for="metis-people-doc-ref">Storage Ref</label><input id="metis-people-doc-ref" class="mw-input" type="text" placeholder="drive://..."></div>
                            <div class="metis-contact-actions"><button type="submit" class="mw-btn-xs">Add Document</button></div>
                        </form>
                    <?php endif; ?>
                    <div class="metis-people-mini-list">
                        <?php if ( ! empty( $person_document_rows ) ) : ?>
                            <?php foreach ( $person_document_rows as $row ) : ?>
                                <div class="metis-people-mini-item">
                                    <div><strong><?php echo esc_html( (string) ( $row['doc_label'] ?? '' ) ); ?></strong> (<?php echo esc_html( (string) ( $row['doc_type'] ?? '' ) ); ?>)</div>
                                    <div class="mw-muted">
                                        <?php echo esc_html( (string) ( $row['storage_ref'] ?? '' ) ); ?>
                                        <?php if ( ! empty( $row['expires_at'] ) ) : ?>
                                            <?php echo esc_html( ' | Expires: ' . (string) $row['expires_at'] ); ?>
                                        <?php endif; ?>
                                        <?php if ( ! empty( $row['remind_at'] ) ) : ?>
                                            <?php echo esc_html( ' | Remind: ' . (string) $row['remind_at'] ); ?>
                                        <?php endif; ?>
                                        <?php if ( ! empty( $row['lifecycle_status'] ) ) : ?>
                                            <?php echo esc_html( ' | Status: ' . (string) $row['lifecycle_status'] ); ?>
                                        <?php endif; ?>
                                    </div>
                                    <?php if ( $can_manage ) : ?>
                                        <div class="metis-people-mini-actions">
                                            <button type="button" class="mw-btn-xs mw-btn-danger metis-doc-delete" data-id="<?php echo esc_attr( (string) ( $row['id'] ?? '' ) ); ?>">Delete</button>
                                        </div>
                                    <?php endif; ?>
                                </div>
                            <?php endforeach; ?>
                        <?php else : ?>
                            <div class="mw-muted">No documents recorded.</div>
                        <?php endif; ?>
                    </div>
                </div>
                ...
                <div class="metis-tab-panel" data-tab-panel="emergency">
                    ...
                    <?php if ( $can_manage && ! $is_new && ! empty( $person['id'] ) ) : ?>
                        <form id="metis-people-emergency-form" class="metis-contact-form">
                            <div class="metis-contact-field metis-contact-field-half"><label for="metis-people-emergency-role">Role Key</label><input id="metis-people-emergency-role" class="mw-input" type="text" placeholder="administrator"></div>
                            <div class="metis-contact-field metis-contact-field-half"><label for="metis-people-emergency-hours">Hours</label><input id="metis-people-emergency-hours" class="mw-input" type="number" min="1" max="72" value="4"></div>
                            <div class="metis-contact-field metis-contact-field-full"><label for="metis-people-emergency-reason">Reason</label><input id="metis-people-emergency-reason" class="mw-input" type="text"></div>
                            <div class="metis-contact-actions"><button type="submit" class="mw-btn-xs">Grant Emergency Access</button></div>
                        </form>
                    <?php endif; ?>
                    <div class="metis-people-mini-list">
                        <?php if ( ! empty( $person_emergency_rows ) ) : ?>
                            <?php foreach ( $person_emergency_rows as $row ) : ?>
                                <div class="metis-people-mini-item">
                                    <div><strong><?php echo esc_html( (string) ( $row['starts_at'] ?? '' ) ); ?></strong> to <?php echo esc_html( (string) ( $row['ends_at'] ?? '' ) ); ?></div>
                                    <div class="mw-muted"><?php echo esc_html( (string) ( $row['reason'] ?? '' ) ); ?> <?php echo ! empty( $row['revoked_at'] ) ? '(Revoked)' : ''; ?></div>
                                    <?php if ( $can_manage && empty( $row['revoked_at'] ) ) : ?>
                                        <div class="metis-people-mini-actions">
                                            <button type="button" class="mw-btn-xs mw-btn-danger metis-emergency-revoke" data-id="<?php echo esc_attr( (string) ( $row['id'] ?? '' ) ); ?>">Revoke</button>
                                        </div>
                                    <?php endif; ?>
                                </div>
                            <?php endforeach; ?>
                        <?php else : ?>
                            <div class="mw-muted">No emergency access records.</div>
                        <?php endif; ?>
                    </div>
                </div>
...
<div id="metis-people-drive-picker-modal" class="metis-contacts-modal" aria-hidden="true">
    <div class="metis-contacts-modal-inner metis-people-modal-inner metis-people-drive-picker-inner">
        <h3 class="metis-contacts-modal-title">Select User Drive Folder</h3>
        <div class="metis-contact-form">
            <div class="metis-contact-field metis-contact-field-full">
                <div class="mw-muted" id="metis-people-drive-picker-subtitle">Choose a folder from the <?php echo esc_html( $drive_users_root_name ); ?> container.</div>
            </div>
            <div class="metis-contact-field metis-contact-field-full">
                <div class="metis-people-drive-picker-toolbar">
                    <button type="button" id="metis-people-drive-picker-up" class="mw-btn mw-btn-ghost">Up</button>
                    <div id="metis-people-drive-picker-path" class="mw-chip"><?php echo esc_html( $drive_users_root_name ); ?></div>
                </div>
            </div>
            <div class="metis-contact-field metis-contact-field-full">
                <div id="metis-people-drive-picker-status" class="mw-muted"></div>
                <div id="metis-people-drive-picker-list" class="metis-people-drive-picker-list"></div>
            </div>
        </div>
        <div class="metis-contact-actions">
            <button type="button" id="metis-people-drive-picker-cancel" class="mw-btn mw-btn-ghost">Cancel</button>
        </div>
    </div>
</div>
<?php endif; ?>
<?php if ( $can_manage && ! $is_new && ! empty( $person['pid'] ) ) : ?>
<div id="metis-people-offboard-modal" class="metis-contacts-modal" aria-hidden="true">
    <div class="metis-contacts-modal-inner metis-people-modal-inner">
        <h3 class="metis-contacts-modal-title">Confirm Offboarding</h3>
        <p>This will set status to inactive, set lifecycle to alumni, clear workspace/stripe access, and remove role assignments.</p>
        <div class="metis-contact-actions">
            <button type="button" id="metis-people-offboard-cancel" class="mw-btn mw-btn-ghost">Cancel</button>
            <button type="button" id="metis-people-offboard-confirm" class="mw-btn mw-btn-danger" data-pid="<?php echo esc_attr( (string) $person['pid'] ); ?>">Run Offboarding</button>
        </div>
    </div>
</div>
<?php endif; ?>
<?php if ( $can_manage && ! $is_new && ! empty( $person['pid'] ) ) : ?>
<div id="metis-people-avatar-modal" class="metis-contacts-modal" aria-hidden="true">
    <div class="metis-contacts-modal-inner metis-people-modal-inner">
        <h3 class="metis-contacts-modal-title">Update Profile Photo</h3>
        <div class="metis-contact-form">
            <div class="metis-contact-field metis-contact-field-full">
                <label for="metis-people-avatar-file">Upload Image</label>
                <input id="metis-people-avatar-file" class="mw-input" type="file" accept="image/*">
            </div>
            <div class="metis-contact-field metis-contact-field-full">
                <label>Crop Preview</label>
                <canvas id="metis-people-avatar-canvas" class="metis-avatar-canvas" width="512" height="512"></canvas>
            </div>
            <div class="metis-contact-field metis-contact-field-half">
                <label for="metis-people-avatar-zoom">Zoom</label>
                <input id="metis-people-avatar-zoom" type="range" min="0.5" max="3" step="0.01" value="1">
            </div>
            <div class="metis-contact-field metis-contact-field-half">
                <label for="metis-people-avatar-offset-x">Horizontal</label>
                <input id="metis-people-avatar-offset-x" type="range" min="-200" max="200" step="1" value="0">
            </div>
            <div class="metis-contact-field metis-contact-field-half">
                <label for="metis-people-avatar-offset-y">Vertical</label>
                <input id="metis-people-avatar-offset-y" type="range" min="-200" max="200" step="1" value="0">
            </div>
        </div>
        <div class="metis-contact-actions">
            <button type="button" id="metis-people-avatar-cancel" class="mw-btn mw-btn-ghost">Cancel</button>
            <button type="button" id="metis-people-avatar-save" class="mw-btn">Save Photo</button>
        </div>
    </div>
</div>

<div id="metis-people-totp-modal" class="metis-contacts-modal" aria-hidden="true">
    <div class="metis-contacts-modal-inner metis-people-modal-inner">
        <h3 class="metis-contacts-modal-title">Set Up Authenticator App</h3>
        <div class="metis-contact-form">
            <div class="metis-contact-field metis-contact-field-full">
                <label>QR Code</label>
                <img id="metis-people-totp-qr" class="metis-people-totp-qr" alt="Authenticator QR code" style="display:none;">
                <div class="mw-muted">Generate a key, then scan this code in your authenticator app.</div>
            </div>
            <div class="metis-contact-field metis-contact-field-full">
                <label>Secret Key</label>
                <div id="metis-people-totp-secret" class="mw-chip">Generate to start.</div>
                <div class="mw-muted">Add this key to Google Authenticator, 1Password, or another TOTP app.</div>
            </div>
            <div class="metis-contact-field metis-contact-field-full">
                <label for="metis-people-totp-uri">Provisioning URI</label>
                <input id="metis-people-totp-uri" class="mw-input" type="text" readonly>
            </div>
            <div class="metis-contact-field metis-contact-field-half">
                <label for="metis-people-totp-code">6-digit code</label>
                <input id="metis-people-totp-code" class="mw-input" type="text" maxlength="6" inputmode="numeric" pattern="[0-9]{6}">
            </div>
        </div>
        <div class="metis-contact-actions">
            <button type="button" id="metis-people-totp-cancel" class="mw-btn mw-btn-ghost">Cancel</button>
            <button type="button" id="metis-people-totp-generate" class="mw-btn mw-btn-ghost">Generate Key</button>
            <button type="button" id="metis-people-totp-verify" class="mw-btn">Verify and Enable</button>
        </div>
    </div>
</div>
<?php endif; ?>
