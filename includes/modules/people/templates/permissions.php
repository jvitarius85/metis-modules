...
$permissions = $wpdb->get_results("SELECT * FROM {$perms_table} ORDER BY module_slug ASC, action_key ASC", ARRAY_A) ?: [];
$metis_role_count = (int) $wpdb->get_var("SELECT COUNT(*) FROM {$roles_table} WHERE role_domain = 'metis'");
?>

<div class="metis-people-permissions">
    <h1 class="mw-page-title">Permissions</h1>
    ...
    <div class="metis-people-toolbar metis-people-roles-toolbar">
        <div class="metis-contacts-toolbar-right">
            ...
            <a href="<?php echo esc_url(metis_people_roles_list_url()); ?>" class="mw-btn mw-btn-ghost">Roles</a>
            <a href="<?php echo esc_url(metis_people_people_list_url()); ?>" class="mw-btn mw-btn-ghost">People</a>
        </div>
    </div>
    ...
    <section class="mw-premium-table">
        ...
        <?php foreach ($permissions as $perm):
            ...
            $coverage = (int) $wpdb->get_var($wpdb->prepare(
        ...
        ?>
            <div class="mw-premium-row" style="display:grid;grid-template-columns:minmax(160px,1fr) minmax(120px,0.7fr) minmax(220px,1.3fr) minmax(130px,0.8fr);">
                <div class="mw-premium-cell"><?php echo esc_html((string) ($perm['module_slug'] ?? '')); ?></div>
                <div class="mw-premium-cell"><?php echo esc_html((string) ($perm['action_key'] ?? '')); ?></div>
                <div class="mw-premium-cell"><?php echo esc_html((string) ($perm['permission_key'] ?? '')); ?></div>
                <div class="mw-premium-cell"><?php echo esc_html((string) $coverage . ' / ' . (string) $metis_role_count); ?></div>
            </div>
        <?php endforeach; ?>
    </section>
</div>
