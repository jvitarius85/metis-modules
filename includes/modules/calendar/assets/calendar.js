document.addEventListener('DOMContentLoaded', function () {
    const ajax = window.metisCalendarAjax || null;
    ...
    function renderCalendarFilters() {
        ...
        filtersEl.innerHTML = availableCalendars.map(function (row) {
            ...
            return '<button type="button" class="metis-calendar-filter-chip' + (isActive ? ' is-active' : '') + '" data-calendar-id="' + escHtml(calendarId) + '">' +
                escHtml(label) +
            '</button>';
        }).join('');
    ...
    function renderSelectedDay() {
        ...
        selectedEventsEl.innerHTML = items.map(function (it) {
            ...
            return '<article class="metis-calendar-event-card" data-event-id="' + escHtml(it.id || '') + '" style="border-left:4px solid ' + escHtml(color) + ';">' +
    ...
    function buildMonthCell(day) {
        ...
        return '<div class="' + classes.join(' ') + '" data-day="' + escHtml(toDayKey(day)) + '" role="button" tabindex="0" aria-label="View events for ' + escHtml(day.toLocaleDateString()) + '">' +
