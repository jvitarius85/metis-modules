...
$meeting_code = sanitize_text_field((string) ($_GET['meeting'] ?? ''));
if ($meeting_code === '') {
...
$agenda_templates = $wpdb->get_results(
...
$decision_templates = $wpdb->get_results(
...
$prior_meetings = $wpdb->get_results($wpdb->prepare(
...
$packet_candidate_docs = $wpdb->get_results($wpdb->prepare(
...
<div class="metis-board-detail" data-meeting-id="<?php echo esc_attr((string) $meeting_id); ?>" data-can-manage="<?php echo $can_manage ? '1' : '0'; ?>">
    <a class="mw-btn mw-btn-ghost" href="<?php echo esc_url($back_url); ?>">Back to Board</a>

    <h1 class="mw-page-title" style="margin-top:10px;"><?php echo esc_html((string) ($meeting['title'] ?? '')); ?></h1>
    <p class="mw-subtitle"><?php echo esc_html((string) ($meeting['meeting_code'] ?? '')); ?> · <?php echo esc_html((string) ($meeting['committee_name'] ?? 'Board-wide')); ?> · <?php echo esc_html(metis_board_format_datetime((string) ($meeting['meeting_date'] ?? ''))); ?></p>
    ...
    <div class="metis-board-stats" style="grid-template-columns: repeat(5, minmax(150px,1fr));">
        <div class="metis-board-stat"><div class="metis-board-stat-label">Status</div><div class="metis-board-stat-value" style="font-size:24px;"><?php echo esc_html($meeting_status); ?></div></div>
        ...
        <div class="metis-board-stat"><div class="metis-board-stat-label">Decisions</div><div class="metis-board-stat-value" style="font-size:24px;"><?php echo esc_html((string) count($decisions)); ?></div></div>
        <div class="metis-board-stat"><div class="metis-board-stat-label">Attendance</div><div class="metis-board-stat-value" style="font-size:24px;"><?php echo esc_html((string) $present_count); ?></div></div>
        <div class="metis-board-stat"><div class="metis-board-stat-label">Quorum</div><div class="metis-board-stat-value" style="font-size:24px;"><?php echo $has_quorum ? 'Met' : 'Pending'; ?></div></div>
    </div>

    <section class="mw-premium-wrap metis-board-tabs-wrap">
        <div class="metis-board-tabs" role="tablist" aria-label="Meeting detail sections">
            <button type="button" class="mw-btn mw-btn-ghost metis-board-tab metis-board-tab-active" data-tab="overview" role="tab" aria-selected="true">Overview</button>
            <button type="button" class="mw-btn mw-btn-ghost metis-board-tab" data-tab="agenda" role="tab" aria-selected="false">Agenda + Minutes</button>
            <button type="button" class="mw-btn mw-btn-ghost metis-board-tab" data-tab="packet" role="tab" aria-selected="false">Packet Builder</button>
            <button type="button" class="mw-btn mw-btn-ghost metis-board-tab" data-tab="voting" role="tab" aria-selected="false">Decisions + Voting</button>
            <button type="button" class="mw-btn mw-btn-ghost metis-board-tab" data-tab="attendance" role="tab" aria-selected="false">Attendance + Quorum</button>
            <button type="button" class="mw-btn mw-btn-ghost metis-board-tab" data-tab="actions" role="tab" aria-selected="false">Action Items</button>
        </div>
        ...
        <div class="metis-board-tab-panels">
            <section class="metis-board-tab-panel" data-panel="overview">
                <div class="metis-board-grid-2">
                    <section class="mw-premium-wrap">
                        <h3 class="metis-people-section-title" style="margin:0 0 10px;">Meeting Profile</h3>
                        <div class="metis-board-linked-list">
                            <div><strong>Meeting Type:</strong> <?php echo esc_html((string) ($meeting['meeting_type'] ?? 'board')); ?></div>
                            <div><strong>Location:</strong> <?php echo esc_html((string) ($meeting['location'] ?? '—')); ?></div>
                            <div><strong>Status:</strong> <?php echo esc_html($meeting_status); ?></div>
                            <div><strong>Created By:</strong> <?php echo esc_html((string) ($meeting['created_by_name'] ?? '—')); ?></div>
                            <div><strong>Packet Published:</strong> <?php echo esc_html($packet_published_at !== '' ? metis_board_format_datetime($packet_published_at) : 'No'); ?></div>
                        </div>
                    </section>
                    ...
                    <section class="mw-premium-wrap">
                        ...
                        <div class="metis-board-linked-list">
                            <div><strong>Calendar Event ID:</strong> <?php echo esc_html((string) ($meeting['google_calendar_event_id'] ?? '—')); ?></div>
                            <div><strong>Calendar Link:</strong> <?php if (!empty($meeting['google_calendar_html_link'])) : ?><a href="<?php echo esc_url((string) $meeting['google_calendar_html_link']); ?>" target="_blank" rel="noopener">Open event</a><?php else : ?>—<?php endif; ?></div>
                            <div><strong>Drive Folder ID:</strong> <?php echo esc_html((string) ($meeting['google_drive_folder_id'] ?? '—')); ?></div>
                            <div><strong>Drive Folder:</strong> <?php if (!empty($meeting['google_drive_folder_url'])) : ?><a href="<?php echo esc_url((string) $meeting['google_drive_folder_url']); ?>" target="_blank" rel="noopener">Open folder</a><?php else : ?>—<?php endif; ?></div>
                        </div>
                    </section>
                </div>
            </section>
            ...
            <section class="metis-board-tab-panel" data-panel="agenda" hidden>
                <?php if ($can_manage) : ?>
                <form id="metis-board-agenda-minutes-form" class="metis-contact-form">
                    <div class="metis-contact-field metis-contact-field-full">
                        <div class="metis-board-builder-toolbar">
                            <label>Agenda Workflow</label>
                        ...
                        <div class="mw-help">Build agenda from reusable sections. This meeting keeps its own snapshot even if templates change later.</div>
                                    ...
                                    <div>
                                        ...
                                        <select class="mw-select metis-board-agenda-template">
                                            ...
                                            <?php foreach ($agenda_templates as $tmpl) : ?>
                                                <option value="<?php echo esc_attr((string) ($tmpl['template_code'] ?? '')); ?>" data-name="<?php echo esc_attr((string) ($tmpl['name'] ?? '')); ?>" data-items="<?php echo esc_attr((string) ($tmpl['default_items_json'] ?? '[]')); ?>">
                                                    <?php echo esc_html((string) ($tmpl['name'] ?? '')); ?>
                                                </option>
                                            <?php endforeach; ?>
                                        </select>
                                    </div>
                            ...
                            <div class="metis-board-agenda-decision-row">
                                <select class="mw-select metis-board-agenda-decision-template">
                                    ...
                                    <?php foreach ($decision_templates as $dt) : ?>
                                        <option value="<?php echo esc_attr((string) ($dt['template_code'] ?? '')); ?>" data-title="<?php echo esc_attr((string) ($dt['title'] ?? '')); ?>">
                                            <?php echo esc_html((string) ($dt['title'] ?? '')); ?>
                                        </option>
                                    <?php endforeach; ?>
                                </select>
                ...
                <div class="metis-board-grid-2">
                            ...
                            <div class="metis-board-agenda-list">
                                ...
                                ?>
                                    <div class="metis-board-agenda-item">
                                        <div class="metis-board-agenda-title"><?php echo esc_html($section_name); ?></div>
                                        <?php if (!empty($items)) : ?><ul><?php foreach ($items as $item) : ?><li><?php echo esc_html((string) $item); ?></li><?php endforeach; ?></ul><?php endif; ?>
                                        <?php if (!empty($decision_labels)) : ?>
                                            <div class="mw-muted" style="margin-top:6px;"><strong>Decision points:</strong></div>
                                            <ul>
                                                <?php foreach ($decision_labels as $dp_label) : ?><li><?php echo esc_html($dp_label); ?></li><?php endforeach; ?>
                                            </ul>
                                        <?php elseif ($decision_label !== '') : ?><div class="mw-muted" style="margin-top:6px;"><strong>Decision point:</strong> <?php echo esc_html($decision_label); ?></div><?php endif; ?>
                                    </div>
                                <?php endforeach; ?>
                            </div>
                        <?php else : ?>
                    ...
                    <section class="mw-premium-wrap">
                        ...
                        <?php if (!empty($meeting['minutes_html'])) : ?>
                            <div class="metis-board-minutes-body"><?php echo metis_kses_post((string) $meeting['minutes_html']); ?></div>
                        <?php else : ?>
            ...
            <section class="metis-board-tab-panel" data-panel="packet" hidden>
                ...
                <div class="metis-board-packet-actions">
                    <button type="button" id="metis-board-prepare-workspace" class="mw-btn mw-btn-ghost">Prepare Meeting Workspace</button>
                ...
                <form id="metis-board-packet-form" class="metis-contact-form">
                    <div class="metis-contact-field metis-contact-field-full">
                        ...
                        <textarea id="metis-board-packet-notes" class="mw-input" rows="4" placeholder="Optional board packet summary or publication notes."><?php echo esc_textarea($packet_notes); ?></textarea>
                    </div>
                    <div class="metis-contact-field metis-contact-field-half">
                        <label for="metis-board-packet-prev-minutes">Prior Minutes Source</label>
                        <select id="metis-board-packet-prev-minutes" class="mw-select">
                            <option value="0">None</option>
                            <?php foreach ($prior_meetings as $pm) : ?>
                                <option value="<?php echo esc_attr((string) ($pm['id'] ?? '0')); ?>" <?php selected((int) ($pm['id'] ?? 0), $packet_prev_minutes_meeting_id); ?>>
                                    <?php echo esc_html((string) ($pm['meeting_code'] ?? '')) . ' · ' . esc_html((string) ($pm['title'] ?? 'Meeting')) . ' · ' . esc_html(metis_board_format_datetime((string) ($pm['meeting_date'] ?? ''))); ?>
                                </option>
                            <?php endforeach; ?>
                        </select>
                    </div>
                    <div class="metis-contact-field metis-contact-field-half">
                        ...
                        <select id="metis-board-packet-financial-doc" class="mw-select">
                            ...
                            ?>
                                <option value="<?php echo esc_attr((string) ($pd['id'] ?? '0')); ?>" <?php selected((int) ($pd['id'] ?? 0), $packet_financial_doc_id); ?>>
                                    <?php echo esc_html((string) ($pd['title'] ?? 'Document')) . ' (' . esc_html((string) ($pd['doc_type'] ?? 'supporting_doc')) . ')'; ?>
                                </option>
                            <?php endforeach; ?>
                        </select>
                    </div>
                    <div class="metis-contact-field metis-contact-field-full">
                        <label><input type="checkbox" id="metis-board-packet-publish" <?php checked($packet_published_at !== ''); ?>> Mark packet as published</label>
                    </div>
                    <div class="metis-contact-actions">
                ...
                <section class="mw-premium-wrap metis-board-drive-wrap"
                    data-meeting-id="<?php echo esc_attr((string) $meeting_id); ?>"
                    data-folder-id="<?php echo esc_attr((string) ($meeting['google_drive_folder_id'] ?? '')); ?>"
                    ...
                    <div class="metis-board-drive-header">
                        ...
                        <div class="mw-muted">Browse live Drive files, then link packet documents to this meeting.</div>
                    </div>
                    <div class="metis-board-drive-toolbar">
                        ...
                        <?php if ($can_manage) : ?>
                            <button type="button" id="metis-board-drive-new-folder" class="mw-btn">New Folder</button>
                            <button type="button" id="metis-board-drive-upload" class="mw-btn">Upload</button>
                        <?php endif; ?>
                        <input type="file" id="metis-board-drive-upload-input" style="display:none;" />
                    </div>
                    <div class="mw-muted" id="metis-board-drive-path">Folder: <code>loading…</code></div>
                    ...
                    <div id="metis-board-linked-doc-rows">
                        <?php foreach ($documents as $doc) : ?>
                            <div class="mw-premium-row">
                                <div class="mw-premium-cell"><strong><?php echo esc_html((string) ($doc['title'] ?? '')); ?></strong></div>
                                <div class="mw-premium-cell"><?php echo esc_html(function_exists('metis_board_doc_type_label') ? metis_board_doc_type_label((string) ($doc['doc_type'] ?? '')) : (string) ($doc['doc_type'] ?? '')); ?></div>
                                <div class="mw-premium-cell"><?php if (!empty($doc['google_drive_url'])) : ?><a href="<?php echo esc_url((string) $doc['google_drive_url']); ?>" target="_blank" rel="noopener">Open file</a><?php else : ?><span class="mw-muted">—</span><?php endif; ?></div>
                                <div class="mw-premium-cell">
                                    <?php if ($can_manage) : ?>
                                        <button type="button" class="mw-btn-xs mw-btn-danger metis-board-unlink-doc" data-document-id="<?php echo esc_attr((string) ($doc['id'] ?? '0')); ?>">Unlink</button>
                                    <?php else : ?>
            ...
            <section class="metis-board-tab-panel" data-panel="voting" hidden>
                ...
                <section class="mw-premium-table metis-board-table">
                    <div class="mw-premium-row mw-premium-header metis-board-decision-header"><div class="mw-premium-cell">Decision</div><div class="mw-premium-cell">Outcome</div><div class="mw-premium-cell">For</div><div class="mw-premium-cell">Against</div><div class="mw-premium-cell">Abstain</div><?php if ($can_manage) : ?><div class="mw-premium-cell">Save</div><?php endif; ?></div>
                    <?php foreach ($decisions as $decision) : ?>
                        <div class="mw-premium-row metis-board-decision-row" data-decision-id="<?php echo esc_attr((string) ((int) ($decision['id'] ?? 0))); ?>">
                            <div class="mw-premium-cell">
                                <strong><?php echo esc_html((string) ($decision['title'] ?? '')); ?></strong>
                                <?php if (!empty($decision['agenda_section_title']) || !empty($decision['agenda_item_title'])) : ?>
                                    <div class="mw-muted">Section: <?php echo esc_html((string) ($decision['agenda_section_title'] ?? '')); ?><?php if (!empty($decision['agenda_item_title'])) : ?> · Item: <?php echo esc_html((string) ($decision['agenda_item_title'] ?? '')); ?><?php endif; ?></div>
                                <?php endif; ?>
                                <?php if (!empty($decision['decision_text'])) : ?><div class="mw-muted"><?php echo esc_html(metis_trim_words((string) $decision['decision_text'], 22)); ?></div><?php endif; ?>
                            </div>
                            <div class="mw-premium-cell">
                                <?php if ($can_manage) : ?>
                                <select class="mw-select metis-board-decision-outcome" style="max-width:160px;">
                                    <?php foreach (['pending' => 'Pending', 'approved' => 'Approved', 'rejected' => 'Rejected', 'tabled' => 'Tabled'] as $outcome_key => $outcome_label) : ?>
                                    <option value="<?php echo esc_attr($outcome_key); ?>" <?php selected((string) ($decision['outcome'] ?? 'pending'), $outcome_key); ?>><?php echo esc_html($outcome_label); ?></option>
                                    <?php endforeach; ?>
                                </select>
                                <?php else : ?>
                                <span class="mw-chip"><?php echo esc_html((string) ($decision['outcome'] ?? 'pending')); ?></span>
                                <?php endif; ?>
                            </div>
                            <div class="mw-premium-cell"><?php if ($can_manage) : ?><input class="mw-input metis-board-decision-for" type="number" min="0" value="<?php echo esc_attr((string) ((int) ($decision['votes_for'] ?? 0))); ?>" style="max-width:90px;"><?php else : ?><?php echo esc_html((string) ((int) ($decision['votes_for'] ?? 0))); ?><?php endif; ?></div>
                            <div class="mw-premium-cell"><?php if ($can_manage) : ?><input class="mw-input metis-board-decision-against" type="number" min="0" value="<?php echo esc_attr((string) ((int) ($decision['votes_against'] ?? 0))); ?>" style="max-width:90px;"><?php else : ?><?php echo esc_html((string) ((int) ($decision['votes_against'] ?? 0))); ?><?php endif; ?></div>
                            <div class="mw-premium-cell"><?php if ($can_manage) : ?><input class="mw-input metis-board-decision-abstain" type="number" min="0" value="<?php echo esc_attr((string) ((int) ($decision['votes_abstain'] ?? 0))); ?>" style="max-width:90px;"><?php else : ?><?php echo esc_html((string) ((int) ($decision['votes_abstain'] ?? 0))); ?><?php endif; ?></div>
                            <?php if ($can_manage) : ?>
                            <div class="mw-premium-cell"><button type="button" class="mw-btn-xs metis-board-decision-save">Save</button></div>
                            <?php endif; ?>
                        </div>
                    <?php endforeach; ?>
            ...
            <section class="metis-board-tab-panel" data-panel="attendance" hidden>
                <div class="metis-board-grid-2">
                    <section class="mw-premium-wrap">
                        ...
                        <div class="metis-board-linked-list">
                            <div><strong>Eligible Voting Members:</strong> <span id="metis-board-eligible-count"><?php echo esc_html((string) $eligible_count); ?></span></div>
                            <div><strong>Present or Remote:</strong> <span id="metis-board-present-count"><?php echo esc_html((string) $present_count); ?></span></div>
                            <div><strong>Required for Quorum:</strong> <span id="metis-board-required-count"><?php echo esc_html((string) $quorum_required); ?></span></div>
                    ...
                    <section class="mw-premium-wrap">
                        ...
                        <?php if ($can_manage) : ?>
                            <div class="metis-board-linked-list">
                                <div><label><input type="checkbox" id="metis-board-attendance-lock" <?php checked((int) ($meeting['attendance_locked'] ?? 0) === 1); ?>> Lock attendance records</label></div>
                ...
                <section class="mw-premium-table metis-board-table" style="margin-top:14px;">
                    <div class="mw-premium-row mw-premium-header metis-board-attendance-header"><div class="mw-premium-cell">Member</div><div class="mw-premium-cell">Role</div><div class="mw-premium-cell">Status</div><div class="mw-premium-cell">Notes</div><?php if ($can_manage) : ?><div class="mw-premium-cell">Save</div><?php endif; ?></div>
                    ...
                    ?>
                    <div class="mw-premium-row metis-board-attendance-row" data-person-id="<?php echo esc_attr((string) $pid); ?>">
                        <div class="mw-premium-cell"><strong><?php echo esc_html((string) ($person['display_name'] ?? '')); ?></strong><div class="mw-muted"><?php echo esc_html((string) ($person['email'] ?? '')); ?></div></div>
                        <div class="mw-premium-cell"><?php if ($can_manage) : ?><input class="mw-input metis-board-attendance-role" type="text" value="<?php echo esc_attr($att_role); ?>" maxlength="64" style="max-width:180px;"><?php else : ?><?php echo esc_html($att_role !== '' ? $att_role : '—'); ?><?php endif; ?></div>
                        <div class="mw-premium-cell">
                            <?php if ($can_manage) : ?>
                            <select class="mw-select metis-board-attendance-status" style="max-width:160px;">
                                ...
                                <option value="<?php echo esc_attr($status_key); ?>" <?php selected($att_status, $status_key); ?>><?php echo esc_html($status_label); ?></option>
                                <?php endforeach; ?>
                            </select>
                            <?php else : ?>
                            <span class="mw-chip"><?php echo esc_html($att_status); ?></span>
                            <?php endif; ?>
                        </div>
                        <div class="mw-premium-cell"><?php if ($can_manage) : ?><input class="mw-input metis-board-attendance-notes" type="text" value="<?php echo esc_attr($att_notes); ?>" maxlength="255"><?php else : ?><?php echo esc_html($att_notes !== '' ? $att_notes : '—'); ?><?php endif; ?></div>
                        <?php if ($can_manage) : ?>
                        <div class="mw-premium-cell"><button type="button" class="mw-btn-xs metis-board-attendance-save">Save</button></div>
                        <?php endif; ?>
                    </div>
                    <?php endforeach; ?>
            ...
            <section class="metis-board-tab-panel" data-panel="actions" hidden>
                ...
                <section class="mw-premium-table metis-board-table">
                    <div class="mw-premium-row mw-premium-header"><div class="mw-premium-cell">Action Item</div><div class="mw-premium-cell">Owner</div><div class="mw-premium-cell">Due</div><div class="mw-premium-cell">Status</div></div>
                    <?php foreach ($actions as $action) : ?>
                        <div class="mw-premium-row">
                            <div class="mw-premium-cell"><strong><?php echo esc_html((string) ($action['title'] ?? '')); ?></strong></div>
        ...
        <form id="metis-board-decision-form" class="metis-contact-form">
            <div class="metis-contact-field metis-contact-field-half">
                ...
                <select id="metis-board-decision-meeting" class="mw-select" required>
                    <option value="<?php echo esc_attr((string) $meeting_id); ?>"><?php echo esc_html((string) ($meeting['meeting_code'] ?? '') . ' - ' . (string) ($meeting['title'] ?? '')); ?></option>
                </select>
            </div>
...
<div id="metis-board-action-modal" class="metis-contacts-modal" aria-hidden="true">
    <div class="metis-contacts-modal-inner">
        ...
        <form id="metis-board-action-form" class="metis-contact-form">
            <div class="metis-contact-field metis-contact-field-half"><label for="metis-board-action-meeting">Meeting</label><select id="metis-board-action-meeting" class="mw-select"><option value="<?php echo esc_attr((string) $meeting_id); ?>"><?php echo esc_html((string) ($meeting['meeting_code'] ?? '') . ' - ' . (string) ($meeting['title'] ?? '')); ?></option></select></div>
...
<div class="metis-contacts-modal" id="metis-board-doc-meta-modal" aria-hidden="true">
    <div class="metis-contacts-modal-inner" style="max-width:640px;">
        ...
        <form class="metis-contact-form" id="metis-board-doc-meta-form">
            ...
            <div class="metis-contact-actions">
                <button type="button" class="mw-btn mw-btn-ghost metis-board-cancel">Cancel</button>
                <button type="submit" class="mw-btn">Save Link</button>
            </div>
        </form>
    </div>
</div>
