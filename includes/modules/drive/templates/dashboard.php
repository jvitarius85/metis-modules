...
if (!metis_drive_can_view()) {
    echo '<div class="mw-alert mw-alert-error">You do not have permission to view Drive.</div>';
    return;
}
$cfg = metis_drive_workspace_settings();
$can_manage = metis_drive_can_manage();
$initial_folder_id = isset($_GET['folder_id']) ? sanitize_text_field(metis_unslash($_GET['folder_id'])) : '';
...
<div class="metis-drive"
     data-shared-drive-id="<?php echo esc_attr((string) ($cfg['shared_drive_id'] ?? '')); ?>"
     data-drive-configs="<?php echo esc_attr(metis_json_encode($drive_configs)); ?>"
     data-users-home-drive-id="<?php echo esc_attr($users_home_drive_id); ?>"
     data-initial-user-folder-id="<?php echo esc_attr($initial_user_folder_id); ?>"
     data-initial-user-folder-name="<?php echo esc_attr($initial_user_folder_name); ?>"
     data-initial-folder-payload="<?php echo esc_attr(metis_json_encode($initial_folder_payload)); ?>"
     data-initial-folder-id="<?php echo esc_attr($initial_folder_id); ?>"
     data-can-manage="<?php echo $can_manage ? '1' : '0'; ?>">

    <div id="metis-drive-alert" class="mw-alert" style="display:none;"></div>

    <?php if (empty($cfg['ok'])) : ?>
        <div class="mw-alert mw-alert-error"><?php echo esc_html((string) ($cfg['error'] ?? 'Drive is not configured.')); ?></div>
    <?php else : ?>
