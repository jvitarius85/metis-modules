...
function metis_drive_ajax_verify(bool $manage = false): void {
    check_ajax_referer('metis_drive', 'nonce');
    if (!metis_drive_can_view()) metis_send_json_error('Unauthorized', 403);
    if ($manage && !metis_drive_can_manage()) metis_send_json_error('Unauthorized', 403);
}

function metis_drive_ajax_cfg_from_request(): array {
    $requested_drive_id = sanitize_text_field(metis_unslash($_POST['drive_id'] ?? ''));
...
function metis_drive_cache_version(): int {
    return max(1, (int) get_option('metis_drive_cache_version', 1));
}

function metis_drive_bump_cache_version(): void {
    update_option('metis_drive_cache_version', metis_drive_cache_version() + 1, false);
}
...
function metis_drive_guard_in_shared_drive_cached(string $file_id, array $cfg): array {
    ...
    if (empty($cached['ok'])) {
        ...
        if (!empty($own_folder['ok']) && (string) ($own_folder['folder_id'] ?? '') === $file_id) {
            ...
            return ['ok' => false, 'error' => (string) ($cached['error'] ?? 'Item is not cached.')];
        }
    }
...
function metis_drive_ajax_guard_target_folder(array $cfg, string $folder_id): array {
    ...
    if (empty($guard['ok'])) {
        return ['ok' => false, 'error' => (string) ($guard['error'] ?? 'Invalid target folder.')];
    }
...
metis_add_action('wp_ajax_metis_drive_list', function () {
    ...
    if ($folder_id !== $shared_drive_id) {
        ...
        if (empty($guard['ok'])) metis_send_json_error((string) ($guard['error'] ?? 'Invalid folder.'), 400);
        if ((string) (($guard['file']['mimeType'] ?? '')) !== 'application/vnd.google-apps.folder') {
            metis_send_json_error('Selected item is not a folder.', 400);
        }
        if ($users_root_id !== '' && $folder_id === $users_root_id) {
            metis_send_json_error('You cannot open the Users container directly.', 403);
        }
        if (!$can_manage && $users_root_id !== '') {
            ...
            if ($inside_users_root) {
                if ($own_folder_id === '') {
                    metis_send_json_error('Your personal folder is not attached yet.', 403);
                }
                $is_own_or_child = ($folder_id === $own_folder_id) || metis_drive_folder_is_descendant_of_cached($cfg, $folder_id, $own_folder_id);
                if (!$is_own_or_child) {
                    metis_send_json_error('You cannot access another user folder.', 403);
                }
            }
        }
    }
    ...
    if (is_array($cached) && !empty($cached['folder_id'])) {
        metis_send_json_success($cached);
    }
    ...
    set_transient($cache_key, $payload, 20);
    metis_send_json_success($payload);
});

metis_add_action('wp_ajax_metis_drive_my_folder', function () {
    metis_drive_ajax_verify(false);
    metis_drive_ensure_schema();
    ...
    if (empty($folder['ok'])) {
        metis_send_json_error((string) ($folder['error'] ?? 'Failed to find or create your folder.'), 500);
    }
...
metis_add_action('wp_ajax_metis_drive_tree_children', function () {
    metis_drive_ajax_verify(false);
    metis_drive_ensure_schema();
    $cfg = metis_drive_ajax_cfg_from_request();
    if (empty($cfg['ok'])) metis_send_json_error((string) ($cfg['error'] ?? 'Workspace config missing.'), 400);
...
metis_add_action('wp_ajax_metis_drive_sync_worker', function () {
    metis_drive_ajax_verify(false);
    metis_drive_ensure_schema();
    $cfg = metis_drive_ajax_cfg_from_request();
    if (empty($cfg['ok'])) metis_send_json_error((string) ($cfg['error'] ?? 'Workspace config missing.'), 400);
    ...
    if (!$force && !metis_drive_sync_needs_refresh((string) ($cfg['shared_drive_id'] ?? ''), $folder_id, metis_drive_background_sync_interval())) {
        metis_send_json_success(['status' => 'fresh']);
    }

    $result = metis_drive_sync_worker($cfg, $folder_id, $depth, $force);
    if (empty($result['ok'])) {
        metis_send_json_error((string) ($result['error'] ?? 'Drive sync failed.'), 500);
    }

    metis_send_json_success($result);
});

metis_add_action('wp_ajax_metis_drive_create_folder', function () {
    metis_drive_ajax_verify(true);
    metis_drive_ensure_schema();
    $cfg = metis_drive_ajax_cfg_from_request();
    if (empty($cfg['ok'])) metis_send_json_error((string) ($cfg['error'] ?? 'Workspace config missing.'), 400);

    $parent_id = sanitize_text_field(metis_unslash($_POST['parent_id'] ?? ''));
    ...
    if ($parent_id !== (string) $cfg['shared_drive_id']) {
        ...
        if (empty($guard['ok'])) metis_send_json_error((string) ($guard['error'] ?? 'Invalid parent folder.'), 400);
    }
    ...
    if (empty($resp['ok'])) metis_send_json_error((string) ($resp['error'] ?? 'Failed to create folder.'), 500);
    ...
    metis_send_json_success(['file' => (array) ($resp['body'] ?? [])]);
});

metis_add_action('wp_ajax_metis_drive_upload_file', function () {
    metis_drive_ajax_verify(true);
    metis_drive_ensure_schema();
    $cfg = metis_drive_ajax_cfg_from_request();
    if (empty($cfg['ok'])) metis_send_json_error((string) ($cfg['error'] ?? 'Workspace config missing.'), 400);
    ...
    if (empty($_FILES['file']) || !is_array($_FILES['file'])) metis_send_json_error('File is required.', 422);
    $file = $_FILES['file'];
    $tmp = (string) ($file['tmp_name'] ?? '');
    $name = sanitize_file_name((string) ($file['name'] ?? ''));
    if ($tmp === '' || $name === '' || !file_exists($tmp)) metis_send_json_error('Invalid upload payload.', 422);
    $bytes = file_get_contents($tmp);
    if ($bytes === false) metis_send_json_error('Could not read uploaded file.', 500);
    ...
    $boundary = 'metis_drive_' . metis_generate_password(12, false, false);
    $body = "--{$boundary}\r\n";
    ...
    $upload_url = add_query_arg(['useDomainAdminAccess' => 'true'], $upload_url);
    $upload = metis_remote_post($upload_url, [
        'timeout' => 60,
        'headers' => [
    ...
    if (metis_is_error($upload)) metis_send_json_error($upload->get_error_message(), 500);
    $code = (int) metis_remote_retrieve_response_code($upload);
    $raw = (string) metis_remote_retrieve_body($upload);
    ...
    metis_drive_bump_cache_version();
    metis_drive_sync_folder_listing($cfg, $parent_id, 0, true);
    metis_send_json_success(['file' => $decoded]);
});

metis_add_action('wp_ajax_metis_drive_create_google_file', function () {
    metis_drive_ajax_verify(true);
    metis_drive_ensure_schema();
    $cfg = metis_drive_ajax_cfg_from_request();
    if (empty($cfg['ok'])) metis_send_json_error((string) ($cfg['error'] ?? 'Workspace config missing.'), 400);
    ...
    $type = sanitize_key(metis_unslash($_POST['google_type'] ?? ''));
    $type_map = [
    ...
    if ($mime_type === '') {
        metis_send_json_error('Invalid Google file type.', 422);
    }
    if ($name === '') {
    ...
    if (empty($resp['ok'])) {
        metis_send_json_error((string) ($resp['error'] ?? 'Failed to create Google file.'), 500);
    }
...
metis_add_action('wp_ajax_metis_drive_move_item', function () {
    metis_drive_ajax_verify(true);
    metis_drive_ensure_schema();
    $cfg = metis_drive_ajax_cfg_from_request();
    if (empty($cfg['ok'])) metis_send_json_error((string) ($cfg['error'] ?? 'Workspace config missing.'), 400);
    ...
    $target_parent_id = sanitize_text_field(metis_unslash($_POST['target_parent_id'] ?? ''));
    if ($file_id === '') metis_send_json_error('File is required.', 422);
    if ($target_parent_id === '') $target_parent_id = (string) ($cfg['shared_drive_id'] ?? '');

    $guard = metis_drive_guard_in_shared_drive($file_id, $cfg);
    if (empty($guard['ok'])) metis_send_json_error((string) ($guard['error'] ?? 'Invalid file.'), 400);
    $file = (array) ($guard['file'] ?? []);

    $target_guard = metis_drive_ajax_guard_target_folder($cfg, $target_parent_id);
    if (empty($target_guard['ok'])) metis_send_json_error((string) ($target_guard['error'] ?? 'Invalid target folder.'), 400);
    ...
    if ((string) ($file['mimeType'] ?? '') === 'application/vnd.google-apps.folder') {
        if ($file_id === $target_parent_id) {
            metis_send_json_error('A folder cannot be moved into itself.', 422);
        }
        if (metis_drive_folder_is_descendant_of_cached($cfg, $target_parent_id, $file_id)) {
            metis_send_json_error('A folder cannot be moved into one of its descendants.', 422);
        }
    }
    ...
    if (empty($resp['ok'])) metis_send_json_error((string) ($resp['error'] ?? 'Failed to move item.'), 500);
...
metis_add_action('wp_ajax_metis_drive_copy_item', function () {
    metis_drive_ajax_verify(true);
    metis_drive_ensure_schema();
    $cfg = metis_drive_ajax_cfg_from_request();
    if (empty($cfg['ok'])) metis_send_json_error((string) ($cfg['error'] ?? 'Workspace config missing.'), 400);
    ...
    $file = (array) ($guard['file'] ?? []);
    if ((string) ($file['mimeType'] ?? '') === 'application/vnd.google-apps.folder') {
        metis_send_json_error('Folders cannot be copied yet.', 422);
    }
    ...
    $resp = metis_drive_google_request('POST', $url, metis_json_encode($payload), $cfg);
    if (empty($resp['ok'])) metis_send_json_error((string) ($resp['error'] ?? 'Failed to copy item.'), 500);
...
metis_add_action('wp_ajax_metis_drive_rename', function () {
    metis_drive_ajax_verify(true);
    metis_drive_ensure_schema();
    $cfg = metis_drive_ajax_cfg_from_request();
    if (empty($cfg['ok'])) metis_send_json_error((string) ($cfg['error'] ?? 'Workspace config missing.'), 400);
    ...
    $file_id = sanitize_text_field(metis_unslash($_POST['file_id'] ?? ''));
    $name = sanitize_text_field(metis_unslash($_POST['name'] ?? ''));
    if ($file_id === '' || $name === '') metis_send_json_error('File and new name are required.', 422);
    $guard = metis_drive_guard_in_shared_drive($file_id, $cfg);
    if (empty($guard['ok'])) metis_send_json_error((string) ($guard['error'] ?? 'Invalid file.'), 400);
    ...
    if (empty($resp['ok'])) metis_send_json_error((string) ($resp['error'] ?? 'Failed to rename file.'), 500);
...
metis_add_action('wp_ajax_metis_drive_trash', function () {
    metis_drive_ajax_verify(true);
    metis_drive_ensure_schema();
    $cfg = metis_drive_ajax_cfg_from_request();
    if (empty($cfg['ok'])) metis_send_json_error((string) ($cfg['error'] ?? 'Workspace config missing.'), 400);
    ...
    if (empty($resp['ok'])) metis_send_json_error((string) ($resp['error'] ?? 'Failed to move file to trash.'), 500);
    ...
    metis_drive_bump_cache_version();
    metis_drive_sync_folder_listing($cfg, (string) (($guard['file']['parents'][0] ?? '') ?: (string) $cfg['shared_drive_id']), 0, true);
    metis_send_json_success(['file_id' => $file_id]);
});
