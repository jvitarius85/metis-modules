...
if ($selected_list_id > 0) {
    $list_subscribers = $wpdb->get_results($wpdb->prepare(
...
<div class="metis-newsletter" data-can-manage="<?php echo esc_attr($can_manage ? '1' : '0'); ?>">
    <h1 class="mw-page-title">Newsletter Lists</h1>
    ...
    <aside class="mw-list-sidebar">
        <div class="mw-list-sidebar-section">
            ...
            <nav class="mw-list-sidebar-nav">
                ...
                <a class="mw-list-sidebar-nav-item is-active" href="<?php echo esc_url($lists_url); ?>">Lists</a>
    ...
    <section class="mw-premium-table metis-newsletter-table" id="metis-newsletter-lists-panel">
        ...
        <div id="metis-newsletter-list-rows">
            <?php foreach ($lists as $list) :
                ...
                $href = add_query_arg(['list_id' => $list_id], $lists_url);
            ?>
                <div class="mw-premium-row metis-newsletter-row" data-search="<?php echo esc_attr($search_blob); ?>" data-row-href="<?php echo esc_url($href); ?>">
                    <div class="mw-premium-cell">
                        <div><strong><?php echo esc_html((string) ($list['name'] ?? '')); ?></strong></div>
                        <div class="mw-muted"><?php echo esc_html((string) ($list['description'] ?? '')); ?></div>
                    </div>
                    <div class="mw-premium-cell"><?php echo esc_html((string) ((int) ($list['subscribed_count'] ?? 0))); ?></div>
                    <div class="mw-premium-cell"><?php echo esc_html((string) ((int) ($list['blocked_count'] ?? 0))); ?></div>
                    ...
                    <div class="mw-premium-cell"><?php echo esc_html(metis_newsletter_format_datetime((string) ($list['updated_at'] ?? ''))); ?></div>
                </div>
            <?php endforeach; ?>
    ...
    <?php if ($selected_list) : ?>
        <section class="mw-premium-wrap">
            <h3 class="metis-people-section-title" style="margin:0 0 12px;"><?php echo esc_html((string) ($selected_list['name'] ?? '')); ?>: Subscribers</h3>
            <div class="metis-contact-form">
                <div class="metis-contact-field metis-contact-field-third">
                    <label>List Name</label>
                    <input id="metis-newsletter-list-name" class="mw-input" type="text" maxlength="191" value="<?php echo esc_attr((string) ($selected_list['name'] ?? '')); ?>">
                </div>
                <div class="metis-contact-field metis-contact-field-third">
                    <label>Description</label>
                    <input id="metis-newsletter-list-description" class="mw-input" type="text" maxlength="255" value="<?php echo esc_attr((string) ($selected_list['description'] ?? '')); ?>">
                </div>
                <div class="metis-contact-field metis-contact-field-third">
                    <label>Status</label>
                    <select id="metis-newsletter-list-active" class="mw-select">
                        <option value="1" <?php selected((int) ($selected_list['is_active'] ?? 0), 1); ?>>Active</option>
                        <option value="0" <?php selected((int) ($selected_list['is_active'] ?? 0), 0); ?>>Inactive</option>
                    </select>
                </div>
            </div>
            <?php if ($can_manage) : ?>
                <div class="metis-contact-actions" style="margin-top:10px;">
                    <button type="button" class="mw-btn" id="metis-newsletter-save-selected-list" data-list-id="<?php echo esc_attr((string) $selected_list_id); ?>">Save List</button>
                </div>
            <?php endif; ?>
                ...
                <div id="metis-newsletter-selected-list-subs-rows">
                    ...
                    ?>
                        <div class="mw-premium-row metis-newsletter-row">
                            <div class="mw-premium-cell"><?php echo esc_html($name !== '' ? $name : '—'); ?></div>
                            <div class="mw-premium-cell"><?php echo esc_html($email); ?></div>
                            <div class="mw-premium-cell">
                                <?php if ($href !== '') : ?><a href="<?php echo esc_url($href); ?>"><?php echo esc_html($cid); ?></a><?php else : ?>—<?php endif; ?>
                            </div>
                            <div class="mw-premium-cell"><?php echo esc_html(metis_newsletter_format_datetime((string) ($sub_row['updated_at'] ?? ''))); ?></div>
                        </div>
                    <?php endforeach; ?>
