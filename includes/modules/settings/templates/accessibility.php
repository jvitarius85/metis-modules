...
<form method="post" class="mw-settings-form" data-metis-settings-form="1">
    <?php metis_nonce_field( 'metis_save_settings_accessibility', 'metis_settings_nonce' ); ?>
        ...
        <div class="mw-settings-body">
            <div class="mw-field">
                <label class="metis-settings-check">
                    <input type="checkbox" name="accessibility_toolbar_enabled" value="1" <?php checked( $accessibility_toolbar_enabled ); ?>>
            ...
            <div class="mw-field">
                <label class="metis-settings-check">
                    <input type="checkbox" name="accessibility_allow_overrides" value="1" <?php checked( $accessibility_allow_overrides ); ?>>
            ...
            <div class="mw-field">
                ...
                <select id="accessibility_default_profile" name="accessibility_default_profile" class="mw-input mw-input-wide">
                    <?php foreach ( $accessibility_profiles as $slug => $profile ) : ?>
                        <option value="<?php echo esc_attr( (string) $slug ); ?>" <?php selected( $accessibility_default_profile, (string) $slug ); ?>>
                            <?php echo esc_html( (string) ( $profile['label'] ?? ucfirst( str_replace( '-', ' ', (string) $slug ) ) ) ); ?>
                        </option>
                    <?php endforeach; ?>
                </select>
