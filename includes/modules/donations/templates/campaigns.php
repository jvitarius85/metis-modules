...
<h1 class="mw-page-title">Campaigns</h1>
    ...
    <?php if ( ! empty( $campaigns ) ) : ?>
        ...
        ?>
            <div class="mw-premium-row mw-campaign-row"
                 data-name="<?php echo esc_attr( strtolower( $c->cname ) ); ?>"
                 data-type="<?php echo esc_attr( strtolower( $c->type ?? '' ) ); ?>"
                 data-active="<?php echo $is_active ? 'active' : 'inactive'; ?>"
                 data-href="<?php echo esc_url( $campaign_url ); ?>">

                <div class="mw-premium-cell mw-campaign-name-cell">
                    <div class="mw-campaign-name"><?php echo esc_html( $c->cname ); ?></div>
                    <div class="mw-muted" style="font-size: 12px;">
                        <?php echo esc_html( $c->cid ); ?>
                        <?php if ( $c->url ) : ?>
                            · <a href="<?php echo esc_url( 'https://mobilizewaco.org' . $c->url ); ?>" target="_blank" class="mw-link-muted">↗</a>
                        <?php endif; ?>
                    </div>
                </div>
                ...
                <div class="mw-premium-cell">
                    <span class="mw-badge <?php echo strtolower( $c->type ?? '' ) === 'ongoing' ? 'blue' : 'muted'; ?>">
                        <?php echo esc_html( ucfirst( $c->type ?? 'Unknown' ) ); ?>
                    </span>
                </div>
                ...
                <div class="mw-premium-cell mw-campaign-progress-cell">
                    ...
                    <?php elseif ( $year_raised > 0 ) : ?>
                        <span class="mw-muted" style="font-size: 13px;">$<?php echo number_format( $year_raised, 0 ); ?> raised · no goal set</span>
                    <?php else : ?>
                        <span class="mw-muted" style="font-size: 13px;">No activity</span>
                    <?php endif; ?>
                </div>
