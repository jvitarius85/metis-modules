...
if (
    ...
    metis_check_referer( 'mw_create_batch', 'mw_batch_nonce' );

    $selected_tids = isset( $_POST['tx'] ) && is_array( $_POST['tx'] )
        ? array_filter( array_map( 'sanitize_text_field', $_POST['tx'] ) )
        : [];
    ...
    if ( empty( $selected_tids ) ) {
        ...
        if ( metis_is_error( $batch_code ) ) {
            $batch_error = $batch_code->get_error_message();
        } else {
...
<h1 class="mw-page-title">Transactions</h1>
<p class="mw-subtitle">Manage individual donations and create deposit batches.</p>

<?php if ( $batch_notice ) : ?>
    <div class="mw-alert mw-alert-success"><?php echo esc_html( $batch_notice ); ?></div>
<?php endif; ?>
<?php if ( $batch_error ) : ?>
    <div class="mw-alert mw-alert-error"><?php echo esc_html( $batch_error ); ?></div>
<?php endif; ?>
...
<form method="post" class="mw-transactions-view">
    <?php metis_nonce_field( 'mw_create_batch', 'mw_batch_nonce' ); ?>
        ...
        <?php if ( ! empty( $transactions ) ) : ?>
            ...
            ?>
                <div class="mw-premium-row mw-tx-row"
                     data-tid="<?php echo esc_attr( $t->tid ); ?>"
                     data-status="<?php echo esc_attr( strtolower( $t->status ) ); ?>"
                     data-deposited="<?php echo esc_attr( $is_deposited ); ?>"
                     data-campaign="<?php echo esc_attr( strtolower( $campaign ) ); ?>"
                     data-method="<?php echo esc_attr( strtolower( $t->payment_method ?? '' ) ); ?>"
                     data-date="<?php echo esc_attr( $iso_date ); ?>"
                     data-amount="<?php echo esc_attr( $amount ); ?>">

                    <div class="mw-tx-col mw-tx-col-select">
                        <?php if ( empty( $t->deposit_batch_id ) ) : ?>
                            <input type="checkbox" class="mw-tx-checkbox" name="tx[]" value="<?php echo esc_attr( $t->tid ); ?>">
                        <?php else : ?>
                    ...
                    <div class="mw-tx-col mw-tx-col-main">
                        <a href="<?php echo esc_url( $tx_url ); ?>" class="mw-tx-main-link">
                            <div class="mw-tx-line1">
                                <span class="mw-tx-date"><?php echo esc_html( $display_date ); ?></span>
                                <span class="mw-tx-sep">·</span>
                                <span class="mw-tx-donor"><?php echo esc_html( $donor_name ?: $t->email ?: 'Unknown donor' ); ?></span>
                            </div>
                            <div class="mw-tx-line2">
                                <span class="mw-tx-campaign"><?php echo esc_html( $campaign ); ?></span>
                            </div>
                        </a>
                    </div>

                    <div class="mw-tx-col mw-tx-col-amount">
                        <span class="mw-tx-amount mw-col-numeric"><?php echo esc_html( $amount_fmt ); ?></span>
                    </div>
