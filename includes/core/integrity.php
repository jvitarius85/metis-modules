...
class Metis_Integrity_Manager {
    ...
    private const STORAGE_DIR          = '.metis-integrity';
    ...
    public static function init(): void {
        ...
        metis_add_action( 'init', [ self::class, 'ensure_runtime' ], 5 );
    ...
    public static function build_baseline( string $reason = 'manual' ): bool {
        ...
        foreach ( $files as $relative => $entry ) {
            ...
            metis_make_dir( dirname( $destination ) );
    ...
    private static function quarantine_file( string $relative ): ?string {
        ...
        $quarantine  = trailingslashit( self::quarantine_dir() ) . $timestamp . '/' . self::normalize_relative_path( $relative );

        metis_make_dir( dirname( $quarantine ) );
    ...
    private static function schedule_scan(): void {
        if ( ! metis_next_scheduled( self::CRON_HOOK ) ) {
            metis_schedule_event( time() + MINUTE_IN_SECONDS, self::CRON_INTERVAL, self::CRON_HOOK );
        }
    }
    ...
    private static function storage_dir(): string {
        return trailingslashit( METIS_PATH ) . self::STORAGE_DIR;
    }

    private static function recovery_dir(): string {
        return trailingslashit( self::storage_dir() ) . self::RECOVERY_DIR;
    }

    private static function quarantine_dir(): string {
        return trailingslashit( self::storage_dir() ) . self::QUARANTINE_DIR;
    }

    private static function manifest_path(): string {
        return trailingslashit( self::storage_dir() ) . self::MANIFEST_FILE;
    }

    private static function manifest_backup_path(): string {
        return trailingslashit( self::storage_dir() ) . self::MANIFEST_BACKUP_FILE;
    }

    private static function signature_path(): string {
        return trailingslashit( self::storage_dir() ) . self::SIGNATURE_FILE;
    }

    private static function signature_backup_path(): string {
        return trailingslashit( self::storage_dir() ) . self::SIGNATURE_BACKUP_FILE;
    }

    private static function recovery_file_path( string $relative ): string {
        return trailingslashit( self::recovery_dir() ) . self::normalize_relative_path( $relative );
    }
    ...
    private static function absolute_path( string $relative ): string {
        return trailingslashit( METIS_PATH ) . ltrim( self::normalize_relative_path( $relative ), '/' );
    }

    private static function relative_from_absolute( string $absolute ): string {
        $base = metis_normalize_path( trailingslashit( METIS_PATH ) );
        $path = metis_normalize_path( $absolute );
    ...
    private static function config(): array {
        ...
        $path = trailingslashit( METIS_PATH ) . 'config/integrity.php';
        if ( ! file_exists( $path ) ) {
            $config = [];
            return $config;
        }
    ...
    private static function public_key_pem(): string {
        ...
        if ( ! str_starts_with( $path, '/' ) ) {
            $path = trailingslashit( METIS_PATH ) . ltrim( $path, '/' );
        }
