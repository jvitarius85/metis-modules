...
function metis_audit_request_id(): string {
    ...
    $header = isset( $_SERVER['HTTP_X_REQUEST_ID'] ) ? sanitize_text_field( (string) $_SERVER['HTTP_X_REQUEST_ID'] ) : '';
    $request_id = $header !== '' ? $header : metis_generate_uuid();
...
function metis_audit_user_agent(): string {
    ...
    return substr( sanitize_text_field( $agent ), 0, 512 );
}
...
function metis_audit_normalize_resource( array $resource ): array {
    ...
    $id    = isset( $resource['id'] ) ? substr( sanitize_text_field( (string) $resource['id'] ), 0, 191 ) : '';
    $label = isset( $resource['label'] ) ? substr( sanitize_text_field( (string) $resource['label'] ), 0, 255 ) : '';
...
function metis_audit_write( string $channel, string $action_type, array $args = [] ): void {
    ...
    $module_slug = sanitize_key( (string) ( $args['module'] ?? '' ) );
    ...
    $payload = [
        'request_id'     => substr( sanitize_text_field( (string) ( $args['request_id'] ?? metis_audit_request_id() ) ), 0, 64 ),
    ...
    if ( $channel === 'security' ) {
        $payload['severity'] = substr( sanitize_key( (string) ( $args['severity'] ?? 'warning' ) ), 0, 24 );
        $payload['outcome']  = substr( sanitize_key( (string) ( $args['outcome'] ?? 'blocked' ) ), 0, 24 );
    }
...
function metis_audit_log_login_success( string $user_login, WP_User $user ): void {
...
metis_add_action( 'metis_login', 'metis_audit_log_login_success', 10, 2 );
...
function metis_audit_log_login_failure( string $username ): void {
    ...
    metis_audit_log_security( 'login_failed', [
        ...
        'resource' => [
            'type'  => 'user',
            'id'    => $user instanceof WP_User ? (string) $user->ID : '',
            'label' => $username,
        ],
        'context'  => [
            'event'    => 'authentication',
...
metis_add_action( 'metis_login_failed', 'metis_audit_log_login_failure' );
...
function metis_audit_log_logout(): void {
    ...
    metis_audit_log_activity( 'logout', [
        ...
        'resource' => [
            'type'  => 'user',
            'id'    => $user instanceof WP_User ? (string) $user->ID : '',
            'label' => $user instanceof WP_User ? (string) $user->user_login : '',
        ],
        'context'  => [
            'event' => 'authentication',
        ],
...
metis_add_action( 'metis_logout', 'metis_audit_log_logout' );

function metis_audit_log_profile_update( int $user_id, WP_User $old_user_data, array $userdata ): void {
    $changed = [];
...
metis_add_action( 'profile_update', 'metis_audit_log_profile_update', 10, 3 );
...
metis_add_action( 'set_user_role', 'metis_audit_log_user_role_change', 10, 3 );
...
function metis_audit_log_media_upload( array $upload ): array {
    ...
    metis_audit_log_activity( 'file_uploaded', [
        ...
        'resource' => [
            'type'  => 'file',
            'id'    => (string) metis_basename( (string) $upload['file'] ),
            'label' => (string) metis_basename( (string) $upload['file'] ),
        ],
        'context'  => [
...
metis_add_filter( 'wp_handle_upload', 'metis_audit_log_media_upload' );
