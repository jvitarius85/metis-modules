...
final class Metis_Security_Enclave {
    ...
    public function handle( string $operation, array $request, callable $callback ): mixed {
        ...
        $this->assert_nonce( $policy, $input, $meta, $operation );
    ...
    private function assert_nonce( Metis_Security_Policy $policy, array $input, array $meta, string $operation ): void {
        ...
        foreach ( [ 'metis_action_nonce', '_wpnonce', 'security', '_ajax_nonce', 'nonce' ] as $field ) {
            if ( isset( $input[ $field ] ) && is_string( $input[ $field ] ) ) {
                $nonce = (string) $input[ $field ];
                break;
            }
        }

        $nonce_key = $policy->nonce_key ?? $operation;
        if ( $nonce === '' || ! $this->nonce_verifier->is_valid( $nonce, $nonce_key, $meta ) ) {
