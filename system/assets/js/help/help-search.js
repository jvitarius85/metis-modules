...
Metis.helpSearch = (function () {
    ...
    function normalizeResultUrl(rawUrl) {
        ...
        if (value.indexOf('/system/enclave/help/admin/help/') !== -1) {
    ...
    function ensure() {
        ...
        modal.className = 'metis-modal-backdrop metis-help-search-modal';
        modal.setAttribute('aria-hidden', 'true');
        modal.innerHTML = '' +
            '<div class="metis-modal metis-help-search-modal__dialog">' +
                '<div class="metis-modal-header">' +
                    '<h2 id="metis-help-search-title">Help Search</h2>' +
                    '<button type="button" class="metis-modal-close" aria-label="Close">&times;</button>' +
                '</div>' +
                '<div class="metis-modal-body">' +
                    ...
                    '<input id="metis-help-search-input" type="search" class="metis-input metis-help-search-modal__input" placeholder="Search help topics, docs, or walkthroughs">' +
    ...
    function render(items) {
        if (!items || !items.length) {
            results.innerHTML = '<div class="metis-muted">No matching help content.</div>';
            return;
        }
    ...
    function search(query) {
        ...
        if (!query) {
            results.innerHTML = '<div class="metis-muted">Search for a workflow, module, or action.</div>';
        ...
        return Metis.request.post(requestConfig(), 'metis_help_search', { query: query }, 'Help search is not configured.')
            ...
            .catch(function (error) {
                results.innerHTML = '<div class="metis-alert metis-alert-error">' + Metis.util.escapeHtml(error.message || 'Search failed.') + '</div>';
            });
    }
