...
$passwordCancellationSynonymSessionCode = $seedRecentEntitySession( 'Show me John Smith' );
$passwordCancellationSynonymStart = $gateway->converse( 'reset his password', $passwordCancellationSynonymSessionCode );
$passwordCancellationSynonymResult = $gateway->converse( 'cancel', $passwordCancellationSynonymSessionCode );
...
$newsletterCancellationSynonymSessionCode = $seedContentEntitySession( 'June Board Update', 'campaign' );
$newsletterCancellationSynonymStart = $gateway->converse( 'schedule that newsletter', $newsletterCancellationSynonymSessionCode );
$newsletterCancellationSynonymResult = $gateway->converse( 'never mind', $newsletterCancellationSynonymSessionCode );
...
$approvalContinuationPeopleSynonymSessionCode = $seedRecentEntitySession( 'Show me John Smith' );
$approvalContinuationPeopleSynonymPrompt = $gateway->converse( 'assign role administrator', $approvalContinuationPeopleSynonymSessionCode );
$approvalContinuationPeopleSynonymResult = $gateway->converse( 'approve', $approvalContinuationPeopleSynonymSessionCode );
...
$approvalRejectionPeopleSynonymSessionCode = $seedRecentEntitySession( 'Show me John Smith' );
$approvalRejectionPeopleSynonymPrompt = $gateway->converse( 'assign role administrator', $approvalRejectionPeopleSynonymSessionCode );
$approvalRejectionPeopleSynonymResult = $gateway->converse( 'cancel', $approvalRejectionPeopleSynonymSessionCode );
...
$approvalContinuationContentSynonymSessionCode = $seedContentEntitySession( 'June Board Update', 'campaign' );
$approvalContinuationContentSynonymPrompt = $gateway->converse( 'cancel that newsletter', $approvalContinuationContentSynonymSessionCode );
$approvalContinuationContentSynonymResult = $gateway->converse( 'go ahead', $approvalContinuationContentSynonymSessionCode );
...
$approvalRejectionContentSynonymSessionCode = $seedContentEntitySession( 'June Board Update', 'campaign' );
$approvalRejectionContentSynonymPrompt = $gateway->converse( 'cancel that newsletter', $approvalRejectionContentSynonymSessionCode );
$approvalRejectionContentSynonymResult = $gateway->converse( 'never mind', $approvalRejectionContentSynonymSessionCode );
...
$assert( (string) ( $passwordCancellationSynonymStart['status'] ?? '' ) === 'workflow_question', 'Password follow-up cancellation synonym should begin with password-scope clarification.' );
$assert( (string) ( $passwordCancellationSynonymResult['status'] ?? '' ) === 'cancelled', 'Cancel should cancel a recent-entity password workflow question.' );
$assert( (string) ( $passwordCancellationSynonymResult['response_type'] ?? '' ) === 'WorkflowCancellation', 'Cancel should return a workflow cancellation response for recent-entity password questions.' );
$assert( (string) ( $passwordCancellationSynonymResult['workflow']['type'] ?? '' ) === 'clarify_password_reset', 'Recent-entity password cancellation synonym should report the pending workflow type.' );
$assert( $memory->recallPendingWorkflow( $passwordCancellationSynonymSessionCode ) === [], 'Recent-entity password workflow cancellation synonyms should clear pending workflow memory.' );
...
$assert( (string) ( $newsletterCancellationSynonymStart['status'] ?? '' ) === 'workflow_question', 'Newsletter schedule follow-up cancellation synonym should begin with schedule-time collection.' );
$assert( (string) ( $newsletterCancellationSynonymResult['status'] ?? '' ) === 'cancelled', 'Never mind should cancel a recent-entity newsletter scheduling workflow question.' );
$assert( (string) ( $newsletterCancellationSynonymResult['response_type'] ?? '' ) === 'WorkflowCancellation', 'Never mind should return a workflow cancellation response for recent-entity newsletter scheduling questions.' );
$assert( (string) ( $newsletterCancellationSynonymResult['workflow']['type'] ?? '' ) === 'newsletter_schedule', 'Recent-entity newsletter scheduling cancellation synonym should report the pending workflow type.' );
$assert( $memory->recallPendingWorkflow( $newsletterCancellationSynonymSessionCode ) === [], 'Recent-entity newsletter scheduling cancellation synonyms should clear pending workflow memory.' );
...
$assert( (string) ( $approvalContinuationPeopleSynonymPrompt['status'] ?? '' ) === 'awaiting_approval', 'Recent-entity people affirmation synonym flow should begin from approval.' );
$assert( (string) ( $approvalContinuationPeopleSynonymResult['response_type'] ?? '' ) === 'WorkflowContinuationResult', 'Approve should continue a recent-entity people approval prompt.' );
$assert( (string) ( $approvalContinuationPeopleSynonymResult['action']['approval_status'] ?? '' ) === 'executed', 'Approve should execute the recent-entity people pending action.' );
$assert( (string) ( $approvalContinuationPeopleSynonymResult['action']['payload']['command_payload']['subject'] ?? '' ) === 'John Smith', 'Approve should preserve the reused subject on recent-entity people approval continuation.' );
$assert( (array) ( $approvalContinuationPeopleSynonymResult['action']['payload']['command_payload']['roles'] ?? [] ) === [ 'administrator' ], 'Approve should preserve the role payload on recent-entity people approval continuation.' );
...
$assert( (string) ( $approvalRejectionPeopleSynonymPrompt['status'] ?? '' ) === 'awaiting_approval', 'Recent-entity people rejection synonym flow should begin from approval.' );
$assert( (string) ( $approvalRejectionPeopleSynonymResult['response_type'] ?? '' ) === 'WorkflowCancellation', 'Cancel should cancel a recent-entity people approval prompt.' );
$assert( (string) ( $approvalRejectionPeopleSynonymResult['status'] ?? '' ) === 'cancelled', 'Cancel should mark a recent-entity people approval prompt as cancelled.' );
$assert( (string) ( $approvalRejectionPeopleSynonymResult['action']['approval_status'] ?? '' ) === 'cancelled', 'Recent-entity people rejection synonym should transition the action to cancelled state.' );
$assert( (string) ( $approvalRejectionPeopleSynonymResult['action']['payload']['command_payload']['subject'] ?? '' ) === 'John Smith', 'Recent-entity people rejection synonym should preserve the reused subject on the cancelled action.' );
...
$assert( (string) ( $approvalContinuationContentSynonymPrompt['status'] ?? '' ) === 'awaiting_approval', 'Recent-entity content affirmation synonym flow should begin from approval.' );
$assert( (string) ( $approvalContinuationContentSynonymResult['response_type'] ?? '' ) === 'WorkflowContinuationResult', 'Go ahead should continue a recent-entity content approval prompt.' );
$assert( (string) ( $approvalContinuationContentSynonymResult['action']['approval_status'] ?? '' ) === 'executed', 'Go ahead should execute the recent-entity content pending action.' );
$assert( (string) ( $approvalContinuationContentSynonymResult['action']['payload']['command_payload']['subject'] ?? '' ) === 'June Board Update', 'Go ahead should preserve the reused subject on recent-entity content approval continuation.' );
$assert( (string) ( $approvalContinuationContentSynonymResult['action']['payload']['operation'] ?? '' ) === 'newsletter_cancel', 'Go ahead should preserve the newsletter_cancel operation on recent-entity content approval continuation.' );
...
$assert( (string) ( $approvalRejectionContentSynonymPrompt['status'] ?? '' ) === 'awaiting_approval', 'Recent-entity content rejection synonym flow should begin from approval.' );
$assert( (string) ( $approvalRejectionContentSynonymResult['response_type'] ?? '' ) === 'WorkflowCancellation', 'Never mind should cancel a recent-entity content approval prompt.' );
$assert( (string) ( $approvalRejectionContentSynonymResult['status'] ?? '' ) === 'cancelled', 'Never mind should mark a recent-entity content approval prompt as cancelled.' );
$assert( (string) ( $approvalRejectionContentSynonymResult['action']['approval_status'] ?? '' ) === 'cancelled', 'Recent-entity content rejection synonym should transition the action to cancelled state.' );
$assert( (string) ( $approvalRejectionContentSynonymResult['action']['payload']['command_payload']['subject'] ?? '' ) === 'June Board Update', 'Recent-entity content rejection synonym should preserve the reused subject on the cancelled action.' );
