...
function metis_finance_ajax_verify_nonce( array $actions = [] ): void {
    $nonceCandidates = [];
    foreach ( [ 'nonce', 'metis_action_nonce' ] as $field ) {
...
function metis_finance_quick_action_gl_entry_form( array $action = [] ): array {
    ...
    $html = '<form class="metis-finance-v2-form metis-quick-action-form" data-quick-action-form="finance_gl_entry">'
        . '<div class="metis-finance-v2-form-grid">'
        . '<div class="metis-field"><label for="qa-gl-entry-date">Date</label><input id="qa-gl-entry-date" name="entry_date" type="date" class="metis-input" value="' . metis_escape_attr( date( 'Y-m-d' ) ) . '" required></div>'
        . '<div class="metis-field"><label for="qa-gl-account-code">Account</label><select id="qa-gl-account-code" name="account_code" class="metis-input" required>' . $accountOptions . '</select></div>'
        . '<div class="metis-field"><label for="qa-gl-amount">Amount</label><input id="qa-gl-amount" name="amount" type="number" min="0.01" step="0.01" class="metis-input" required></div>'
        . '<div class="metis-field"><label for="qa-gl-dc-type">Type</label><select id="qa-gl-dc-type" name="dc_type" class="metis-input" required><option value="debit">Debit</option><option value="credit">Credit</option></select></div>'
        . '<div class="metis-field"><label for="qa-gl-category-code">Category</label><select id="qa-gl-category-code" name="category_code" class="metis-input">' . $categoryOptions . '</select></div>'
        . '<div class="metis-field metis-field-full"><label for="qa-gl-description">Description</label><input id="qa-gl-description" name="description" type="text" class="metis-input" required></div>'
        . '</div>'
        . '</form>';
