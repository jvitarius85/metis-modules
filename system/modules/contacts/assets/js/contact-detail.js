...
window.MetisContactsModules.initDetail = function (context) {
    ...
    if (detailForm) {
        ...
        const pageTitle = document.querySelector('.metis-page-title');
        ...
        function addEmailRow(value) {
            ...
            row.innerHTML = '<input type="email" class="metis-input metis-email-row-input" placeholder="name@example.org">' +
                '<button type="button" class="metis-btn-xs metis-btn-danger">Remove</button>';
        ...
        function renderAdditionalEmailChips(emails) {
            ...
            if (!normalized.length) {
                if (additionalEmailsSection && !openAdditionalEmailPlus) {
                    ...
                    additionalEmailsView.innerHTML = '<div class="metis-muted">No additional emails recorded.</div>';
                }
                return;
            }
        ...
        function renderNewsletterChips(items) {
            ...
            if (!list.length) {
                if (newsletterSection && !openNewsletterPlus) {
                    ...
                    newsletterView.innerHTML = '<div class="metis-muted">No newsletter subscriptions.</div>';
                }
                return;
            }
            ...
            el.addEventListener('dblclick', function () {
                ...
                if (field === 'address_full') {
                    ...
                    editor.innerHTML = '' +
                        ...
                        '<input type="text" class="metis-input metis-inline-address-street" placeholder="Street address">' +
                        '</div>' +
                        '<div class="metis-inline-address-row metis-inline-address-row-local">' +
                        '<input type="text" class="metis-input metis-inline-address-city" placeholder="City">' +
                        '<select class="metis-select metis-inline-address-state" aria-label="State">' +
                        ...
                        '</select>' +
                        '<input type="text" class="metis-input metis-inline-address-zip" maxlength="10" placeholder="ZIP">' +
                        '</div>' +
                        '<div class="metis-inline-address-actions">' +
                        '<button type="button" class="metis-btn-xs metis-inline-address-save">Save</button>' +
                        '<button type="button" class="metis-btn-xs metis-btn-ghost metis-inline-address-cancel">Cancel</button>' +
                        '</div>';
        ...
        function renderRelationships() {
            ...
            if (!relationships.length) {
                relationshipsList.innerHTML = '<div class="metis-muted">No relationships added.</div>';
            ...
            relationships.forEach(function (rel, index) {
                ...
                if (editingRelationshipIndex === index && relationshipContactsTemplate && relationshipTypesTemplate) {
                    ...
                    contactSel.className = 'metis-select metis-inline-rel-contact';
                    ...
                    saveBtn.type = 'button';
                    saveBtn.className = 'metis-btn-xs';
                    ...
                    cancelBtn.className = 'metis-btn-xs metis-btn-ghost';
                    ...
                    row.innerHTML = '<div class="metis-editor-row-label">' + escapeHtml(line) + '</div>' +
                        ...
                        '<button type="button" class="metis-btn-xs metis-rel-edit-btn">Edit</button>' +
                        '<button type="button" class="metis-btn-xs metis-btn-danger metis-rel-delete-btn">Delete</button>' +
                        '</div>';
        ...
        function addRelationshipPickerRow(seed) {
            ...
            const contactSel = document.createElement('select');
            contactSel.className = 'metis-select metis-rel-contact';
            contactSel.innerHTML = relationshipContactsTemplate.innerHTML;

            const typeSel = document.createElement('select');
            typeSel.className = 'metis-select metis-rel-type';
            typeSel.innerHTML = relationshipTypesTemplate.innerHTML;

            const notesInput = document.createElement('input');
            notesInput.className = 'metis-input metis-rel-notes';
            notesInput.type = 'text';
            notesInput.placeholder = 'Notes (optional)';

            const removeBtn = document.createElement('button');
            removeBtn.type = 'button';
            removeBtn.className = 'metis-btn-xs metis-btn-danger metis-rel-row-remove';
