...
if (!metis_newsletter_can_view()) {
    echo '<div class="metis-alert metis-alert-error">You do not have permission to view subscribers.</div>';
    return;
}
...
<div class="metis-newsletter" data-can-manage="<?php echo metis_escape_attr($can_manage ? '1' : '0'); ?>">
    <h1 class="metis-page-title"><?php echo metis_escape_html( metis_current_module_view_title( 'Newsletter Subscribers' ) ); ?></h1>
    <p class="metis-subtitle">Review subscription status by contact and list.</p>
    ...
    <aside class="metis-list-sidebar">
        <div class="metis-list-sidebar-section">
            ...
            <nav class="metis-list-sidebar-nav">
                ...
                <a class="metis-list-sidebar-nav-item is-active" href="<?php echo metis_escape_url($subscribers_url); ?>">Subscribers</a>
            </nav>
        </div>
        <div class="metis-list-sidebar-section">
            <div class="metis-list-sidebar-label">Search</div>
            <input id="metis-newsletter-subscriber-search" class="metis-input" type="text" placeholder="Name, email, list, or CID">
        </div>
        <?php if ( $can_manage ) : ?>
        <div class="metis-list-sidebar-actions">
            <button id="metis-newsletter-batch-btn" type="button" class="metis-btn metis-btn-xs">Batch Add Subscribers</button>
        </div>
        <?php endif; ?>
    </aside>
    ...
    <div id="metis-newsletter-batch-panel" style="display:none; margin-bottom:14px;">
        <div class="mbe-toolbar">
            ...
            <button type="button" id="metis-newsletter-batch-close" class="metis-btn metis-btn-xs metis-btn-ghost">Close</button>
        </div>
        <div id="metis-newsletter-batch-entry"></div>
    </div>
    <?php endif; ?>
    <table class="metis-premium-table metis-newsletter-table" id="metis-newsletter-subscribers-panel">
        <thead>
        ...
        <tbody id="metis-newsletter-subscriber-rows">
            ...
            ?>
                <tr class="metis-premium-row metis-newsletter-row" data-search="<?php echo metis_escape_attr($search_blob); ?>" <?php if ($href !== '') : ?>data-row-href="<?php echo metis_escape_url($href); ?>"<?php endif; ?>>
                    <td class="metis-premium-cell">
                        <strong><?php echo metis_escape_html($name !== '' ? $name : '—'); ?></strong>
                        <?php if ($email !== '') : ?><div class="metis-muted"><?php echo metis_escape_html($email); ?></div><?php endif; ?>
                    </td>
                    <td class="metis-premium-cell">
                        <div class="metis-newsletter-chip-wrap">
                            <?php foreach ($list_names as $list_name) : ?><span class="metis-chip"><?php echo metis_escape_html($list_name); ?></span><?php endforeach; ?>
                            <?php if (empty($list_names)) : ?><span class="metis-muted">—</span><?php endif; ?>
                        </div>
                    </td>
                    <td class="metis-premium-cell"><?php echo metis_escape_html($cid !== '' ? $cid : '—'); ?></td>
                    <td class="metis-premium-cell"><?php echo metis_escape_html(metis_newsletter_format_datetime((string) ($row['updated_at'] ?? ''))); ?></td>
                </tr>
            <?php endforeach; ?>
            <?php if (empty($rows)) : ?><tr class="metis-premium-row"><td class="metis-premium-cell metis-muted" colspan="4">No subscribers found.</td></tr><?php endif; ?>
        </tbody>
    </table>
