...
if ( $editor_new === '' ) {
    if ( preg_match( '#/cms/page(?:s)?/editor/new/?$#i', $request_path ) === 1 ) {
        $editor_new = 'page';
    } elseif ( preg_match( '#/cms/posts/editor/new/?$#i', $request_path ) === 1 ) {
        $editor_new = 'post';
    }
}
...
$is_template_editor = (
    $editor_new === 'template'
    || preg_match( '#/(?:cms/)?editor/template/(?:[A-Za-z0-9_-]+)/?$#i', $request_path ) === 1
    || preg_match( '#/(?:cms/)?editor/new/template/?$#i', $request_path ) === 1
);

if ( ! headers_sent() && function_exists( 'metis_safe_redirect' ) ) {
    if ( $is_template_editor ) {
        $template_target = $portal_base . '/cms/templates/new/';
        if ( $editor_key !== '' ) {
            $template_target = $portal_base . '/cms/templates/edit/' . rawurlencode( $editor_key ) . '/';
        }
...
if ( ! $is_editor_route ) {
    if ( ! headers_sent() && function_exists( 'metis_safe_redirect' ) && function_exists( 'metis_portal_url' ) ) {
        metis_safe_redirect( (string) metis_portal_url( 'cms', 'pages' ) );
        exit;
    }
