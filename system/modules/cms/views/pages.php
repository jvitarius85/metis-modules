...
if ( ! metis_cms_require_view_permission( 'pages' ) ) {
    return;
}
...
if ( $editor_new === '' && preg_match( '#/cms/page(?:s)?/editor/new/?$#i', $request_path ) === 1 ) {
    $editor_new = 'page';
}
if ( $editor_page_id < 1 && $editor_key === '' ) {
    $matches = [];
    if ( preg_match( '#/cms/pages/editor/([A-Za-z0-9_-]+)/?$#i', $request_path, $matches ) === 1 && ! empty( $matches[1] ) ) {
...
if ( $is_editor_route ) {
    ...
    $context = 'cms_page';
    $editor_target_id = 0;
    if ( $editor_new === 'post' || strtoupper( substr( $editor_key, 0, 4 ) ) === 'CMSP' ) {
        $context = 'cms_post';
    }
    if ( $editor_page_id > 0 ) {
...
<div class="metis-page-header">
    <div class="metis-page-header-left">
        ...
        <p class="metis-subtitle"><?php echo metis_escape_html( $total_pages_count ); ?> page<?php echo $total_pages_count !== 1 ? 's' : ''; ?> in CMS content.</p>
    </div>
        ...
        <?php if ( $page_count > 1 ) : ?>
            <div class="metis-pagination" style="margin-top:16px;">
                <?php if ( $current_page > 1 ) : ?>
                    <a class="metis-btn-xs" href="<?php echo metis_escape_url( $page_link( $current_page - 1 ) ); ?>">Prev</a>
                <?php else : ?>
                    <span class="metis-btn-xs" aria-disabled="true">Prev</span>
                <?php endif; ?>
                <span id="metis-pages-page" class="metis-muted">Page <?php echo metis_escape_html( (string) $current_page ); ?> of <?php echo metis_escape_html( (string) $page_count ); ?></span>
                <?php if ( $current_page < $page_count ) : ?>
                    <a class="metis-btn-xs" href="<?php echo metis_escape_url( $page_link( $current_page + 1 ) ); ?>">Next</a>
                <?php else : ?>
