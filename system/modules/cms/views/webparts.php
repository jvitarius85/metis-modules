...
if ( ! metis_cms_require_view_permission( 'webparts' ) ) {
    return;
}
?>
        ...
        <div class="metis-modal metis-config-modal">
            ...
            <div class="metis-modal-body metis-config-modal-body">
                ...
                <div class="metis-field">
                    ...
                    <p class="metis-field-help">Content is stored as a CMS text block and can use approved dynamic tokens.</p>
                </div>
                <div class="metis-field">
...
(function($) {
    ...
    function csrf(action) {
        if (window.MetisCMS && typeof window.MetisCMS._csrfPayload === 'function') {
            return window.MetisCMS._csrfPayload(action);
        }
        var cms = window.metisCmsAjax || {};
        var actionNonces = cms.action_nonces && typeof cms.action_nonces === 'object' ? cms.action_nonces : {};
        var nonce = actionNonces[action] || cms.nonce || '';
        return {
            nonce: nonce,
    ...
    function request(action, payload, onSuccess) {
        ...
        $.ajax({
            ...
            success: function(response) {
                ...
                toast((response && response.data && response.data.message) ? response.data.message : 'CMS request failed.', 'error');
            },
            error: function(xhr) {
                var data = xhr && xhr.responseJSON ? xhr.responseJSON.data : null;
                toast((data && data.message) ? data.message : 'CMS request failed.', 'error');
            }
        });
    }
    ...
    function loadList() {
        request('metis_cms_webparts_list', {}, function(data) {
    ...
    function saveWebPart() {
        ...
        request('metis_cms_webpart_save', payload, function(data) {
            state.webparts = Array.isArray(data.webparts) ? data.webparts : [];
            renderRows();
            closeModal();
    ...
    $(document).on('click.metisCMSWebParts', '#metis-webpart-create-btn, #metis-webpart-create-btn-empty', function(e) {
        e.preventDefault();
        openModal(null);
    });
    $(document).on('click.metisCMSWebPartClose', '#metis-webpart-modal-close, #metis-webpart-cancel-btn', function(e) {
        e.preventDefault();
        closeModal();
    });
    $(document).on('change.metisCMSWebParts', '#metis-webpart-status-filter', renderRows);
    $(document).on('click.metisCMSWebPartEdit', '.metis-webpart-edit', function(e) {
        e.preventDefault();
        e.stopPropagation();
    ...
    $(document).on('click.metisCMSWebPartDelete', '.metis-webpart-delete', function(e) {
        e.preventDefault();
        e.stopPropagation();
        var id = String($(this).attr('data-id') || '');
        var remove = function() {
            request('metis_cms_webpart_delete', { id: id }, function(data) {
                state.webparts = Array.isArray(data.webparts) ? data.webparts : [];
                renderRows();
                toast(data.message || 'Web part deleted.', 'success');
            });
        };
        if (window.MetisCMS && typeof window.MetisCMS._confirm === 'function') {
            window.MetisCMS._confirm('Delete this web part?', remove, { title: 'Delete Web Part', confirmLabel: 'Delete' });
        } else {
            remove();
        }
    });
    $(document).on('click.metisCMSWebPartSave', '#metis-webpart-save-btn', function(e) {
        e.preventDefault();
        saveWebPart();
    });
    $(document).on('click.metisCMSWebPartRow', '.metis-webparts-table tbody tr', function() {
        var id = String($(this).attr('data-id') || '');
        var part = state.webparts.find(function(item) { return String(item.id) === id; });
        if (part) {
            openModal(part);
