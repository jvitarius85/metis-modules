...
<div class="metis-page-header">
    <div class="metis-page-header-left">
        ...
        <p class="metis-subtitle"><?php echo count( $redirects ); ?> redirect<?php echo count( $redirects ) !== 1 ? 's' : ''; ?> configured for cms routing.</p>
    </div>
...
function renderRedirectTable(redirects) {
    ...
    $('.metis-page-header .metis-subtitle').first().text(list.length + ' redirect' + (list.length === 1 ? '' : 's') + ' configured for cms routing.');
    $wrap.empty();
...
function saveRedirect() {
    var payload = {
        action: 'metis_cms_redirect_save',
        nonce: nonceFor('metis_cms_redirect_save'),
...
function deleteRedirect(id) {
    window.metis_confirm('Delete this redirect?', function() {
        $.post(ajaxConfig().ajax_url, {
            action: 'metis_cms_redirect_delete',
            nonce: nonceFor('metis_cms_redirect_delete'),
            id: String(id || '')
        }).done(function(resp) {
            if (resp && resp.success) {
