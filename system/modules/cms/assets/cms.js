...
if (!$) {
    ...
    var fallbackPortalBasePath = function() {
        ...
        var m = p.match(/^(.*?\/)(?:cms|newsletter|settings|people|finance|portal|editor)(?:\/|$)/i);
    ...
    var fallbackAjaxConfig = function() {
        if (window.Metis && Metis.request && typeof Metis.request.config === 'function') {
            try {
                return Metis.request.config(window.metisCmsAjax || null, 'Request endpoint is unavailable.');
            } catch (_error) {
                /* Fall through to the plain object merge below. */
            }
        }
        var cfg = window.metisCmsAjax || window.metisAjax || {};
        return {
    ...
    var fallbackRemoveRow = function(trigger, emptyText) {
        var row = trigger && trigger.closest ? trigger.closest('.metis-premium-row') : null;
        var container = row && row.parentElement ? row.parentElement : null;
        if (row && row.parentNode) {
            row.parentNode.removeChild(row);
        }
        if (container && !container.querySelector('.metis-premium-row')) {
            var isTableBody = String(container.tagName || '').toLowerCase() === 'tbody';
            var placeholder = document.createElement(isTableBody ? 'tr' : 'div');
            var cell = document.createElement(isTableBody ? 'td' : 'div');
            var table = isTableBody && container.closest ? container.closest('table') : null;
            var colCount = table ? table.querySelectorAll('thead th').length : 1;
            placeholder.className = 'metis-premium-row';
            cell.className = 'metis-premium-cell metis-muted';
            if (isTableBody) {
                cell.setAttribute('colspan', String(Math.max(1, colCount)));
            }
            cell.textContent = String(emptyText || 'No records found.');
    ...
    document.addEventListener('click', function(e) {
        ...
        if (createPage) {
            e.preventDefault();
            fallbackGo('cms/pages/editor/new/');
            return;
        }
        ...
        if (editPage) {
            ...
            if (pageRef !== '') {
                fallbackGo('cms/pages/editor/' + encodeURIComponent(pageRef) + '/');
            }
            return;
        }
        ...
        if (createPost) {
            e.preventDefault();
            fallbackGo('cms/posts/editor/new/');
            return;
        }
        ...
        if (editPost) {
            ...
            if (postRef !== '') {
                fallbackGo('cms/posts/editor/' + encodeURIComponent(postRef) + '/');
            }
            return;
        }
        ...
        if (deletePage) {
            ...
            if (!Number.isNaN(deletePageId) && deletePageId > 0) {
                fallbackConfirm('Delete this page? This cannot be undone.', function() {
                    fallbackAjax('metis_cms_page_delete', { id: deletePageId }, function() {
        ...
        if (deletePost) {
            ...
            if (!Number.isNaN(deletePostId) && deletePostId > 0) {
                fallbackConfirm('Delete this post? This cannot be undone.', function() {
                    fallbackAjax('metis_cms_post_delete', { id: deletePostId }, function() {
...
var MetisCMS = {
    ...
    _updateListSubtitle: function(shellSelector, count, singularLabel, pluralLabel, suffix) {
        ...
        shell.find('.metis-page-header .metis-subtitle').first().text(safeCount + ' ' + noun + ' ' + String(suffix || ''));
    },

    _pageRows: function() {
        return $('#metis-pages-list-shell .metis-pages-table .metis-premium-row').not('.metis-premium-header');
    },

    _pageRowForId: function(id) {
        return $('#metis-pages-list-shell .metis-pages-table').find('.metis-edit-page[data-id="' + String(id || '') + '"]').first().closest('.metis-premium-row');
    },
    ...
    _applyHomepagePageId: function(homepageId) {
        ...
        if ($row.length) {
            var $titleCell = $row.children('.metis-premium-cell').eq(0);
    ...
    _updatePageRow: function(page) {
        ...
        $row.children('.metis-premium-cell').eq(0).html(
        ...
        $row.children('.metis-premium-cell').eq(1).html('<code class="metis-inline-code-muted">' + safeSlug + '</code>');
        $row.children('.metis-premium-cell').eq(2).html('<span class="metis-status metis-status-' + safeStatus + '">' + safeStatusLabel + '</span>');
        $row.children('.metis-premium-cell').eq(3).html('<code class="metis-inline-code">' + safeCode + '</code>');
        $row.children('.metis-premium-cell').eq(4).text(this._formatShortDate(page.updated_at || page.last_edit || ''));
        $row.children('.metis-premium-cell').eq(5).html(this._pageActionsMarkup(page));
        this._syncHomepageSelectorOption(page);
        this._applyHomepagePageId(page.is_homepage ? page.id : $('#metis-homepage-selector').val());
        this._updateListSubtitle('#metis-pages-list-shell', this._pageRows().length, 'page', 'pages', 'in CMS content.');
    },
    ...
    _publicSiteBaseUrl: function() {
        var ajax = (window.metisCmsAjax && window.metisCmsAjax.ajax_url) ? String(window.metisCmsAjax.ajax_url) : '';
        if (ajax !== '') {
            try {
    ...
    _baseNonce: function() {
        var cms = window.metisCmsAjax || {};
        var core = window.metisAjax || {};
        var metisAjax = window.Metis && window.Metis.ajax ? window.Metis.ajax : null;
        return String(cms.nonce || core.nonce || (metisAjax && metisAjax.nonce) || '').trim();
    },
    ...
    _csrfPayload: function(action) {
        ...
        return {
            ...
            metis_csrf_action: actionNonce !== '' ? ('metis_ajax:' + key) : 'metis_cms'
        };
    },
    ...
    _bindDashboardActions: function() {
        $(document).off('click.metisCMSDashboardPage').on('click.metisCMSDashboardPage', '#metis-dashboard-new-page-btn, #metis-dashboard-quick-new-page-btn', function() {
            MetisCMS.openPageEditor(null);
        });
    ...
    _bindPageActions: function() {
        $(document).off('click.metisCMSCreatePage').on('click.metisCMSCreatePage', '#metis-create-page-btn', function() {
            MetisCMS.openPageEditor(null);
        });

        $(document).off('click.metisCMSEditPage').on('click.metisCMSEditPage', '.metis-edit-page', function() {
            var code = String($(this).data('code') || '').trim();
            if (code !== '') {
                MetisCMS.openPageEditor(code);
                return;
            }
            var id = parseInt($(this).data('id'), 10);
            if (!Number.isNaN(id) && id > 0) {
                MetisCMS.openPageEditor(String(id));
            }
        });

        $(document).off('click.metisCMSViewPage').on('click.metisCMSViewPage', '.metis-view-page', function(e) {
            e.stopPropagation();
        });

        $(document).off('click.metisCMSPublishPage').on('click.metisCMSPublishPage', '.metis-publish-page', function() {
            var id = parseInt($(this).data('id'), 10);
            MetisCMS._publishPage(id);
        });

        $(document).off('click.metisCMSUnpublishPage').on('click.metisCMSUnpublishPage', '.metis-unpublish-page', function() {
            var id = parseInt($(this).data('id'), 10);
            MetisCMS._unpublishPage(id);
        });

        $(document).off('click.metisCMSDeletePage').on('click.metisCMSDeletePage', '.metis-delete-page', function() {
            var id = parseInt($(this).data('id'), 10);
            var title = String($(this).closest('.metis-premium-row').find('.metis-premium-cell:first strong').first().text() || '').trim();
            MetisCMS._confirm('Delete "' + title + '"? This cannot be undone.', function() {
                MetisCMS._deletePage(id);
            });
        });

        $(document).off('click.metisCMSHomepage').on('click.metisCMSHomepage', '#metis-set-homepage-btn', function() {
            var id = parseInt(String($('#metis-homepage-selector').val() || ''), 10);
            if (Number.isNaN(id) || id < 1) {
                metis_toast('Select a published page first.', 'warning');
                return;
            }
            MetisCMS._setHomepage(id);
        });
    },

    openPageEditor: function(pageRef) {
        if (pageRef) {
            this._gotoEditorPath('cms/pages/editor/' + encodeURIComponent(String(pageRef)) + '/');
            return;
        }
        this._gotoEditorPath('cms/pages/editor/new/');
    },
    ...
    _deletePage: function(id) {
        ...
        $.ajax({
            url: metisCmsAjax.ajax_url,
            type: 'POST',
            data: { action: 'metis_cms_page_delete', nonce: self._nonceFor('metis_cms_page_delete'), id: id },
            success: function(r) {
                if (r && r.success) {
                    metis_toast('Page deleted.', 'success');
                    MetisCMS._removePageRow((r.data && r.data.id) ? r.data.id : id);
                    MetisCMS._applyHomepagePageId((r.data && r.data.homepage_page_id) ? r.data.homepage_page_id : $('#metis-homepage-selector').val());
                }
    ...
    _publishPage: function(id) {
        ...
        $.ajax({
            url: metisCmsAjax.ajax_url,
            type: 'POST',
            data: { action: 'metis_cms_page_publish', nonce: self._nonceFor('metis_cms_page_publish'), id: id },
            success: function(r) {
                if (r && r.success && r.data && r.data.page) {
                    metis_toast('Page published.', 'success');
                    MetisCMS._updatePageRow(r.data.page);
                }
    ...
    _unpublishPage: function(id) {
        ...
        $.ajax({
            url: metisCmsAjax.ajax_url,
            type: 'POST',
            data: { action: 'metis_cms_page_unpublish', nonce: self._nonceFor('metis_cms_page_unpublish'), id: id },
            success: function(r) {
                if (r && r.success && r.data && r.data.page) {
    ...
    _setHomepage: function(id) {
        ...
        $.ajax({
            url: metisCmsAjax.ajax_url,
            type: 'POST',
            data: { action: 'metis_cms_homepage_set', nonce: self._nonceFor('metis_cms_homepage_set'), id: id },
            success: function(r) {
                if (r && r.success && r.data) {
                    ...
                    MetisCMS._applyHomepagePageId(r.data.homepage_page_id || id);
                    return;
                }
    ...
    _bindPostActions: function() {
        $(document).off('click.metisCMSCreatePost').on('click.metisCMSCreatePost', '#metis-create-post-btn', function() {
            MetisCMS.openPostEditor(null);
        });

        $(document).off('click.metisCMSEditPost').on('click.metisCMSEditPost', '.metis-edit-post', function() {
            var code = String($(this).data('code') || '').trim();
            if (code !== '') {
                MetisCMS.openPostEditor(code);
                return;
            }
            var id = parseInt($(this).data('id'), 10);
            if (!Number.isNaN(id) && id > 0) {
                MetisCMS.openPostEditor(String(id));
            }
        });

        $(document).off('click.metisCMSPublishPost').on('click.metisCMSPublishPost', '.metis-publish-post', function() {
            var id = parseInt($(this).data('id'), 10);
            MetisCMS._publishPost(id);
        });

        $(document).off('click.metisCMSDeletePost').on('click.metisCMSDeletePost', '.metis-delete-post', function() {
            var id = parseInt($(this).data('id'), 10);
            var title = String($(this).closest('.metis-premium-row').find('.metis-premium-cell:first strong').first().text() || '').trim();
            MetisCMS._confirm('Delete "' + title + '"? This cannot be undone.', function() {
                MetisCMS._deletePost(id);
            });
        });
    },
    ...
    openPostEditor: function(postRef) {
        if (postRef) {
            this._gotoEditorPath('cms/posts/editor/' + encodeURIComponent(String(postRef)) + '/');
            return;
        }
        this._gotoEditorPath('cms/posts/editor/new/');
    },
    ...
    _bindCategoryActions: function() {
        ...
        $(document).off('click.metisCMSCreateCategory').on('click.metisCMSCreateCategory', '#metis-create-post-category-btn, #metis-create-post-category-btn-empty', function(e) {
            e.preventDefault();
            MetisCMS._openPostCategoryCreate();
        });

        $(document).off('click.metisCMSEditCategory').on('click.metisCMSEditCategory', '.metis-edit-post-category', function(e) {
            e.preventDefault();
            MetisCMS._openPostCategoryEdit(this);
        });

        $(document).off('click.metisCMSDeleteCategory').on('click.metisCMSDeleteCategory', '.metis-delete-post-category', function(e) {
            e.preventDefault();
            MetisCMS._deletePostCategory(this);
        });

        $(document).off('click.metisCMSCloseCategory').on('click.metisCMSCloseCategory', '#metis-post-category-modal-close, #metis-post-category-cancel-btn', function(e) {
            e.preventDefault();
            MetisCMS._closePostCategoryModal();
        });

        $(document).off('click.metisCMSSaveCategory').on('click.metisCMSSaveCategory', '#metis-post-category-save-btn', function(e) {
            e.preventDefault();
            MetisCMS._savePostCategory();
        });

        $(document).off('click.metisCMSCategoryBackdrop').on('click.metisCMSCategoryBackdrop', '#metis-post-category-modal', function(e) {
    ...
    _postCategoryNonce: function(action) {
        ...
        var key = String(action || '').trim();
        if (key === 'metis_cms_post_category_save') {
            return String(view.attr('data-save-nonce') || '').trim();
        }
        if (key === 'metis_cms_post_category_delete') {
    ...
    _renderPostCategoryParentOptions: function(selectedId, currentId) {
        ...
        this._postCategoryParentOptions().forEach(function(option) {
            ...
            html.push('<option value="' + String(id) + '"' + (id === selected ? ' selected' : '') + '>' + MetisCMS._escHtml(label) + '</option>');
        });
    ...
    _renderPostCategoriesTable: function(categories) {
        ...
        wrap.empty();
        view.find('.metis-page-header .metis-subtitle').text(list.length + ' categor' + (list.length === 1 ? 'y' : 'ies') + ' available for posts.');
    ...
    _savePostCategory: function() {
        var payload = {
            action: 'metis_cms_post_category_save',
        ...
        $.ajax({
            ...
            success: function(r) {
                if (r && r.success) {
                    MetisCMS._renderPostCategoriesTable((r.data && r.data.categories) || []);
                    MetisCMS._closePostCategoryModal();
    ...
    _deletePostCategory: function(button) {
        ...
        this._confirm('Delete this category? This cannot be undone.', function() {
            var security = MetisCMS._csrfPayload('metis_cms_post_category_delete');
            var pageActionNonce = MetisCMS._postCategoryNonce('metis_cms_post_category_delete');
            $.ajax({
                url: metisCmsAjax.ajax_url,
                type: 'POST',
                data: {
                    action: 'metis_cms_post_category_delete',
                    nonce: security.nonce,
                    metis_action_nonce: pageActionNonce !== '' ? pageActionNonce : security.metis_action_nonce,
                    metis_csrf_action: (pageActionNonce !== '' ? 'metis_ajax:metis_cms_post_category_delete' : security.metis_csrf_action),
                    id: id
                },
                success: function(r) {
                    if (r && r.success) {
    ...
    _bindTemplateActions: function() {
        ...
        var self = this;
        $(document).off('click.metisCMSLayoutItem').on('click.metisCMSLayoutItem', '.metis-layout-gallery-item', function() {
        ...
        $(document).off('click.metisCMSPreviewOpen').on('click.metisCMSPreviewOpen', '#metis-layout-preview-open', function() {
            self._openLayoutPreviewModal();
        });
        $(document).off('change.metisCMSPreviewSelect').on('change.metisCMSPreviewSelect', '#metis-layout-preview-select', function() {
            var key = String($(this).val() || '').trim();
            self._renderLayoutPreviewCanvas(key);
        });
        $(document).off('change.metisCMSPreviewSource').on('change.metisCMSPreviewSource', '#metis-layout-preview-source', function() {
            var key = String($('#metis-layout-preview-select').val() || '').trim();
            self._renderLayoutPreviewCanvas(key);
        });
        $(document).off('click.metisCMSPreviewClose').on('click.metisCMSPreviewClose', '#metis-layout-preview-modal [data-action=\"close\"]', function() {
    ...
    _saveSiteLayoutProfile: function(key, name) {
        ...
        $.ajax({
            ...
            type: 'POST',
            data: {
                action: 'metis_cms_layout_profile_save',
                nonce: this._nonceFor('metis_cms_layout_profile_save'),
    ...
    _renderTemplateRows: function() {
        ...
        for (var i = 0; i < items.length; i += 1) {
            ...
            var row = [
                ...
                '  <td class="metis-premium-cell"><strong>' + this._escHtml(item.name || '') + '</strong></td>',
                '  <td class="metis-premium-cell">' + this._escHtml(this._templateTypeLabel(item.template_type || '')) + '</td>',
                '  <td class="metis-premium-cell"><span class="metis-status metis-status-' + this._escHtml(item.status || 'draft') + '">' + this._escHtml(this._titleCase(item.status || 'draft')) + '</span></td>',
                '  <td class="metis-premium-cell">' + (item.is_default ? 'Yes' : 'No') + '</td>',
                '  <td class="metis-premium-cell"><code>' + this._escHtml(item.template_key || '') + '</code></td>',
    ...
    _deleteTemplate: function(id) {
        ...
        $.ajax({
            ...
            type: 'POST',
            data: {
                action: 'metis_cms_template_delete',
                nonce: this._nonceFor('metis_cms_template_delete'),
                id: id
            },
            success: function(r) {
                if (!r || !r.success) {
    ...
    _deletePost: function(id) {
        ...
        $.ajax({
            url: metisCmsAjax.ajax_url,
            type: 'POST',
            data: { action: 'metis_cms_post_delete', nonce: self._nonceFor('metis_cms_post_delete'), id: id },
            success: function(r) {
                if (r && r.success) { metis_toast('Post deleted.', 'success'); MetisCMS._removePostRow((r.data && r.data.id) ? r.data.id : id); }
                else { metis_toast(r.data && r.data.message ? r.data.message : 'Delete failed.', 'error'); }
            },
            error: function() { metis_toast('Request failed.', 'error'); }
        });
    ...
    _publishPost: function(id) {
        ...
        $.ajax({
            url: metisCmsAjax.ajax_url,
            type: 'POST',
            data: { action: 'metis_cms_post_publish', nonce: self._nonceFor('metis_cms_post_publish'), id: id },
            success: function(r) {
                if (r && r.success && r.data && r.data.post) { metis_toast('Post published.', 'success'); MetisCMS._updatePostRow(r.data.post); }
                else { metis_toast(r.data && r.data.message ? r.data.message : 'Publish failed.', 'error'); }
            },
            error: function() { metis_toast('Request failed.', 'error'); }
        });
...
window.MetisCMS = MetisCMS;
function initMetisCmsModule() {
    MetisCMS.init();
}
if (document.readyState === 'loading') {
    document.addEventListener('DOMContentLoaded', initMetisCmsModule);
} else {
    initMetisCmsModule();
}
if (window.Metis && Metis.page && typeof Metis.page.register === 'function') {
    Metis.page.register('cms', initMetisCmsModule);
}
