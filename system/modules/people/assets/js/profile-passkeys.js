...
window.MetisPeopleProfileModules.initPasskeys = function (context) {
    ...
    function setPasskeyConfiguredUI() {
        ...
        const chip = rowWrap ? rowWrap.querySelector('.metis-chip') : null;
        if (chip) {
            ...
            post('metis_people_revoke_passkey', { passkey_id: passkeyId })
                .then(function (resp) {
                    ...
                    if (list && !list.querySelector('.metis-people-mini-item')) {
                        list.innerHTML = '<div class="metis-muted">No passkeys registered.</div>';
                    }
                    ...
                    if (passkey) {
                        ...
                        if (list) {
                            ...
                            item.innerHTML = '<div><strong>' + String(passkey.label || 'Passkey') + '</strong></div>'
                                + '<div class="metis-muted">Created: ' + String(passkey.created_at || '') + '</div>'
                                + '<div class="metis-people-mini-actions"><button type="button" class="metis-btn-xs metis-btn-danger metis-passkey-revoke" data-id="' + String(passkey.id || '') + '">Revoke</button></div>';
                            list.prepend(item);
