...
if ( function_exists( 'metis_portal_url' ) ) {
    $target = rtrim( metis_portal_url( 'settings', 'identity' ), '/' ) . '/general/';
    if ( ! headers_sent() ) {
        header( 'Location: ' . $target, true, 302 );
        exit;
    }
    echo '<script>(function(){var target=' . metis_json_encode( $target ) . ';if(window.Metis&&Metis.navigation&&typeof Metis.navigation.go==="function"){Metis.navigation.go(target);return;}window.location.assign(target);}());</script>';
    return;
}
...
$portal_slug        = Core_Settings_Service::get( 'portal_slug',           'metis-portal' );
...
<p class="metis-subtitle">Admin controls for Metis and integrations.</p>

<?php if ( $saved && empty( $errors ) ) : ?>
    <div class="metis-alert metis-alert-success">Settings saved.</div>
<?php endif; ?>

<?php if ( ! empty( $errors ) ) : ?>
    <div class="metis-alert metis-alert-error">
        <?php foreach ( $errors as $e ) echo '<p>' . $e . '</p>'; ?>
    </div>
<?php endif; ?>

<form method="post" class="metis-settings-form">
    <?php metis_runtime_nonce_field( 'metis_save_portal_settings', 'metis_settings_nonce' ); ?>
<div class="metis-list-layout">
    ...
    <aside class="metis-list-sidebar">
        ...
        <button type="button" class="metis-btn-xs metis-profile-tab-btn is-active" data-settings-tab="general">General</button>
        <button type="button" class="metis-btn-xs metis-profile-tab-btn" data-settings-tab="profile">Profile</button>
        <button type="button" class="metis-btn-xs metis-profile-tab-btn" data-settings-tab="newsletter">Newsletter</button>
        <button type="button" class="metis-btn-xs metis-profile-tab-btn" data-settings-tab="workspace">Workspace</button>
        <button type="button" class="metis-btn-xs metis-profile-tab-btn" data-settings-tab="drive">Drive</button>
        <button type="button" class="metis-btn-xs metis-profile-tab-btn" data-settings-tab="calendar">Calendar</button>
        <button type="button" class="metis-btn-xs metis-profile-tab-btn" data-settings-tab="api">API Keys</button>
        </div>
        <?php endif; ?>
    </aside>
        ...
        <div class="metis-settings-card">
            ...
            <div class="metis-settings-body">
                <div class="metis-field">
                    ...
                    <input type="text" id="portal_slug" name="portal_slug" class="metis-input metis-input-wide" value="<?php echo metis_escape_attr( $portal_slug ); ?>" placeholder="metis-portal">
                </div>
                <div class="metis-field">
            ...
            <div class="metis-settings-body">
                <div class="metis-field">
                    <label class="metis-people-check">
                        <input type="checkbox" name="profile_allow_name_edit" value="1" <?php metis_attr_metis_attr_checked( $profile_allow_name_edit ); ?>>
    ...
    <div class="metis-settings-panel" data-settings-panel="newsletter">
        <div class="metis-settings-card">
            <div class="metis-settings-header"><h2>Newsletter Defaults</h2></div>
            <div class="metis-settings-body">
                <div class="metis-field">
                ...
                <div class="metis-field">
                    ...
                    <input type="password" id="newsletter_klipy_api_key" name="newsletter_klipy_api_key" class="metis-input metis-input-wide" value="" autocomplete="new-password" placeholder="<?php echo metis_escape_attr( is_string( $newsletter_klipy_api_key ) && trim( $newsletter_klipy_api_key ) !== '' ? metis_mask_key( (string) $newsletter_klipy_api_key ) : 'Paste your Klipy API key' ); ?>">
                    <p class="metis-help">Leave blank to keep the current API key.</p>
                    <p class="metis-help">Used by the Klipy block and GIF search in the newsletter editor.</p>
                </div>
                <div class="metis-field">
    ...
    <div class="metis-settings-panel" data-settings-panel="drive">
        <div class="metis-settings-card">
            <div class="metis-settings-header"><h2>Drive Configurations</h2></div>
            <div class="metis-settings-body">
                <?php if ( ! $is_system_admin ) : ?>
                    <div class="metis-callout metis-callout-warning">Only system admins can manage Drive configurations.</div>
                <?php else : ?>
                    <p class="metis-help">Add one or more shared drives. Mark one as the default Drive module source and mark one as the users-home drive.</p>
                    <?php if ( $workspace_shared_drive_error !== '' ) : ?>
                        <p class="metis-help" style="color:#b91c1c;"><?php echo metis_escape_html( $workspace_shared_drive_error ); ?></p>
                    <?php endif; ?>
                        ...
                        <div class="metis-settings-repeatable-list" data-repeatable-list="drive">
                            <?php foreach ( $workspace_drive_configs as $index => $row ) : ?>
                                <div class="metis-settings-row" data-repeatable-row>
                                    <div class="metis-field">
                                        <label>Label</label>
                                        <input type="text" class="metis-input" name="workspace_drive_configs[<?php echo (int) $index; ?>][label]" value="<?php echo metis_escape_attr( (string) ( $row['label'] ?? '' ) ); ?>" placeholder="Board Drive">
                                    </div>
                                    <div class="metis-field">
                                        <label>Shared Drive</label>
                                        <select class="metis-input" name="workspace_drive_configs[<?php echo (int) $index; ?>][drive_id]" data-drive-select>
                                            <option value="">Select shared drive</option>
                                            <?php foreach ( $workspace_shared_drive_options as $opt ) :
                                                $opt_id = (string) ( $opt['id'] ?? '' );
                                                if ( $opt_id === '' ) { continue; }
                                            ?>
                                                <option value="<?php echo metis_escape_attr( $opt_id ); ?>" data-drive-name="<?php echo metis_escape_attr( (string) ( $opt['name'] ?? $opt_id ) ); ?>" <?php metis_attr_metis_attr_selected( (string) ( $row['drive_id'] ?? '' ), $opt_id ); ?>>
                                    ...
                                    <label class="metis-settings-flag"><input type="checkbox" name="workspace_drive_configs[<?php echo (int) $index; ?>][is_default]" value="1" <?php metis_attr_metis_attr_checked( ! empty( $row['is_default'] ) ); ?>> Default</label>
                                    <label class="metis-settings-flag"><input type="checkbox" name="workspace_drive_configs[<?php echo (int) $index; ?>][is_users_home]" value="1" <?php metis_attr_metis_attr_checked( ! empty( $row['is_users_home'] ) ); ?>> Users folder</label>
                                    <button type="button" class="metis-btn metis-btn-xs metis-btn-danger" data-repeatable-remove>Remove</button>
                                </div>
                            <?php endforeach; ?>
                        </div>
                        <button type="button" class="metis-btn metis-btn-secondary metis-btn-xs" data-repeatable-add="drive">Add Drive</button>
                    </div>
                <?php endif; ?>
            </div>
        </div>
    ...
    <div class="metis-settings-panel" data-settings-panel="calendar">
        <div class="metis-settings-card">
            <div class="metis-settings-header"><h2>Calendar Configurations</h2></div>
            <div class="metis-settings-body">
                <?php if ( ! $is_system_admin ) : ?>
                    <div class="metis-callout metis-callout-warning">Only system admins can manage Calendar configurations.</div>
                <?php else : ?>
                    <p class="metis-help">Add one or more calendars. Mark one as the default Calendar module source.</p>
                    <?php if ( $workspace_calendar_error !== '' ) : ?>
                        <p class="metis-help" style="color:#b91c1c;"><?php echo metis_escape_html( $workspace_calendar_error ); ?></p>
                    <?php endif; ?>
                        ...
                        <div class="metis-settings-repeatable-list" data-repeatable-list="calendar">
                            <?php foreach ( $workspace_calendar_configs as $index => $row ) : ?>
                                <div class="metis-settings-row" data-repeatable-row>
                                    <div class="metis-field">
                                        <label>Label</label>
                                        <input type="text" class="metis-input" name="workspace_calendar_configs[<?php echo (int) $index; ?>][label]" value="<?php echo metis_escape_attr( (string) ( $row['label'] ?? '' ) ); ?>" placeholder="Board Calendar">
                                    </div>
                                    <div class="metis-field">
                                        <label>Calendar</label>
                                        <select class="metis-input" name="workspace_calendar_configs[<?php echo (int) $index; ?>][calendar_id]" data-calendar-select>
                                            <option value="">Select calendar</option>
                                            <?php foreach ( $workspace_calendar_options as $opt ) :
                                                $opt_id = (string) ( $opt['id'] ?? '' );
                                                if ( $opt_id === '' ) { continue; }
                                            ?>
                                                <option value="<?php echo metis_escape_attr( $opt_id ); ?>" data-calendar-name="<?php echo metis_escape_attr( (string) ( $opt['name'] ?? $opt_id ) ); ?>" <?php metis_attr_metis_attr_selected( (string) ( $row['calendar_id'] ?? '' ), $opt_id ); ?>>
                                                    <?php echo metis_escape_html( (string) ( $opt['name'] ?? $opt_id ) ); ?>
                                                </option>
                                            <?php endforeach; ?>
                                        </select>
                                    ...
                                    <label class="metis-settings-flag"><input type="checkbox" name="workspace_calendar_configs[<?php echo (int) $index; ?>][is_default]" value="1" <?php metis_attr_metis_attr_checked( ! empty( $row['is_default'] ) ); ?>> Default</label>
                                    <button type="button" class="metis-btn metis-btn-xs metis-btn-danger" data-repeatable-remove>Remove</button>
                                </div>
                            <?php endforeach; ?>
                        </div>
                        <button type="button" class="metis-btn metis-btn-secondary metis-btn-xs" data-repeatable-add="calendar">Add Calendar</button>
                    </div>
                <?php endif; ?>
            </div>
        </div>
    ...
    <div class="metis-settings-panel" data-settings-panel="api">
        <div class="metis-settings-card">
            ...
            <div class="metis-settings-body">
                <?php if ( ! $is_system_admin ) : ?>
                    <div class="metis-callout metis-callout-warning">Only system admins can view or update API keys and service account credentials.</div>
                <?php else : ?>
                    <div class="metis-field">
                        <label for="stripe_secret">Stripe Secret Key</label>
                        <input type="password" id="stripe_secret" name="stripe_secret" class="metis-input metis-input-wide" autocomplete="new-password" placeholder="<?php echo $stripe_connected ? metis_escape_attr( metis_mask_key( $stripe_secret ) ) : 'sk_live_••••••••••'; ?>">
                    </div>
                    <div class="metis-field">
                    ...
                    <div class="metis-field">
                        ...
                        <input type="password" id="stripe_webhook_secret" name="stripe_webhook_secret" class="metis-input metis-input-wide" autocomplete="new-password" placeholder="<?php echo $webhook_configured ? metis_escape_attr( metis_mask_key( $webhook_secret ) ) : 'whsec_••••••••••'; ?>">
                    </div>
                    <div class="metis-field">
                    ...
                    <div class="metis-field">
                        ...
                        <textarea id="workspace_service_account_json" name="workspace_service_account_json" class="metis-input metis-input-wide" rows="8" autocomplete="off" spellcheck="false" placeholder='{"type":"service_account","client_email":"...","private_key":"...","token_uri":"https://oauth2.googleapis.com/token"}'></textarea>
                    </div>
                    <div class="metis-field">
                        <label>Webhook Endpoint URL</label>
                        <div class="metis-shortcode-wrap">
                            <code class="metis-shortcode" id="metis-webhook-url"><?php echo metis_escape_html( metis_webhook_url( 'stripe' ) ); ?></code>
                            <button type="button" class="metis-btn metis-btn-xs metis-btn-ghost" id="metis-copy-webhook-url">Copy</button>
                        </div>
                    </div>
                <?php endif; ?>
            </div>
    ...
    <div class="metis-settings-actions">
        <button type="submit" class="metis-btn">Save Settings</button>
    </div>
</div></div>
</form>
<script>
(function () {
    ...
    const copyBtn = document.getElementById('metis-copy-webhook-url');
    if (!copyBtn) return;
    copyBtn.addEventListener('click', function () {
        const urlEl = document.getElementById('metis-webhook-url');
        if (!urlEl) return;
    ...
    function createDriveRow(index) {
        ...
        wrap.innerHTML = `
            <div class="metis-field">
                <label>Label</label>
                <input type="text" class="metis-input" name="workspace_drive_configs[${index}][label]" placeholder="Board Drive">
            </div>
            <div class="metis-field">
                <label>Shared Drive</label>
                <select class="metis-input" name="workspace_drive_configs[${index}][drive_id]" data-drive-select>
                    <option value="">Select shared drive</option>
                    <?php foreach ( $workspace_shared_drive_options as $opt ) :
                        $opt_id = (string) ( $opt['id'] ?? '' );
                        if ( $opt_id === '' ) { continue; }
    ...
    function createCalendarRow(index) {
        ...
        wrap.innerHTML = `
            <div class="metis-field">
                <label>Label</label>
                <input type="text" class="metis-input" name="workspace_calendar_configs[${index}][label]" placeholder="Board Calendar">
            </div>
            <div class="metis-field">
                <label>Calendar</label>
                <select class="metis-input" name="workspace_calendar_configs[${index}][calendar_id]" data-calendar-select>
                    <option value="">Select calendar</option>
                    <?php foreach ( $workspace_calendar_options as $opt ) :
                        $opt_id = (string) ( $opt['id'] ?? '' );
                        if ( $opt_id === '' ) { continue; }
