function initMetisCalendar(context) {
    ...
    function renderSelectedDay() {
        ...
        selectedEventsEl.innerHTML = items.map(function (it) {
            ...
            const actions = [];
            if (it.htmlLink) actions.push('<a class="metis-btn-xs metis-btn-ghost" href="' + escHtml(it.htmlLink) + '" target="_blank" rel="noopener">Open</a>');
            if (canManage) actions.push('<button type="button" class="metis-btn-xs metis-calendar-edit" data-id="' + escHtml(it.id || '') + '">Edit</button>');
            return '<article class="metis-calendar-event-card" data-event-id="' + escHtml(it.id || '') + '" role="button" tabindex="0" aria-label="View details for ' + escHtml(it.summary || '(untitled)') + '" style="border-left:4px solid ' + escHtml(color) + ';">' +
                '<div class="metis-calendar-event-card-top">' +
                    ...
                    '<span class="metis-chip" style="border-color:' + escHtml(color) + ';color:' + escHtml(color) + ';">' + escHtml(itemBadge(it)) + '</span>' +
                '</div>' +
