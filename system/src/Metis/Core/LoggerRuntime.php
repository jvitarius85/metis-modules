...
class Metis_Logger {
    ...
    private static function resolve_log_file( int $incoming_bytes = 0 ): string {
        ...
        for ( $index = count( $paths ) - 1; $index >= 0; $index-- ) {
            $path = $paths[ $index ] ?? '';
            if ( ! is_string( $path ) || $path === '' || ! self::path_allows_append( $path ) ) {
                continue;
            }
        ...
        $next_path = self::part_path( $base_path, self::next_log_part_number( $base_path, $paths ) );
    ...
    private static function period_log_paths( ?string $base_path = null ): array {
        ...
        $prefix = substr( $base_path, 0, -4 );
        foreach ( glob( $prefix . '-part*.log' ) ?: [] as $part_path ) {
            if ( ! is_string( $part_path ) ) {
                continue;
            }

            if ( preg_match( '/-part(\d+)\.log$/', $part_path, $matches ) !== 1 ) {
                continue;
            }

            $part = max( 2, (int) $matches[1] );
            $paths[ $part ] = $part_path;
        }
    ...
    private static function next_log_part_number( string $base_path, array $paths ): int {
        $max_part = file_exists( $base_path ) ? 1 : 0;
        ...
        foreach ( $paths as $path ) {
            ...
            if ( preg_match( '/-part(\d+)\.log$/', $path, $matches ) === 1 ) {
                $max_part = max( $max_part, (int) $matches[1] );
            }
        }

        return max( 2, $max_part + 1 );
    }
