...
function metis_runtime_storage_path( string $file ): string {
    $dir = rtrim( (string) METIS_PATH, '/\\' ) . '/storage/runtime';
    if ( ! is_dir( $dir ) ) {
        mkdir( $dir, 0775, true );
    }
    if ( is_dir( $dir ) ) {
    ...
    if ( is_dir( $path_dir ) ) {
        @chmod( $path_dir, 02775 );
    }

    return $path;
}

function metis_runtime_prepare_storage_file( string $path ): void {
    $path_dir = dirname( $path );
    if ( is_dir( $path_dir ) ) {
        @chmod( $path_dir, 02775 );
    }
...
function metis_runtime_json_store_write( string $file, array $payload ): void {
    $path = metis_runtime_storage_path( $file );
    metis_runtime_prepare_storage_file( $path );
    ...
    file_put_contents( $temp_path, $encoded, LOCK_EX );
