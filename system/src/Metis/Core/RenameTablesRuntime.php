...
metis_on( 'metis_admin_init', function () {
    ...
    if ( $mode === 'collisions' ) {
        ...
        if ( empty( $result['collision'] ) ) {
            ...
            metis_runtime_die(
                '<pre style="font-family:monospace; font-size:14px; line-height:1.7;">' . metis_esc_html( implode( "\n", $lines ) ) . '</pre>',
    ...
    if ( $mode === 'resolve' ) {
        ...
        if ( $result === false ) {
            metis_runtime_die(
                "<pre>❌ Rename failed. Review logs for details.</pre>",
                'Metis — Rename Failed',
                [ 'response' => 500 ]
            );
        }
