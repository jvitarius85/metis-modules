...
function metis_module_boot_failures(): array {
    $service = Metis::service( 'modules' );
    if ( method_exists( $service, 'bootFailures' ) ) {
        return (array) $service->bootFailures();
    }

    return [];
}

function metis_module_compliance_report( bool $force_refresh = false ): array {
    $service = Metis::service( 'modules' );
    if ( method_exists( $service, 'complianceReport' ) ) {
        return (array) $service->complianceReport( $force_refresh );
    }
...
if ( function_exists( 'metis_quick_actions_service' ) ) {
    metis_quick_actions_service()->bootstrap();
}
