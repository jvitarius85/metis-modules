...
final class GitHubClient {
    ...
    public function repositoryFile(string $owner, string $repo, string $path, string $destination, string $ref = '', string $token = ''): array {
        $headers = [
            'Accept: application/vnd.github.raw',
            'User-Agent: Metis-Module-Updater/1.0',
        ];
        ...
        if (!is_dir($directory) && !mkdir($directory, 0775, true) && !is_dir($directory)) {
            throw new \RuntimeException('Unable to create module archive directory.');
        }

        if (function_exists('curl_init')) {
            $fp = @fopen($destination, 'wb');
            if (!is_resource($fp)) {
                throw new \RuntimeException('Unable to create module archive file.');
            }

            $ch = curl_init($url);
            if ($ch === false) {
                fclose($fp);
                throw new \RuntimeException('Unable to initialize module archive download.');
            }
            ...
            if ($ok !== true || $status >= 400 || !is_file($destination) || filesize($destination) < 1) {
                @unlink($destination);
                throw new \RuntimeException($error !== '' ? $error : sprintf('GitHub contents download failed with status [%d].', $status));
            }
        } else {
            ...
            if (!is_string($data) || $data === '') {
                throw new \RuntimeException('GitHub contents download failed.');
            }
            if (@file_put_contents($destination, $data, LOCK_EX) === false) {
                throw new \RuntimeException('Unable to save module archive.');
            }
        }
