...
/**
 * Persists editor revision timeline entries for cms entities.
 */
final class RevisionTimelineService {
    ...
    public static function save( string $entity_type, int $entity_id, array $payload, string $note = '' ): bool {
        ...
        $latest = self::db()->fetchOne(
            'SELECT id, revision_data FROM ' . \Metis_Tables::get( 'cms_revisions' ) . ' WHERE entity_type = %s AND entity_id = %d ORDER BY id DESC LIMIT 1',
        ...
        $inserted = self::db()->insert(
            \Metis_Tables::get( 'cms_revisions' ),
            [
                'entity_type' => $normalized_type,
                'entity_id' => $entity_id,
    ...
    public static function list( string $entity_type, int $entity_id, int $limit = 50 ): array {
        ...
        $rows = self::db()->fetchAll(
            'SELECT id, revision_note, created_by, created_at, revision_data FROM ' . \Metis_Tables::get( 'cms_revisions' ) . ' WHERE entity_type = %s AND entity_id = %d ORDER BY id DESC LIMIT %d',
    ...
    public static function payloadForRevision( string $entity_type, int $entity_id, int $revision_id ): ?array {
        ...
        $row = self::db()->fetchOne(
            'SELECT revision_data FROM ' . \Metis_Tables::get( 'cms_revisions' ) . ' WHERE id = %d AND entity_type = %s AND entity_id = %d LIMIT 1',
