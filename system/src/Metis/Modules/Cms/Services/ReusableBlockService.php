 ...
 *
 * Stores and retrieves reusable editor blocks in cms_blocks.
 */
final class ReusableBlockService {
    ...
    public static function listForContext( string $context, string $render_mode = '', string $search = '', int $limit = 200 ): array {
        $db = self::db();
        $table = \Metis_Tables::get( 'cms_blocks' );
        $rows = $db->fetchAll(
        ...
        $registry = EditorContextPolicy::filterRegistry( \Metis\Modules\Cms\BlockRegistry::all(), $normalized_context, $normalized_mode );
    ...
    public static function getByCode( string $block_code ): ?array {
        ...
        $row = self::db()->fetchOne(
            "SELECT id, block_code, name, type, block_json, category, is_global, created_by, updated_by, created_at, updated_at
             FROM " . \Metis_Tables::get( 'cms_blocks' ) . " WHERE block_code = %s LIMIT 1",
            [ $code ]
        );
        if ( ! is_array( $row ) ) {
            return null;
    ...
    public static function save( array $block, string $name, string $category = 'custom', bool $is_global = true, string $block_code = '' ): ?array {
        ...
        if ( function_exists( 'metis_entity_id_service' ) ) {
            $payload = \metis_entity_id_service()->assignForInsert( 'cms_block', $payload );
        }
        ...
        if ( function_exists( 'metis_entity_id_service' ) ) {
            \metis_entity_id_service()->register( 'cms_block', $id, (string) ( $payload['block_code'] ?? '' ) );
        }
