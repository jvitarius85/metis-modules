...
use Metis\Modules\Cms\Entities\GlobalLayout;
...
final class GlobalLayoutService {
    ...
    public static function getDefault( string $type ): ?GlobalLayout {
        $db    = self::db();
        $table = \Metis_Tables::get( 'cms_global_layouts' );
        $row   = $db->fetchOne(
    ...
    public static function getById( int $id ): ?GlobalLayout {
        $row = self::db()->fetchOne( "SELECT * FROM " . \Metis_Tables::get( 'cms_global_layouts' ) . " WHERE id = %d", [ $id ] );
        return is_array( $row ) ? GlobalLayout::fromRow( $row ) : null;
    }

    public static function create( array $data ): int|false {
        $db = self::db();
        $result = $db->insert( \Metis_Tables::get( 'cms_global_layouts' ), [
    ...
    public static function update( int $id, array $data ): bool {
        ...
        return (bool) self::db()->update( \Metis_Tables::get( 'cms_global_layouts' ), $update, [ 'id' => $id ] );
    }

    public static function delete( int $id ): bool {
        return (bool) self::db()->delete( \Metis_Tables::get( 'cms_global_layouts' ), [ 'id' => $id ] );
    }
