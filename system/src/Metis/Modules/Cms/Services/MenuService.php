...
/**
 * Menu Service — manages cms navigation menus.
 */
final class MenuService {

    public static function getAll(): array {
        $rows = self::db()->fetchAll( "SELECT * FROM " . \Metis_Tables::get( 'cms_menus' ) . " ORDER BY name ASC" );
        return is_array( $rows ) ? $rows : [];
    }

    public static function getById( int $id ): ?array {
        $row = self::db()->fetchOne( "SELECT * FROM " . \Metis_Tables::get( 'cms_menus' ) . " WHERE id = %d", [ $id ] );
        return is_array( $row ) ? $row : null;
    }

    public static function getByLocation( string $location ): ?array {
        $row = self::db()->fetchOne(
            "SELECT * FROM " . \Metis_Tables::get( 'cms_menus' ) . " WHERE location = %s AND status = 'active' LIMIT 1",
            [ $location ]
        );
        return is_array( $row ) ? $row : null;
    }
    ...
    public static function create( array $data ): int|false {
        $result = self::db()->insert( \Metis_Tables::get( 'cms_menus' ), [
    ...
    public static function update( int $id, array $data ): bool {
        ...
        return (bool) self::db()->update( \Metis_Tables::get( 'cms_menus' ), $update, [ 'id' => $id ] );
    }

    public static function delete( int $id ): bool {
        return (bool) self::db()->delete( \Metis_Tables::get( 'cms_menus' ), [ 'id' => $id ] );
    }
