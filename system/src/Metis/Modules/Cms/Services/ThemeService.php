...
/**
 * Theme Service — manages cms-wide design tokens and theme configuration.
 */
final class ThemeService {
    ...
    public static function getActive(): ?array {
        $row = self::db()->fetchOne(
            "SELECT * FROM " . \Metis_Tables::get( 'cms_theme_config' ) . " WHERE is_active = 1 ORDER BY id DESC LIMIT 1"
        );
    ...
    public static function getById( int $id ): ?array {
        $row = self::db()->fetchOne(
            "SELECT * FROM " . \Metis_Tables::get( 'cms_theme_config' ) . " WHERE id = %d",
            [ $id ]
        );
        return is_array( $row ) ? $row : null;
    }
    ...
    public static function save( array $data ): int|false {
        ...
        $result = self::db()->insert( \Metis_Tables::get( 'cms_theme_config' ), [
    ...
    public static function activate( int $id ): bool {
        $db    = self::db();
        $table = \Metis_Tables::get( 'cms_theme_config' );
    ...
    public static function update( int $id, array $data ): bool {
        ...
        $ok = (bool) self::db()->update( \Metis_Tables::get( 'cms_theme_config' ), $update, [ 'id' => $id ] );
    ...
    private static function normalizeThemeParts( array $global_styles, array $typography, array $colors, array $spacing, array $custom_tokens ): array {
        ...
        $site_layout_profile = LayoutProfileService::sanitizeCMSProfile(
            (string) ( $global_settings['site_layout_profile'] ?? LayoutProfileService::defaultCMSProfileKey() )
        );
    ...
    private static function localFontFacesCss( array $selected_families = [] ): array {
        $roots = [
            ...
            [ 'dir' => METIS_MODULES_PATH . 'cms/assets/fonts', 'source' => 'module' ],
        ];
        $faces = [];
        $seen_files = [];
            ...
            foreach ( $iterator as $file_info ) {
                ...
                if ( $source === 'runtime' ) {
                    ...
                    $url = function_exists( 'metis_module_asset_url' )
                        ? (string) metis_module_asset_url( 'cms', 'fonts/' . $relative )
                        : ( function_exists( 'metis_home_url' )
                            ? (string) metis_home_url( '/assets/modules/cms/fonts/' . self::encodeAssetPath( $relative ) )
                            : '/assets/modules/cms/fonts/' . self::encodeAssetPath( $relative )
                        );
                }
