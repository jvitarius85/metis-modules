...
final class BannerService {
    ...
    public static function getAll( array $filters = [] ): array {
        $db     = self::db();
        $table  = \Metis_Tables::get( 'cms_banners' );
        $where  = [];
        $params = [];
    ...
    public static function getById( int $id ): ?array {
        $row = self::db()->fetchOne(
            'SELECT * FROM ' . \Metis_Tables::get( 'cms_banners' ) . ' WHERE id = %d LIMIT 1',
            [ $id ]
        );
    ...
    public static function create( array $data ): int|false {
        ...
        if ( function_exists( 'metis_entity_id_service' ) ) {
            $normalized = \metis_entity_id_service()->assignForInsert( 'cms_banner', $normalized );
        } else {
            $normalized['banner_code'] = \metis_generate_code( 'CMB', \Metis_Tables::get( 'cms_banners' ), 'banner_code' );
        }

        $ok = self::db()->insert( \Metis_Tables::get( 'cms_banners' ), $normalized );
        if ( ! $ok ) {
            return false;
        }
        ...
        if ( $id > 0 && ! empty( $normalized['banner_code'] ) ) {
            if ( function_exists( 'metis_entity_id_service' ) ) {
                \metis_entity_id_service()->register( 'cms_banner', $id, (string) $normalized['banner_code'] );
            }
        }
    ...
    public static function update( int $id, array $data ): bool {
        ...
        return (bool) self::db()->update( \Metis_Tables::get( 'cms_banners' ), $update, [ 'id' => $id ] );
    }

    public static function delete( int $id ): bool {
        return (bool) self::db()->delete( \Metis_Tables::get( 'cms_banners' ), [ 'id' => $id ] );
    }
