...
use Metis\Modules\Cms\Entities\Page;
...
final class CmsRenderer {
    ...
    public static function renderStructuredEditorPreview( string $layout_json, array $options = [] ): array {
        $context_name = metis_key_clean( (string) ( $options['context'] ?? 'cms' ) );
        if ( $context_name === '' ) {
            $context_name = 'cms';
        }
        ...
        $sections = StructuredCmsBuilderService::sectionsFromLayout( $layout_json, $is_post );
        $hero = StructuredCmsBuilderService::heroFromLayout( $layout_json, $is_homepage );
    ...
    public static function renderHomepagePlaceholder(): string {
        $admin_url = function_exists( 'metis_portal_url' )
            ? (string) metis_portal_url( 'cms', 'pages' )
            : ( function_exists( 'metis_admin_url' ) ? (string) metis_admin_url( 'cms/pages' ) : '/cms/pages/' );
        ...
        $html .= '<p class="brand-kicker">Metis CMS Setup</p>';
        ...
        $html .= '<p class="lead">Create and publish a page, then set it as homepage in CMS editor or Settings to make it the live root page.</p>';
        $html .= '<div class="meta">';
        ...
        $html .= '<div class="meta-item"><span class="meta-label">Next step</span><div class="meta-value">Open CMS Pages, create or publish your homepage, then refresh this route.</div></div>';
        $html .= '</div>';
        $html .= '<div class="actions"><a class="btn btn-secondary" href="' . metis_escape_attr( $admin_url ) . '">Open CMS Pages</a></div>';
    ...
    public static function renderPageEntity( Page $page, array $context = [] ): string {
        ...
        $sections = StructuredCmsBuilderService::sectionsFromLayout( (string) $layout_raw, false );
        ...
        $hero = StructuredCmsBuilderService::heroFromLayout( (string) $layout_raw, $is_homepage );
        $context = array_merge(
            [
    ...
    public static function renderPostEntity( Post $post, array $context = [] ): string {
        ...
        $layout_settings = self::layoutSettingsFromRaw( $layout_raw );
        $sections = StructuredCmsBuilderService::sectionsFromLayout( (string) $layout_raw, true );
        $hero = StructuredCmsBuilderService::heroFromLayout( (string) $layout_raw, false );
    ...
    private static function renderWithPipeline( array $input ): string {
        ...
        $site_layout_profile_key = LayoutProfileService::sanitizeCMSProfile(
        ...
        $site_layout_profile = LayoutProfileService::resolveCMSProfile( $site_layout_profile_key );
    ...
    private static function buildPageHtml( array $data ): string {
        ...
        $site_layout_profile_key = metis_key_clean( (string) ( $data['site_layout_profile_key'] ?? ( $site_layout_profile['key'] ?? LayoutProfileService::defaultCMSProfileKey() ) ) );
        ...
        if ( $site_layout_profile_key === '' ) {
            $site_layout_profile_key = LayoutProfileService::defaultCMSProfileKey();
        }
        ...
        $head[] = '  <meta property="og:type" content="' . metis_escape_attr( metis_key_clean( (string) ( $context['content_type'] ?? '' ) ) === 'post' ? 'article' : 'cms' ) . '">';
        if ( $og_image !== '' ) {
    ...
    private static function stylesheetHref( array $context, array $template_structure = [], bool $template_preview_mode = false ): string {
        $base = function_exists( 'metis_home_url' )
            ? (string) metis_home_url( '/v1/cms/theme.css' )
            : '/metis/v1/cms/theme.css';
    ...
    private static function publicAssetUrl( string $asset ): string {
        ...
        if ( function_exists( 'metis_module_asset_url' ) ) {
            $base = (string) metis_module_asset_url( 'cms', $file );
        } else {
            $base = '/metis/assets/modules/cms/' . $file;
        }
    ...
    private static function assetVersionToken( array $template_structure = [], bool $include_theme_state = false ): string {
        ...
        $candidates = [
            METIS_SRC_PATH . 'Metis/Modules/CMS/Services/CmsRenderer.php',
            METIS_SRC_PATH . 'Metis/Modules/CMS/Services/ThemeService.php',
            METIS_MODULES_PATH . 'cms/assets/public-navigation.js',
        ];
        $template_slug = self::templateVariantSlug( $template_structure );
        if ( $template_slug !== '' ) {
            $candidates[] = METIS_MODULES_PATH . 'cms/Templates/' . $template_slug . '/structure.css';
            $candidates[] = METIS_MODULES_PATH . 'cms/Templates/' . $template_slug . '/menu.css';
        }
        $latest = 0;
    ...
    private static function renderShellHeader( array $context, array $site_layout_profile = [], array $menu_dataset = [] ): string {
        ...
        $site_name = function_exists( 'metis_portal_name' ) ? (string) metis_portal_name() : 'CMS';
        $org_name = class_exists( 'Core_Settings_Service' ) ? (string) \Core_Settings_Service::get( 'org_name', '' ) : '';
    ...
    private static function renderShellPartial( string $name, array $vars ): string {
        ...
        $path = METIS_MODULES_PATH . 'cms/Templates/shell/' . $slug . '.php';
        if ( ! is_file( $path ) ) {
            return '';
        }
    ...
    public static function renderThemeMenuPreviewHtml(): string {
        $theme = ThemeService::getActiveNormalized();
        $profile_key = LayoutProfileService::sanitizeCMSProfile(
            (string) ( $theme['global_settings']['site_layout_profile'] ?? LayoutProfileService::defaultCMSProfileKey() )
        );
        $profile = LayoutProfileService::resolveCMSProfile( $profile_key );
    ...
    private static function previewLayoutProfileOverride(): string {
        ...
        if ( ! function_exists( 'metis_security_user_can' ) || ! metis_security_user_can( 'cms.edit' ) ) {
            return '';
        }
        ...
        return LayoutProfileService::sanitizeCMSProfile( $raw );
    }
    ...
    private static function renderStructuredEventsSection( array $content, array $context = [] ): string {
        ...
        if ( $source === 'manual' ) {
            ...
            try {
                $configs = self::calendarConfigsForCMS( $requested_calendar_id );
     ...
     * @return array<int,array<string,mixed>>
     */
    private static function calendarConfigsForCMS( string $requested_calendar_id = '' ): array {
        $configs = [];
        $all = [];
    ...
    private static function publicBaseCss(): string {
        return implode( "\n", [
            ...
            '.metis-structured-events__item,.metis-structured-posts-list__item{background: var(--metis-surface, #fff);border:1px solid color-mix(in srgb,var(--metis-color-accent,#ff7542) 42%, var(--metis-color-border,#dbe3ef));border-radius:22px;padding:20px;box-shadow:0 14px 28px rgba(15,23,42,.06);backdrop-filter:blur(8px);}',
    ...
    private static function readMenuCssAsset( string $variant ): string {
        ...
        $path = METIS_ASSETS_PATH . 'css/cms/menu-' . $name . '.css';
