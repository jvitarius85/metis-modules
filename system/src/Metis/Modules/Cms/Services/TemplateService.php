...
use Metis\Modules\Cms\Entities\Template;
use Metis\Modules\Cms\Entities\Page;
use Metis\Modules\Cms\Entities\Post;

final class TemplateService {
    private const ACTIVE_TEMPLATE_KEY = 'cms_active_template_slug';
    ...
    public static function getDefault( string $type ): ?Template {
        ...
        $template_key = StructuredCmsBuilderService::defaultTemplateForPageType( $page_type );
    ...
    private static function templateKeys(): array {
        return array_values( array_unique( array_merge( [ 'default' ], StructuredCmsBuilderService::templateKeys() ) ) );
    }

    private static function normalizeStructuredTemplateKey( string $raw, string $page_type ): string {
        return StructuredCmsBuilderService::resolveTemplateKey( $raw, $page_type );
    }
    ...
    private static function canonicalTemplateKeyOrRaw( string $raw ): string {
        ...
        $candidate = StructuredCmsBuilderService::resolveTemplateKey( $normalized_raw, 'page' );
        if ( $candidate === '' || ! StructuredCmsBuilderService::isStructuredTemplateKey( $candidate ) ) {
            return '';
        }
        return $candidate;
    }
    ...
    private static function pageTypeFromEntity( object $entity, bool $is_post = false ): string {
        ...
        if ( $page_type === '' ) {
            ...
            if ( is_array( $layout ) ) {
                $meta = StructuredCmsBuilderService::structuredMetaFromDecodedLayout( $layout );
