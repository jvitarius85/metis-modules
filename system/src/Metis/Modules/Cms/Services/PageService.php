...
/**
 * Page Service — CRUD for cms pages.
 */
final class PageService {
    ...
    public static function getById( int $id ): ?Page {
        $db    = self::db();
        $table = \Metis_Tables::get( 'cms_pages' );
    ...
    public static function create( array $data ): ?Page {
        ...
        if ( function_exists( 'metis_entity_id_service' ) ) {
            $insert = \metis_entity_id_service()->assignForInsert( 'cms_page', $insert );
        } else {
            $insert['page_code'] = \metis_generate_code( 'CMP', $table, 'page_code' );
        }
        ...
        if ( function_exists( 'metis_entity_id_service' ) ) {
            \metis_entity_id_service()->register( 'cms_page', $new_id, (string) ( $insert['page_code'] ?? '' ) );
        }
    ...
    public static function delete( int $id ): bool {
        $deleted = (bool) self::db()->delete( \Metis_Tables::get( 'cms_pages' ), [ 'id' => $id ] );
        if ( $deleted ) {
    ...
    private static function hasColumn( string $column ): bool {
        if ( self::$columnMap === null ) {
            $rows = self::db()->fetchAll( 'SHOW COLUMNS FROM ' . \Metis_Tables::get( 'cms_pages' ) );
            self::$columnMap = [];
