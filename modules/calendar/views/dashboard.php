...
<div class="metis-calendar"
    ...
    <h1 class="mw-page-title"><?php echo metis_escape_html( metis_current_module_label( 'Calendar' ) ); ?></h1>
    <p class="mw-subtitle">Manage organization events from Metis using the configured Google Calendar.</p>
    <div id="metis-calendar-alert" class="mw-alert" role="status" aria-live="polite" style="display:none;"></div>

    <?php if (empty($workspace['ok'])) : ?>
        <div class="mw-alert mw-alert-error"><?php echo metis_escape_html('Calendar integration is not configured.'); ?></div>
    <?php else : ?>
        <div class="mw-stats-row metis-calendar-stats">
            <article class="mw-stat-card metis-calendar-stat">
                <div class="mw-stat-label metis-calendar-stat-label">Calendars</div>
                <div class="mw-stat-value metis-calendar-stat-value" id="metis-calendar-name"><?php echo metis_escape_html(count($calendar_rows) === 1 ? (string) ($calendar_rows[0]['calendar_label'] ?? 'Calendar') : ('All ' . count($calendar_rows))); ?></div>
                <div class="mw-stat-sub mw-muted" id="metis-calendar-id"><?php echo metis_escape_html(count($calendar_rows) === 1 ? ((string) ($calendar_rows[0]['calendar_name'] ?? '')) : 'Combined calendar view'); ?></div>
            </article>
            <article class="mw-stat-card metis-calendar-stat"><div class="mw-stat-label metis-calendar-stat-label">Visible Events</div><div class="mw-stat-value metis-calendar-stat-value" id="metis-calendar-count">0</div></article>
            <article class="mw-stat-card metis-calendar-stat"><div class="mw-stat-label metis-calendar-stat-label">Upcoming (7d)</div><div class="mw-stat-value metis-calendar-stat-value" id="metis-calendar-upcoming">0</div></article>
        </div>
        ...
        <div class="metis-contacts-toolbar">
            <div class="metis-contacts-toolbar-left metis-calendar-toolbar-left">
                ...
                <div class="metis-calendar-view-switch" role="tablist" aria-label="Calendar Views">
                    <button type="button" id="metis-calendar-view-month" class="mw-btn mw-btn-ghost is-active" role="tab" aria-selected="true" aria-controls="metis-calendar-grid" tabindex="0">Month</button>
                    <button type="button" id="metis-calendar-view-week" class="mw-btn mw-btn-ghost" role="tab" aria-selected="false" aria-controls="metis-calendar-timeview" tabindex="-1">Week</button>
                    <button type="button" id="metis-calendar-view-day" class="mw-btn mw-btn-ghost" role="tab" aria-selected="false" aria-controls="metis-calendar-timeview" tabindex="-1">Day</button>
                </div>
...
<div class="metis-contacts-modal" id="metis-calendar-detail-modal" aria-hidden="true">
    <div class="metis-contacts-modal-inner metis-calendar-detail-modal-inner" role="dialog" aria-modal="true" aria-labelledby="metis-calendar-detail-title">
...
<div class="metis-contacts-modal" id="metis-calendar-event-modal" aria-hidden="true">
    <div class="metis-contacts-modal-inner" style="max-width:760px;" role="dialog" aria-modal="true" aria-labelledby="metis-calendar-event-title">
