...
$required_module_slugs = is_array( $module_catalog['required_modules'] ?? null )
    ...
    : [];
$official_module_slugs = is_array( $module_catalog['official_modules'] ?? null )
    ? (array) $module_catalog['official_modules']
    : [];
...
<p class="mw-subtitle">Review the installed Metis version and current release state.</p>
...
<div class="mw-settings-card">
    <div class="mw-settings-header">
        <h2>Metis Version</h2>
        <span class="mw-settings-status is-ok"><?php echo metis_escape_html( (string) ( $system_version['metis_version'] ?? 'unknown' ) ); ?></span>
    </div>
    <div class="mw-settings-body">
        <div class="mw-shortcode-wrap" style="align-items:flex-start; gap:16px; flex-wrap:wrap;">
            <div>
                ...
                <div class="mw-help"><?php echo metis_escape_html( (string) ( $system_version['metis_version'] ?? 'unknown' ) ); ?></div>
            </div>
            <div>
                <div><strong>Build Stamp</strong></div>
                <div class="mw-help"><?php echo metis_escape_html( (string) ( $system_version['build'] ?? 'unknown' ) ); ?></div>
            </div>
            <div>
            ...
            <?php if ( $repository_head !== '' ) : ?>
                <div>
                    ...
                    <div class="mw-help"><code><?php echo metis_escape_html( $repository_head ); ?></code></div>
                </div>
            <?php endif; ?>
        </div>
    </div>
...
<?php if ( $module_details !== [] ) : ?>
    <div class="mw-settings-card">
        <div class="mw-settings-header"><h2>Module Versions</h2></div>
        <div class="mw-settings-body">
            <div style="display:grid; grid-template-columns:repeat(auto-fit, minmax(180px, 1fr)); gap:12px;">
                <?php foreach ( $module_details as $module_detail ) : ?>
                    ...
                    $module_badge = 'Custom';
                    if ( ! empty( $required_module_lookup[ $module_slug ] ) || ! empty( $modules[ $module_slug ]['config']['required'] ) ) {
                        $module_badge = 'Required';
                    } elseif ( ! empty( $official_module_lookup[ $module_slug ] ) ) {
                        $module_badge = 'Official';
                    }
                    ...
                    ?>
                    <div style="padding:14px 16px; border:1px solid #d9deea; border-radius:12px; background:#f7f8fc;">
                        <div style="font-weight:700; color:#1f2330;">
                            <?php echo metis_escape_html( ucwords( str_replace( [ '_', '-' ], ' ', (string) $module_slug ) ) ); ?>
                        </div>
                        <div style="display:flex; align-items:center; justify-content:space-between; gap:12px; margin-top:8px;">
                            <div class="mw-help" style="margin:0;"><code><?php echo metis_escape_html( (string) $module_version ); ?></code></div>
                            <div style="display:flex; align-items:center; gap:6px; flex-wrap:wrap; justify-content:flex-end;">
                                ...
                                <span class="mw-chip" style="flex-shrink:0; <?php echo metis_escape_attr( $module_badge_style ); ?>"><?php echo metis_escape_html( $module_badge ); ?></span>
                            </div>
                        </div>
                    </div>
                <?php endforeach; ?>
...
<div class="mw-settings-card">
    <div class="mw-settings-header"><h2>Release Status</h2></div>
    <div class="mw-settings-body">
        <div class="mw-field">
            <div class="mw-shortcode-wrap" style="align-items:flex-start; gap:16px; flex-wrap:wrap;">
                <div>
                    ...
                    <div class="mw-help">Version <?php echo metis_escape_html( (string) ( $release_current['version'] ?? $release_status['installed_version'] ?? ( $system_version['metis_version'] ?? 'unknown' ) ) ); ?></div>
                </div>
                <div>
                ...
                <div>
                    ...
                    ?></code></div>
                    <div class="mw-help">Last check: <?php echo metis_escape_html( (string) ( $release_status['last_checked_at'] ?? 'never' ) ); ?></div>
                </div>
            </div>
            <?php if ( ! empty( $release_status['remote_error'] ) ) : ?>
                <p class="mw-help" style="color:#b91c1c;"><?php echo metis_escape_html( (string) $release_status['remote_error'] ); ?></p>
            <?php else : ?>
                <p class="mw-help">Release checks trust semantic Git tags and cache the results locally.</p>
            <?php endif; ?>
        </div>
        <div class="mw-field">
            ...
            <p class="mw-help">Release checks use semantic Git tags discovered from the local repository and the configured remote source.</p>
        </div>
    </div>
</div>
...
<?php if ( $is_system_admin ) : ?>
    <div class="mw-settings-actions">
        <button type="button" class="mw-btn" data-release-check-updates="1">Refresh Releases</button>
        ...
        <?php if ( $release_can_rollback ) : ?>
            <button type="button" class="mw-btn mw-btn-secondary" data-release-rollback="1">Rollback</button>
        <?php endif; ?>
    </div>
<?php endif; ?>
