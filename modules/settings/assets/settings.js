document.addEventListener('DOMContentLoaded', function () {
    ...
    function initSettingsMediaPicker() {
        ...
        modal.innerHTML = [
            ...
            '    <button type="button" class="mw-btn mw-btn-ghost mw-btn-sm" data-settings-media-close>Close</button>',
            '  </div>',
            '  <div class="metis-settings-media-modal__toolbar">',
            '    <input type="text" class="mw-input" placeholder="Search by filename" data-settings-media-search>',
            '    <button type="button" class="mw-btn mw-btn-ghost mw-btn-sm" data-settings-media-refresh>Refresh</button>',
            '  </div>',
    ...
    (function initMenuEditor() {
        ...
        const ensureItemControls = function (item) {
            ...
            if (fields && !fields.querySelector('[data-menu-parent-select]')) {
                ...
                row.innerHTML = '<label>Parent</label><select class="mw-input" data-menu-parent-select></select>';
            ...
            item.innerHTML =
                '<div class="metis-menu-item-card">' +
                    ...
                    '<div class="metis-menu-item-fields">' +
                        '<div class="metis-menu-field-row"><label>Label</label><input class="mw-input" type="text" data-menu-label value="' + Metis.util.escapeHtml(label) + '"></div>' +
                        '<div class="metis-menu-field-row"><label>Icon</label><input type="hidden" data-menu-icon value="' + Metis.util.escapeHtml(icon) + '"><div class="metis-menu-icon-picker-row"><div class="metis-menu-icon-preview" data-menu-icon-preview></div><button type="button" class="mw-btn mw-btn-secondary mw-btn-sm" data-menu-icon-picker-toggle>Choose Icon</button><button type="button" class="mw-btn mw-btn-secondary mw-btn-sm" data-menu-icon-clear>Clear</button><span class="metis-menu-icon-token" data-menu-icon-token></span></div></div>' +
    ...
    function renderBackupHistoryRows(runs) {
        if (!Array.isArray(runs) || !runs.length) {
            return '<tr><td colspan="6"><span class="mw-help">No backup runs have been recorded yet.</span></td></tr>';
        }
        ...
        return runs.map(function (run) {
            ...
            const driveCell = fullLink
                ...
                : (driveFolderId ? '<code>' + escapeHtml(driveFolderId) + '</code>' : '<span class="mw-help">Not uploaded</span>');

            const restoreButton = canRestore
                ? '<button type="button" class="mw-btn mw-btn-xs mw-btn-ghost" data-backup-restore-run="' + escapeHtml(runUuid) + '">Restore</button>'
                : '<button type="button" class="mw-btn mw-btn-xs mw-btn-ghost" disabled>Restore</button>';

            const errorNote = lastError ? '<div class="mw-help" style="margin-top:6px;color:#b91c1c;">Backup run failed. Review logs for details.</div>' : '';
    ...
    function loadBackupHistory() {
        ...
        return Metis.request.postForm(window.metisAjax || null, action, body, 'Settings AJAX not configured.').then(function (data) {
            ...
            backupHistoryBody.innerHTML = '<tr><td colspan="6"><span class="mw-help" style="color:#b91c1c;">' + escapeHtml(message) + '</span></td></tr>';
            if (backupHistoryStatusEl) {
    ...
    function renderSchedulerHistoryRows(jobs) {
        ...
        return jobs.map(function (row) {
            ...
            const jobDetail = failed
                ? '<div class="mw-help" style="color:#b91c1c;">Job failed. Review logs for details.</div>'
                : '<span class="mw-help">' + escapeHtml(queueName) + ' queue</span>';

            return [
                '<div class="mw-premium-row">',
                '  <div class="mw-premium-cell"><div class="metis-scheduler-history-task"><strong>' + escapeHtml(taskLabel) + '</strong><code>' + escapeHtml(jobCode) + '</code></div></div>',
                '  <div class="mw-premium-cell">' + escapeHtml(association) + '</div>',
                '  <div class="mw-premium-cell"><span class="metis-status-chip is-' + safeStatus + '">' + escapeHtml(ucfirst(status || 'unknown')) + '</span></div>',
                '  <div class="mw-premium-cell">' + escapeHtml(started) + '</div>',
                '  <div class="mw-premium-cell">' + escapeHtml(finished) + '</div>',
                '  <div class="mw-premium-cell">' + jobDetail + '</div>',
                '</div>'
            ].join('');
        }).join('');
    }
    ...
    if (checkerRoot) {
        ...
        const renderCheckerRows = function (checks) {
            if (!Array.isArray(checks) || checks.length === 0) {
                return '<div class="mw-premium-row"><div class="mw-premium-cell">No checks available.</div></div>';
            }
            ...
            return checks.map(function (check) {
                ...
                return [
                    '<div class="mw-premium-row is-' + escapeHtml(status) + '" data-checker-status="' + escapeHtml(status) + '">',
                    '  <div class="mw-premium-cell"><strong class="metis-checker-status metis-checker-status-' + escapeHtml(status) + '">' + escapeHtml(status.toUpperCase()) + '</strong></div>',
                    '  <div class="mw-premium-cell">' + escapeHtml(ucfirst(category)) + '</div>',
                    '  <div class="mw-premium-cell">' + escapeHtml(title) + '</div>',
                    '  <div class="mw-premium-cell">' + escapeHtml(message) + '</div>',
                    '  <div class="mw-premium-cell">' + escapeHtml(recommendation || '-') + '</div>',
                    '</div>'
                ].join('');
            }).join('');
        };
        ...
        const applyCheckerFilter = function () {
            if (!tableEl) return;
            const rows = Array.prototype.slice.call(tableEl.querySelectorAll('.mw-premium-row[data-checker-status]'));
            let visibleCount = 0;
            ...
            if (visibleCount === 0) {
                if (!emptyRow) {
                    ...
                    emptyRow.innerHTML = '<div class="mw-premium-cell">No checks in this tab.</div>';
        ...
        const renderCheckerReport = function (report) {
            ...
            if (tableEl) {
                const header = tableEl.querySelector('.mw-premium-header');
                tableEl.innerHTML = '';
                if (header) {
                ...
                tableEl.insertAdjacentHTML('beforeend', renderCheckerRows(checks));
            }
