function initMetisFinanceApp(context) {
        ...
        reviewQueueBody.innerHTML = state.reconReviewQueue.map(function (row) {
            ...
            if (status === 'pending_confirmation') {
                actionCell = [
                    ...
                    '  <button type="button" class="mw-btn mw-btn-xs" data-recon-review-action="approve" data-recon-review-id="' + esc(row.id) + '">Approve</button>',
                    '  <button type="button" class="mw-btn mw-btn-xs mw-btn-ghost" data-recon-review-action="manual_confirmed" data-recon-review-id="' + esc(row.id) + '">Adjust</button>',
                    '  <button type="button" class="mw-btn mw-btn-xs mw-btn-ghost" data-recon-review-action="reject" data-recon-review-id="' + esc(row.id) + '">Reject</button>',
                    '</div>'
                ].join('');
            }
    ...
    function renderReconciliationLines(lines, suggestions) {
        ...
        reconLinesBody.innerHTML = rows.map(function (row) {
            ...
            if (suggestion && suggestion.suggested_type && Number(suggestion.suggested_id || 0) > 0) {
                actionCell = '<button type="button" class="mw-btn mw-btn-xs" data-recon-line-match="1" data-bank-line-id="' + esc(row.id) + '" data-match-type="' + esc(suggestion.suggested_type) + '" data-match-id="' + esc(suggestion.suggested_id) + '">Match</button>';
            }
            ...
            if (!history.length) {
                ...
                reconHistoryBody.innerHTML = history.map(function (row) {
                    ...
                    var statementCell = row.statement_media_url
                        ? '<a class="mw-btn mw-btn-xs mw-btn-ghost" href="' + esc(row.statement_media_url) + '" target="_blank" rel="noopener">Open</a>'
                        : '-';
                    var monthCell = '<button type="button" class="mw-btn mw-btn-xs mw-btn-ghost" data-recon-month-select="' + esc(monthId) + '">' + esc(monthLabel(String(row.recon_month || '').slice(0, 7))) + '</button>';
                    var openCell = currentSection === 'reconciliation' && reconStepsUrl
                        ? '<button type="button" class="mw-btn mw-btn-xs" data-recon-open-steps="' + esc(monthId) + '">Open Workflow</button>'
                        : '<button type="button" class="mw-btn mw-btn-xs mw-btn-ghost" data-recon-month-select="' + esc(monthId) + '">Select</button>';
                    if (currentSection === 'reconciliation' && row && row.can_delete) {
                        openCell += ' <button type="button" class="mw-btn mw-btn-xs mw-btn-ghost" data-recon-delete-month="' + esc(monthId) + '">Delete</button>';
                    }
                    return [
                        '<tr' + active + '>',
                        '  <td>' + monthCell + '</td>',
    ...
    function renderBudget(snapshot) {
        ...
        budgetLinesBody.innerHTML = lines.map(function (row) {
            ...
            var plannedCell = canEdit
                ? '<input type="number" step="0.01" class="mw-input metis-budget-line-input" data-budget-line-account="' + esc(row.account_code || '') + '" value="' + esc(planned) + '">'
    ...
    function renderInvoices(snapshot) {
        ...
        invoicesBody.innerHTML = rows.map(function (row) {
            ...
            if (status !== 'paid') {
                var actions = [];
                if (status === 'draft') {
                    actions.push('<button type="button" class="mw-btn mw-btn-xs" data-invoice-send="' + esc(row.id) + '">Send</button>');
                }
                if (status === 'sent' || status === 'overdue' || status === 'draft') {
                    actions.push('<button type="button" class="mw-btn mw-btn-xs mw-btn-ghost" data-invoice-paid="' + esc(row.id) + '">Mark Paid</button>');
                }
        ...
        payoutsBody.innerHTML = state.payouts.map(function (row) {
            ...
            if (status !== 'matched') {
                ...
                matchCell = [
                    ...
                    '  <select class="mw-input" data-match-bank-line-select="' + esc(row.id) + '">' + options.join('') + '</select>',
                    '  <button type="button" class="mw-btn mw-btn-xs" data-match-payout-btn="' + esc(row.id) + '">Match</button>',
                    '</div>'
                ].join('');
    ...
    if (glForm) {
        function glRowTemplate() {
            ...
            return [
                '<tr data-gl-row="1">',
                '  <td><input type="date" class="mw-input" name="entry_date" value="' + esc(dateDefault) + '" required></td>',
                '  <td><select class="mw-input" name="account_code" required>' + accountOptions + '</select></td>',
                '  <td><input type="text" class="mw-input" name="description" maxlength="255" required></td>',
                '  <td><input type="number" class="mw-input" name="amount" step="0.01" required></td>',
                '  <td><select class="mw-input" name="dc_type"><option value="debit">Debit</option><option value="credit">Credit</option></select></td>',
                '  <td><select class="mw-input" name="category_code">' + categoryOptions + '</select></td>',
                '  <td><button type="button" class="mw-btn mw-btn-xs mw-btn-ghost" data-gl-remove-row="1">Remove</button></td>',
                '</tr>'
            ].join('');
        }
