...
<div class="metis-forms-page">
    <header class="metis-forms-hero">
        <div>
            ...
            <h1 class="mw-page-title"><?php echo metis_escape_html( metis_current_module_label( 'Forms' ) ); ?></h1>
            <p class="mw-subtitle">Build forms, route follow-up, collect payments only when a form needs them, and keep the publishing flow predictable.</p>
        </div>
        <?php if ( metis_forms_can_manage() ) : ?>
            <a class="mw-btn" href="<?php echo metis_escape_url( metis_forms_build_url() ); ?>">Create form</a>
        <?php endif; ?>
    </header>
    ...
    <?php if ( empty( $forms ) ) : ?>
        <section class="metis-forms-empty-card">
            ...
            <?php if ( metis_forms_can_manage() ) : ?>
                <a class="mw-btn" href="<?php echo metis_escape_url( metis_forms_build_url() ); ?>">Create your first form</a>
            <?php endif; ?>
        </section>
    <?php else : ?>
            ...
            <?php foreach ( $forms as $form ) : ?>
                <article class="metis-forms-card">
                    ...
                    <div class="metis-forms-card__actions">
                        <a class="mw-btn mw-btn-xs mw-btn-ghost" href="<?php echo metis_escape_url( metis_forms_detail_url( (int) $form['id'] ) ); ?>">Overview</a>
                        <a class="mw-btn mw-btn-xs mw-btn-ghost" href="<?php echo metis_escape_url( metis_forms_entries_url( (int) $form['id'] ) ); ?>">Entries</a>
                        <a class="mw-btn mw-btn-xs" href="<?php echo metis_escape_url( metis_forms_build_url( (int) $form['id'] ) ); ?>">Open builder</a>
                    </div>
                </article>
            <?php endforeach; ?>
        </section>
