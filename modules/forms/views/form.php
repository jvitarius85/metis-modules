...
if ( ! is_array( $form ) ) {
    echo '<div class="mw-alert mw-alert-error">That form could not be found.</div>';
    return;
}
...
<div class="metis-forms-page">
    <header class="metis-forms-hero">
        <div>
            <a class="mw-btn mw-btn-xs mw-btn-ghost" href="<?php echo metis_escape_url( metis_forms_base_url() ); ?>">Back to Forms</a>
            <h1 class="mw-page-title"><?php echo metis_escape_html( (string) ( $form['name'] ?? '' ) ); ?></h1>
            <p class="mw-subtitle"><?php echo metis_escape_html( (string) ( $form['description'] ?? '' ) ); ?></p>
        </div>
        <div class="metis-forms-hero__actions">
            <a class="mw-btn mw-btn-xs mw-btn-ghost" href="<?php echo metis_escape_url( metis_forms_entries_url( $form_id ) ); ?>">Entries</a>
            <a class="mw-btn" href="<?php echo metis_escape_url( metis_forms_build_url( $form_id ) ); ?>">Open builder</a>
        </div>
    </header>
    ...
    <div class="metis-forms-two-column">
        ...
        <section class="metis-forms-card">
            ...
            <?php if ( empty( $versions ) ) : ?>
                <p class="mw-muted">No versions yet.</p>
            <?php else : ?>
