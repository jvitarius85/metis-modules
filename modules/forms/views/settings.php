...
<div class="metis-forms-page">
    <header class="metis-forms-hero">
        <div>
            <a class="mw-btn mw-btn-xs mw-btn-ghost" href="<?php echo metis_escape_url( metis_forms_detail_url( $form_id ) ); ?>">Back to Overview</a>
            <h1 class="mw-page-title"><?php echo metis_escape_html( (string) ( $form['name'] ?? '' ) ); ?> Settings</h1>
            <p class="mw-subtitle">This page summarizes the current settings. Edit them in the builder so the build, settings, and publish flow stay in one place.</p>
        </div>
        <div class="metis-forms-hero__actions">
            <a class="mw-btn" href="<?php echo metis_escape_url( metis_forms_build_url( $form_id ) . '?step=settings' ); ?>">Open settings in builder</a>
        </div>
    </header>
