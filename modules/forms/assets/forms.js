(function () {
  ...
  function initBuilder(root, boot) {
    ...
    root.addEventListener('click', async (event) => {
      ...
      if (event.target.closest('#metis-forms-delete') && state.current?.id && canDelete) {
        const confirmed = await confirmAction(`Delete ${state.current.name}? This removes versions and submissions.`, {
          title: 'Delete Form',
    ...
    // Immediately persist depends_on to state when changed in inspector
    // so re-opening the same field retains the selected parent.
    ui.fieldDependsOn?.addEventListener('change', () => {
      const field = selectedField();
      if (!field) return;
      field.depends_on = ui.fieldDependsOn.value || '';
    ...
    ui.canvas?.addEventListener('contextmenu', async (event) => {
      const card = event.target.closest('[data-field-id]');
      if (!card || !canManage) return;
      event.preventDefault();
      ...
      if (!field) return;
      const confirmed = await confirmAction(`Delete "${field.label || field.key}"?`, {
        title: 'Delete Field',
  ...
  function setupChainedSelects(form) {
    qsa(form, '.metis-form-field[data-field-type="select"][data-depends-on]').forEach((fieldNode) => {
      ...
      // Prefer the actual <select>; searchable fields also render a text <input> with same name
      const parent = qs(form, `select[name="${cssEscape(parentKey)}"]`) || qs(form, `[name="${cssEscape(parentKey)}"]`);
