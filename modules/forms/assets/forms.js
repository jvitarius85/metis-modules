(function () {
    ...
    return `
      <div class="metis-forms-admin">
        <header class="metis-forms-admin__header">
          <div>
            <a class="mw-btn mw-btn-xs mw-btn-ghost" href="${escapeAttr(String(boot.urls?.home || '/forms/'))}">Back to Forms</a>
            <h1 class="mw-page-title" data-ui="title">${title}</h1>
          </div>
          <div class="metis-forms-admin__actions">
            ${state.form.id ? `<button type="button" class="mw-btn mw-btn-xs mw-btn-ghost" data-duplicate-form ${!canManage ? 'disabled' : ''}>Duplicate</button>` : ''}
            <button type="button" class="mw-btn mw-btn-xs mw-btn-ghost" data-publish-form ${!canManage ? 'disabled' : ''}>Publish</button>
            <button type="button" class="mw-btn mw-btn-xs" data-save-form ${!canManage ? 'disabled' : ''}>Save</button>
            ${state.form.id && canDelete ? `<button type="button" class="mw-btn mw-btn-xs mw-btn-ghost" data-delete-form>Delete</button>` : ''}
          </div>
        </header>
          ...
          <div class="metis-forms-builder-grid">
            <section class="metis-forms-surface">
              <div class="metis-forms-surface__head">
                <h2>Fields</h2>
                <button type="button" class="mw-btn mw-btn-xs" data-open-picker ${!canManage ? 'disabled' : ''}>Add field</button>
              </div>
            ...
            <section class="metis-forms-surface">
              <div class="metis-forms-surface__head">
                ...
                ${currentField ? `<button type="button" class="mw-btn mw-btn-xs mw-btn-ghost" data-remove-selected ${!canManage ? 'disabled' : ''}>Remove</button>` : ''}
              </div>
      ...
      return `
        <article class="metis-forms-field-card${selected ? ' is-selected' : ''}">
          ...
          <div class="metis-forms-field-card__tail">
            ...
            <button type="button" class="mw-btn mw-btn-xs mw-btn-ghost" data-toggle-width="field:${escapeAttr(field.id)}">${escapeHtml(widthLabel(field.width))}</button>
          </div>
        </article>
      `;
    }).join('');
  ...
  function renderEditorHtml(state, options) {
    ...
    const header = node.kind === 'subfield'
      ? `<div class="metis-forms-editor-note"><button type="button" class="mw-btn mw-btn-xs mw-btn-ghost" data-back-to-repeater>Back to repeater</button><span>Editing subfield inside ${escapeHtml(node.parent.label || 'Repeater')}</span></div>`
      : '';
    ...
    return `
      <div class="metis-forms-editor">
        ...
        <div class="metis-forms-editor-tools">
          <button type="button" class="mw-btn mw-btn-xs mw-btn-ghost" data-move-node="${escapeAttr(selectionKey(node))}" data-move-delta="-1">Move earlier</button>
          <button type="button" class="mw-btn mw-btn-xs mw-btn-ghost" data-move-node="${escapeAttr(selectionKey(node))}" data-move-delta="1">Move later</button>
        </div>
    ...
    return `
      <div class="metis-forms-grid">
        <label class="metis-forms-control">
          <span>Type</span>
          <select class="mw-select" data-field-prop="type">
            ${typeOptions.map((type) => `<option value="${escapeAttr(type)}"${field.type === type ? ' selected' : ''}>${escapeHtml(fieldLabel(type))}</option>`).join('')}
          </select>
        </label>
        <label class="metis-forms-control">
          <span>Width</span>
          <select class="mw-select" data-field-prop="width" ${field.type === 'payment' ? 'disabled' : ''}>
        ...
        <label class="metis-forms-control">
          <span>Label</span>
          <input class="mw-input" type="text" value="${escapeAttr(field.label || '')}" data-field-prop="label">
        </label>
        <label class="metis-forms-control">
          <span>Key</span>
          <input class="mw-input" type="text" value="${escapeAttr(field.key || '')}" data-field-prop="key">
        </label>
        <label class="metis-forms-control metis-forms-control--full">
          <span>Help text</span>
          <textarea class="mw-input" rows="3" data-field-prop="help">${escapeHtml(field.help || '')}</textarea>
        </label>
        ${fieldSupportsPlaceholder(field.type) ? `
          <label class="metis-forms-control metis-forms-control--full">
            <span>Placeholder</span>
            <input class="mw-input" type="text" value="${escapeAttr(field.placeholder || '')}" data-field-prop="placeholder">
          </label>
        ` : ''}
        ${field.type === 'number' ? `
          <label class="metis-forms-control">
            <span>Minimum value</span>
            <input class="mw-input" type="number" step="0.01" value="${field.validation.min_value ?? ''}" data-field-validation="min_value">
          </label>
          <label class="metis-forms-control">
            <span>Maximum value</span>
            <input class="mw-input" type="number" step="0.01" value="${field.validation.max_value ?? ''}" data-field-validation="max_value">
          </label>
        ` : ''}
        ${field.type !== 'payment' ? `
    ...
    return `
      <div class="metis-forms-grid">
        <label class="metis-forms-control">
          ...
          <select class="mw-select" data-field-source="type">
        ...
        <label class="metis-forms-control">
          ...
          <select class="mw-select" data-field-source="parent_field">
        ...
        ${sourceType === 'static' || sourceType === '' ? `
          <label class="metis-forms-control metis-forms-control--full">
            <span>Options</span>
            <textarea class="mw-input" rows="8" data-field-source="options_text">${escapeHtml(toOptionsText(field.options || []))}</textarea>
          </label>
        ` : `
  ...
  function renderVisibilitySection(field, node) {
    ...
    return `
      <div class="metis-forms-editor-tools">
        <button type="button" class="mw-btn mw-btn-xs" data-add-condition>Add rule</button>
      </div>
        ...
        <div class="metis-forms-condition-list">
          ${conditions.map((condition, index) => `
            <div class="metis-forms-condition-row">
              <select class="mw-select" data-condition-field="1" data-condition-index="${index}">
              ...
              <select class="mw-select" data-condition-operator="1" data-condition-index="${index}">
              ...
              <input class="mw-input" type="text" value="${escapeAttr(Array.isArray(condition.value) ? condition.value.join(', ') : String(condition.value || ''))}" data-condition-value="1" data-condition-index="${index}">
              <button type="button" class="mw-btn mw-btn-xs mw-btn-ghost" data-remove-condition="${index}">Remove</button>
            </div>
          `).join('')}
        </div>
      `}
    ...
    return `
      <div class="metis-forms-grid">
        <label class="metis-forms-control">
          ...
          <input class="mw-input" type="number" min="1" max="25" value="${escapeAttr(String(field.repeat_limit || 5))}" data-field-prop="repeat_limit">
        </label>
      </div>
      <div class="metis-forms-editor-tools">
        <button type="button" class="mw-btn mw-btn-xs" data-add-subfield>Add subfield</button>
      </div>
            ...
            return `
              <article class="metis-forms-subfield-card${selected ? ' is-selected' : ''}">
                ...
                <div class="metis-forms-field-card__tail">
                  ...
                  <button type="button" class="mw-btn mw-btn-xs mw-btn-ghost" data-toggle-width="subfield:${escapeAttr(field.id)}:${escapeAttr(subfield.id)}">${escapeHtml(widthLabel(subfield.width))}</button>
                </div>
              </article>
            `;
          }).join('')}
  ...
  function renderPaymentSection(field, options) {
    ...
    return `
      <div class="metis-forms-grid">
        <label class="metis-forms-control">
          <span>Campaign</span>
          <select class="mw-select" data-payment-prop="campaign_code">
        ...
        <label class="metis-forms-control">
          ...
          <input class="mw-input" type="text" value="${escapeAttr((payment.donation_amounts || []).join(', '))}" data-payment-prop="donation_amounts">
        </label>
        <label class="metis-forms-check">
        ...
        <label class="metis-forms-control">
          ...
          <input class="mw-input" type="text" value="${escapeAttr(payment.custom_amount_label || 'Other amount')}" data-payment-prop="custom_amount_label">
        </label>
        <label class="metis-forms-check">
        ...
        <label class="metis-forms-control">
          ...
          <input class="mw-input" type="text" value="${escapeAttr(payment.cover_fees_label || '')}" data-payment-prop="cover_fees_label">
        </label>
        <label class="metis-forms-control">
          <span>Total label</span>
          <input class="mw-input" type="text" value="${escapeAttr(payment.summary_label || 'Total')}" data-payment-prop="summary_label">
        </label>
        <label class="metis-forms-control metis-forms-control--full">
          <span>Success message</span>
          <textarea class="mw-input" rows="4" data-payment-prop="success_message">${escapeHtml(payment.success_message || '')}</textarea>
        </label>
      </div>
    `;
  }
      ...
      <div class="metis-forms-settings-grid">
        <section class="metis-forms-surface">
          ...
          <div class="metis-forms-grid">
            <label class="metis-forms-control">
              <span>Form name</span>
              <input class="mw-input" type="text" value="${escapeAttr(form.name || '')}" data-form-prop="name">
            </label>
            <label class="metis-forms-control">
              <span>Public slug</span>
              <input class="mw-input" type="text" value="${escapeAttr(form.slug || '')}" data-form-prop="slug">
            </label>
            <label class="metis-forms-control metis-forms-control--full">
              <span>Description</span>
              <textarea class="mw-input" rows="4" data-form-prop="description">${escapeHtml(form.description || '')}</textarea>
            </label>
          </div>
        </section>
        ...
        <section class="metis-forms-surface">
          ...
          <div class="metis-forms-grid">
            <label class="metis-forms-control">
              ...
              <select class="mw-select" data-setting-path="binding.module">
            ...
            <label class="metis-forms-control">
              ...
              <select class="mw-select" data-setting-path="binding.flow">
        ...
        <section class="metis-forms-surface">
          ...
          <div class="metis-forms-grid">
            <label class="metis-forms-control">
              ...
              <select class="mw-select" data-setting-path="access.mode">
            ...
            ${access.mode === 'password' ? `
              <label class="metis-forms-control">
                <span>Password</span>
                <input class="mw-input" type="text" value="${escapeAttr(access.password || '')}" data-setting-path="access.password">
              </label>
            ` : ''}
            <label class="metis-forms-control metis-forms-control--full">
              <span>Closed or restricted message</span>
              <textarea class="mw-input" rows="4" data-setting-path="access.denied_message">${escapeHtml(access.denied_message || '')}</textarea>
            </label>
            ${access.mode === 'role' ? `
          ...
          <div class="metis-forms-grid">
            ...
            ${schedule.enabled ? `
              <label class="metis-forms-control">
                ...
                <input class="mw-input" type="datetime-local" value="${escapeAttr(schedule.start_at || '')}" data-setting-path="schedule.start_at">
              </label>
              <label class="metis-forms-control">
                <span>End date</span>
                <input class="mw-input" type="datetime-local" value="${escapeAttr(schedule.end_at || '')}" data-setting-path="schedule.end_at">
              </label>
              <label class="metis-forms-control metis-forms-control--full">
                <span>Scheduled closed message</span>
                <textarea class="mw-input" rows="4" data-setting-path="schedule.closed_message">${escapeHtml(schedule.closed_message || '')}</textarea>
              </label>
            ` : ''}
          </div>
        </section>
        ...
        <section class="metis-forms-surface">
          ...
          <div class="metis-forms-grid">
            ...
            <label class="metis-forms-control">
              ...
              <select class="mw-select" data-setting-path="notifications.submitter.recipient_field">
            ...
            <label class="metis-forms-control">
              <span>From name</span>
              <input class="mw-input" type="text" value="${escapeAttr(submitter.from_name || '')}" data-setting-path="notifications.submitter.from_name">
            </label>
            <label class="metis-forms-control">
              <span>From email</span>
              <input class="mw-input" type="email" value="${escapeAttr(submitter.from_email || '')}" data-setting-path="notifications.submitter.from_email">
            </label>
            <label class="metis-forms-control">
              <span>Subject</span>
              <input class="mw-input" type="text" value="${escapeAttr(submitter.subject || '')}" data-setting-path="notifications.submitter.subject">
            </label>
            <label class="metis-forms-check metis-forms-control--full">
        ...
        <section class="metis-forms-surface">
          ...
          <div class="metis-forms-grid">
            ...
            <label class="metis-forms-control">
              ...
              <input class="mw-input" type="email" value="${escapeAttr(internal.general_email || '')}" data-setting-path="notifications.internal.general_email">
            </label>
            <label class="metis-forms-control">
              <span>From name</span>
              <input class="mw-input" type="text" value="${escapeAttr(internal.from_name || '')}" data-setting-path="notifications.internal.from_name">
            </label>
            <label class="metis-forms-control">
              <span>From email</span>
              <input class="mw-input" type="email" value="${escapeAttr(internal.from_email || '')}" data-setting-path="notifications.internal.from_email">
            </label>
            <label class="metis-forms-control">
              <span>Routing field</span>
              <select class="mw-select" data-setting-path="notifications.internal.routing_field">
            ...
            <label class="metis-forms-control">
              <span>Subject</span>
              <input class="mw-input" type="text" value="${escapeAttr(internal.subject || '')}" data-setting-path="notifications.internal.subject">
            </label>
            <label class="metis-forms-check metis-forms-control--full">
            ...
            <div class="metis-forms-control metis-forms-control--full">
              <div class="metis-forms-editor-tools">
                ...
                <button type="button" class="mw-btn mw-btn-xs" data-add-route>Add route</button>
              </div>
              ${Array.isArray(internal.routes) && internal.routes.length ? internal.routes.map((route, index) => `
                <div class="metis-forms-route-row">
                  <input class="mw-input" type="text" value="${escapeAttr(String(route.value || ''))}" data-route-value="1" data-route-index="${index}" placeholder="Match value">
                  <select class="mw-select" data-route-user="1" data-route-index="${index}">
                  ...
                  <button type="button" class="mw-btn mw-btn-xs mw-btn-ghost" data-remove-route="${index}">Remove</button>
                </div>
      ...
      <div class="metis-forms-two-column">
        <section class="metis-forms-surface">
          ...
          <div class="metis-forms-grid">
            <label class="metis-forms-control">
              <span>Status</span>
              <select class="mw-select" data-form-prop="status">
      ...
      <div class="metis-forms-picker${state.pickerOpen ? ' is-open' : ''}" ${state.pickerOpen ? '' : 'hidden'} data-picker-backdrop>
        <aside class="metis-forms-picker__sheet" role="dialog" aria-modal="true" aria-label="Add field">
          <div class="metis-forms-picker__head">
            <h2>Add field</h2>
            <button type="button" class="mw-btn mw-btn-xs mw-btn-ghost" data-close-picker>Close</button>
          </div>
  ...
  function renderPreviewField(field) {
      ...
      return `
        <section class="metis-forms-preview-field is-full">
          ...
          <div class="metis-forms-preview-payment">
            ...
            ${payment.allow_custom_amount ? `<input class="mw-input" type="text" value="${escapeAttr(payment.custom_amount_label || 'Other amount')}" disabled>` : ''}
    ...
    return `
      <section class="metis-forms-preview-field is-${normalizeWidthValue(field.width)}">
        <label>${escapeHtml(field.label || field.key)}</label>
        ${field.type === 'textarea'
          ? '<textarea class="mw-input" rows="3" disabled></textarea>'
          : `<input class="mw-input" type="${field.type === 'email' ? 'email' : field.type === 'number' ? 'number' : field.type === 'date' ? 'date' : 'text'}" disabled>`}
      </section>
    `;
  }
  ...
  function renderRichSettingEditor(path, html, mergeTokens) {
    ...
    return `
      <div class="metis-forms-rich" data-rich-editor-key="${escapeAttr(path)}">
        ...
        <div class="metis-forms-rich-editor mw-input" contenteditable="true" data-rich-setting-path="${escapeAttr(path)}">${sanitizeRichHtml(html)}</div>
      </div>
    `;
  }
  ...
  function showPublicAlert(node, message, kind) {
    if (!node) return;
    node.hidden = false;
    node.className = `mw-alert ${kind === 'success' ? 'mw-alert-success' : 'mw-alert-error'}`;
