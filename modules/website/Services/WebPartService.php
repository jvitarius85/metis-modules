...
use Metis\Modules\Website\Entities\WebPart;
...
final class WebPartService {
    ...
    public static function getAll( array $filters = [] ): array {
        $db = self::db();
        $table = \Metis_Tables::get( 'website_web_parts' );
        $where = [];
        $params = [];
    ...
    public static function getById( int $id ): ?WebPart {
        ...
        try {
            $row = self::db()->fetchOne(
                "SELECT * FROM " . \Metis_Tables::get( 'website_web_parts' ) . " WHERE id = %d LIMIT 1",
                [ $id ]
            );
        } catch ( \Throwable $e ) {
            return null;
    ...
    public static function create( array $data ): int|false {
        ...
        $result = self::db()->insert( \Metis_Tables::get( 'website_web_parts' ), $payload );
        if ( ! $result ) {
            return false;
        }
        ...
        if ( $new_id > 0 && function_exists( 'metis_entity_id_service' ) ) {
            try {
                \metis_entity_id_service()->register( 'website_web_part', $new_id, (string) ( $payload['part_code'] ?? '' ) );
            } catch ( \Throwable $e ) {
    ...
    public static function update( int $id, array $data ): bool {
        ...
        return (bool) self::db()->update( \Metis_Tables::get( 'website_web_parts' ), $payload, [ 'id' => $id ] );
    }
    ...
    public static function delete( int $id ): bool {
        ...
        return (bool) self::db()->delete( \Metis_Tables::get( 'website_web_parts' ), [ 'id' => $id ] );
    }
    ...
    private static function normalizePayload( array $data, bool $for_create ): array {
        ...
        if ( $for_create ) {
            if ( function_exists( 'metis_entity_id_service' ) ) {
                $payload = \metis_entity_id_service()->assignForInsert( 'website_web_part', $payload );
            } else {
                $payload['part_code'] = \metis_generate_code( 'WWP', \Metis_Tables::get( 'website_web_parts' ), 'part_code' );
            }
        }
