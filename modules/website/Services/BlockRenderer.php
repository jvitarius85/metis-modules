...
final class BlockRenderer {
    ...
    private static function renderImageCarousel( array $data, array $style, array $context ): string {
        ...
        $cinematic_animation = $id . '-cinematic';
        ...
        $html .= '<div class="metis-block-image-carousel__viewport">';
        foreach ( $normalized as $index => $slide ) {
            ...
            $html .= '<div class="metis-block-image-carousel__slide' . ( $active ? ' is-active' : '' ) . '" style="--metis-carousel-slide-duration:' . $slide['duration_ms'] . 'ms" data-duration-ms="' . $slide['duration_ms'] . '"' . ( $active ? '' : ' aria-hidden="true" inert' ) . '>' . $image . '</div>';
        }
        ...
        $html .= '</div></div>';
        $html .= '<style>#' . $id . '{position:relative;left:50%;width:100vw;max-width:100vw;margin-left:-50vw;margin-right:-50vw;box-sizing:border-box;overflow:hidden}#' . $id . ' .metis-block-image-carousel__viewport{position:relative;width:100%;aspect-ratio:1983 / 793;overflow:hidden;background:#eef2f7}#' . $id . ' .metis-block-image-carousel__slide{position:absolute;inset:0;overflow:hidden;opacity:0;pointer-events:none;transform:translateX(8%);transition:opacity ' . $transition_duration . 'ms ease,transform ' . $transition_duration . 'ms ease}#' . $id . '.is-transition-fade .metis-block-image-carousel__slide,#' . $id . '.is-transition-crossfade .metis-block-image-carousel__slide,#' . $id . '.is-transition-ken_burns .metis-block-image-carousel__slide,#' . $id . '.is-transition-cinematic .metis-block-image-carousel__slide{transform:none}#' . $id . '.is-transition-crossfade .metis-block-image-carousel__slide{transition:opacity ' . $transition_duration . 'ms cubic-bezier(.4,0,.2,1)}#' . $id . '.is-transition-reveal .metis-block-image-carousel__slide{transform:none;clip-path:inset(0 100% 0 0);transition:opacity ' . $transition_duration . 'ms ease,clip-path ' . $transition_duration . 'ms cubic-bezier(.22,1,.36,1)}#' . $id . '.is-transition-reveal .metis-block-image-carousel__slide.is-active{clip-path:inset(0)}#' . $id . '.is-transition-ken_burns .metis-block-image-carousel__slide.is-active img{animation:' . $id . '-ken-burns var(--metis-carousel-slide-duration) ease-out both}@keyframes ' . $id . '-ken-burns{from{transform:scale(1)}to{transform:scale(1.075)}}#' . $id . '.is-transition-cinematic .metis-block-image-carousel__slide{transition:opacity ' . $transition_duration . 'ms cubic-bezier(.22,1,.36,1),transform ' . $transition_duration . 'ms cubic-bezier(.22,1,.36,1),filter ' . $transition_duration . 'ms cubic-bezier(.22,1,.36,1)}#' . $id . '.is-transition-cinematic .metis-block-image-carousel__slide.is-active{animation:' . $cinematic_animation . ' ' . $transition_duration . 'ms cubic-bezier(.22,1,.36,1) both}@keyframes ' . $cinematic_animation . '{0%{filter:brightness(.94) saturate(.92);transform:scale(1.055)}100%{filter:brightness(1) saturate(1);transform:scale(1)}}#' . $id . ' .metis-block-image-carousel__slide.is-active{opacity:1;pointer-events:auto;transform:translateX(0)}#' . $id . ' img,#' . $id . ' .metis-block-image-carousel__action{display:block;width:100%;height:100%}#' . $id . ' img{object-fit:cover}#' . $id . ' .metis-block-image-carousel__action{border:0;padding:0;background:none;cursor:pointer}#' . $id . ' .metis-block-image-carousel__control{position:absolute;top:50%;z-index:2;display:flex;align-items:center;justify-content:center;transform:translateY(-50%);border:0;border-radius:999px;width:42px;height:42px;padding:0;background:rgba(15,23,42,.68);color:#fff;font-size:34px;line-height:1;cursor:pointer}#' . $id . ' .is-prev{left:14px}#' . $id . ' .is-next{right:14px}#' . $id . ' .metis-block-image-carousel__dots{position:absolute;z-index:2;bottom:14px;left:50%;transform:translateX(-50%);display:flex;gap:8px}#' . $id . ' .metis-block-image-carousel__dot{width:10px;height:10px;border:1px solid rgba(15,23,42,.7);border-radius:50%;padding:0;background:rgba(255,255,255,.55);cursor:pointer}#' . $id . ' .metis-block-image-carousel__dot.is-active{background:#fff}@media (prefers-reduced-motion:reduce){#' . $id . ' .metis-block-image-carousel__slide{transition:none;animation:none}#' . $id . '.is-transition-ken_burns .metis-block-image-carousel__slide.is-active img{animation:none}}</style>';
