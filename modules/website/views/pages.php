...
if ( $is_editor_route ) {
    ...
    (function() {
        function launchEditor() {
            if (!window.MetisPageBuilder) return false;
            var editorNew = <?php echo function_exists( 'metis_json_encode' ) ? metis_json_encode( $editor_new ) : json_encode( $editor_new ); ?>;
            var targetId = parseInt(<?php echo function_exists( 'metis_json_encode' ) ? metis_json_encode( (string) $editor_target_id ) : json_encode( (string) $editor_target_id ); ?>, 10);
            var context = <?php echo function_exists( 'metis_json_encode' ) ? metis_json_encode( $context ) : json_encode( $context ); ?>;

            if (editorNew === 'page' || editorNew === 'post') {
                window.MetisPageBuilder.openNew(context);
                return true;
            }
            if (!Number.isNaN(targetId) && targetId > 0) {
                window.MetisPageBuilder.openEdit(targetId, context);
                return true;
            }
            return false;
        }
        ...
        function onReady() {
            ...
            var timer = setInterval(function() {
                tries += 1;
                if (launchEditor() || tries >= 40) clearInterval(timer);
            }, 75);
        }
...
<div class="mw-page-header">
    <div class="mw-page-header-left">
        ...
        <p class="mw-subtitle"><?php echo esc_html( count( $pages ) ); ?> page<?php echo count( $pages ) !== 1 ? 's' : ''; ?> in website content.</p>
    </div>
    <div class="mw-page-header-right">
        ...
        <select id="metis-homepage-selector" class="mw-input mw-input-sm metis-homepage-selector">
            ...
            <?php foreach ( $pages as $page ) : ?>
                <?php if ( $page->status !== 'published' ) { continue; } ?>
                <option value="<?php echo esc_attr( (string) $page->id ); ?>"<?php echo ( $homepage_page_id !== null && (int) $homepage_page_id === (int) $page->id ) ? ' selected' : ''; ?>>
                    <?php echo esc_html( $page->title ); ?> (/<?php echo esc_html( $page->slug ); ?>)
                </option>
            <?php endforeach; ?>
        </select>
                ...
                <?php foreach ( $ordered_page_entries as $entry ) : ?>
                    ...
                    <div class="mw-premium-row">
                        <div class="mw-premium-cell">
                            <strong class="metis-edit-page metis-page-title metis-page-depth-<?php echo esc_attr( (string) $depth ); ?>" data-id="<?php echo esc_attr( (string) $page->id ); ?>" data-code="<?php echo esc_attr( (string) ( $page->page_code ?? '' ) ); ?>"><?php echo esc_html( $page->title ); ?></strong>
                        ...
                        <div class="mw-premium-cell"><code class="metis-inline-code-muted">/<?php echo esc_html( $page->slug ); ?></code></div>
                        <div class="mw-premium-cell"><span class="metis-status metis-status-<?php echo esc_attr( $page->status ); ?>"><?php echo esc_html( ucfirst( $page->status ) ); ?></span></div>
                        <div class="mw-premium-cell"><code class="metis-inline-code"><?php echo esc_html( $page->page_code ?? '—' ); ?></code></div>
                        <div class="mw-premium-cell metis-table-updated"><?php echo esc_html( $page->updated_at ? date( 'M j, Y', strtotime( $page->updated_at ) ) : '—' ); ?></div>
                        <div class="mw-premium-cell mw-col-right">
                            <div class="metis-table-actions">
                                <button class="metis-action-btn metis-edit-page" data-id="<?php echo esc_attr( (string) $page->id ); ?>" data-code="<?php echo esc_attr( (string) ( $page->page_code ?? '' ) ); ?>" title="Edit in builder">
                                    <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M11 4H4a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-7"/><path d="M18.5 2.5a2.121 2.121 0 0 1 3 3L12 15l-4 1 1-4 9.5-9.5z"/></svg>
                                </button>
                                <?php if ( $page->status === 'draft' ) : ?>
                                <button class="metis-action-btn metis-action-btn-primary metis-publish-page" data-id="<?php echo esc_attr( (string) $page->id ); ?>" title="Publish">
                                    <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><polyline points="20 6 9 17 4 12"/></svg>
                                </button>
                                <?php else : ?>
                                <button class="metis-action-btn metis-unpublish-page" data-id="<?php echo esc_attr( (string) $page->id ); ?>" title="Unpublish">
                                    <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><line x1="18" y1="6" x2="6" y2="18"/><line x1="6" y1="6" x2="18" y2="18"/></svg>
                                </button>
                                <?php endif; ?>
                                <?php if ( $page->status === 'published' ) : ?>
                                <a href="<?php echo esc_attr( ( $homepage_page_id !== null && (int) $homepage_page_id === (int) $page->id ) ? '/' : '/' . ltrim( (string) $page->slug, '/' ) ); ?>" class="metis-action-btn" title="View live" target="_blank" rel="noopener">
                                    <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M18 13v6a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2V8a2 2 0 0 1 2-2h6"/><polyline points="15 3 21 3 21 9"/><line x1="10" y1="14" x2="21" y2="3"/></svg>
                                </a>
                                <?php endif; ?>
                                <button class="metis-action-btn metis-action-btn-danger metis-delete-page" data-id="<?php echo esc_attr( (string) $page->id ); ?>" title="Delete">
