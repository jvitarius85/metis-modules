...
$menu_button_colors = [
    'mw_primary' => metis_hex_color_clean( (string) ( $theme_saved['mw_primary'] ?? $theme_defaults['mw_primary'] ) ) ?: $theme_defaults['mw_primary'],
    'mw_accent' => metis_hex_color_clean( (string) ( $theme_saved['mw_accent'] ?? $theme_defaults['mw_accent'] ) ) ?: $theme_defaults['mw_accent'],
    'mw_text' => metis_hex_color_clean( (string) ( $theme_saved['mw_text'] ?? $theme_defaults['mw_text'] ) ) ?: $theme_defaults['mw_text'],
    'mw_surface' => metis_hex_color_clean( (string) ( $theme_saved['mw_surface'] ?? $theme_defaults['mw_surface'] ) ) ?: $theme_defaults['mw_surface'],
];
?>
<div class="mw-page-header">
    <div class="mw-page-header-left">
        <h1 class="mw-page-title">Menus</h1>
        <p class="mw-subtitle"><?php echo count( $menus ); ?> menu<?php echo count( $menus ) !== 1 ? 's' : ''; ?> available for website navigation.</p>
    </div>
    <div class="mw-page-header-right">
        <button type="button" class="mw-btn mw-btn-primary" id="metis-create-menu-btn">
...
<div class="metis-table-wrap">
    <?php if ( empty( $menus ) ) : ?>
        <div class="metis-empty-state">
            ...
            <button type="button" class="mw-btn mw-btn-primary" id="metis-create-menu-btn-empty">New Menu</button>
        </div>
    <?php else : ?>
        <div class="mw-premium-table metis-menus-table">
            <div class="mw-premium-row mw-premium-header">
                <div class="mw-premium-cell">Name</div>
                <div class="mw-premium-cell">Location</div>
                ...
                ?>
                    <div class="mw-premium-row">
                        <div class="mw-premium-cell"><strong><?php echo metis_escape_html( $menu['name'] ?? '' ); ?></strong></div>
                        <div class="mw-premium-cell"><?php echo metis_escape_html( $locations[ $menu['location'] ?? '' ] ?? ( $menu['location'] ?? '—' ) ); ?></div>
                        <div class="mw-premium-cell" style="color:var(--mw-text-muted,#888);font-size:13px;"><?php echo count( $items ); ?> item<?php echo count( $items ) !== 1 ? 's' : ''; ?></div>
                        <div class="mw-premium-cell"><span class="metis-status metis-status-<?php echo metis_escape_attr( $menu['status'] ?? 'active' ); ?>"><?php echo metis_escape_html( ucfirst( $menu['status'] ?? 'active' ) ); ?></span></div>
...
<!-- Menu Editor Modal -->
<div id="metis-menu-modal" class="mw-modal-overlay" style="display:none;" role="dialog" aria-modal="true" aria-label="Menu Editor">
    <div class="mw-modal" style="max-width:640px;width:95%;">
        <div class="mw-modal-header">
            <h2 class="mw-modal-title" id="metis-menu-modal-title">New Menu</h2>
            <button type="button" class="mw-modal-close" id="metis-menu-modal-close" aria-label="Close">&times;</button>
        </div>
        <div class="mw-modal-body" style="padding:20px;">

            <div class="mw-field" style="margin-bottom:14px;">
                <label class="mw-label">Menu Name <span style="color:#dc3545;">*</span></label>
                <input type="text" id="metis-menu-name" class="mw-input" placeholder="e.g. Main Navigation">
            </div>

            <div class="mw-field" style="margin-bottom:20px;">
                <label class="mw-label">Location</label>
                <select id="metis-menu-location" class="mw-input">
            ...
            <div style="margin-bottom:8px;font-weight:600;font-size:13px;color:var(--mw-text,#1a1f2b);">Menu Items</div>

            <div id="metis-menu-items-list" style="min-height:60px;border:1px solid var(--mw-border,#e2e6ea);border-radius:6px;padding:4px;margin-bottom:12px;background:var(--mw-surface-alt,#f7f8fa);">
                <div id="metis-menu-items-empty" style="text-align:center;padding:20px;color:var(--mw-text-muted,#aaa);font-size:13px;">No items yet — add one below.</div>
            </div>

            <div style="display:flex;gap:8px;align-items:flex-end;padding:12px;border:1px solid var(--mw-border,#e2e6ea);border-radius:6px;background:var(--mw-surface,#fff);flex-wrap:wrap;">
                <div class="mw-field" style="flex:1;min-width:160px;margin:0;">
                    <label class="mw-label" style="font-size:11px;">Label</label>
                    <input type="text" id="metis-menu-item-label" class="mw-input mw-input-sm" placeholder="Link label">
                </div>
                <div class="mw-field" style="flex:2;min-width:200px;margin:0;">
                    <label class="mw-label" style="font-size:11px;">URL</label>
                    <input type="text" id="metis-menu-item-url" class="mw-input mw-input-sm" placeholder="https:// or /page-slug">
                </div>
                <div class="mw-field" style="margin:0;">
                    <label class="mw-label" style="font-size:11px;">Opens in</label>
                    <select id="metis-menu-item-target" class="mw-input mw-input-sm" style="width:110px;">
                        <option value="">Same tab</option>
                        <option value="_blank">New tab</option>
                    </select>
                </div>
                <div class="mw-field" style="margin:0;display:flex;align-items:center;gap:6px;">
                    <label class="mw-label" style="font-size:11px;margin:0;">Button</label>
                    <input type="checkbox" id="metis-menu-item-as-button" value="1">
                </div>
                <div class="mw-field" style="margin:0;">
                    <label class="mw-label" style="font-size:11px;">Button Color</label>
                    <select id="metis-menu-item-button-color" class="mw-input mw-input-sm" style="width:150px;">
                        <?php foreach ( $menu_button_colors as $color_key => $color_hex ) : ?>
                            <option value="<?php echo metis_escape_attr( $color_key ); ?>"><?php echo metis_escape_html( strtoupper( str_replace( 'mw_', '', $color_key ) ) . ' (' . $color_hex . ')' ); ?></option>
                        <?php endforeach; ?>
                    </select>
                </div>
                <button type="button" class="mw-btn mw-btn-secondary mw-btn-sm" id="metis-menu-add-item-btn" style="flex-shrink:0;align-self:flex-end;">+ Add Item</button>
            </div>
        ...
        <div class="mw-modal-footer">
            <button type="button" class="mw-btn mw-btn-ghost" id="metis-menu-cancel-btn">Cancel</button>
            <button type="button" class="mw-btn mw-btn-primary" id="metis-menu-save-btn">Save Menu</button>
        </div>
    </div>
</div>
...
function parseMenuItems(raw) {
    ...
    var normalized = source.map(function(item) {
        ...
        return {
            ...
            button_color_key: String(obj.button_color_key || 'mw_primary').trim() || 'mw_primary'
        };
...
function renderMenusTable(menus) {
    ...
    $('.mw-page-header .mw-subtitle').first().text(list.length + ' menu' + (list.length === 1 ? '' : 's') + ' available for website navigation.');
    $wrap.empty();
    ...
    var rows = list.map(function(menu) {
        ...
        return '<div class="mw-premium-row">'
            + '<div class="mw-premium-cell"><strong>' + escapeHtml(String(menu && menu.name ? menu.name : '')) + '</strong></div>'
            + '<div class="mw-premium-cell">' + escapeHtml(locationLabel) + '</div>'
            + '<div class="mw-premium-cell" style="color:var(--mw-text-muted,#888);font-size:13px;">' + escapeHtml(String(itemCount)) + ' item' + (itemCount === 1 ? '' : 's') + '</div>'
            + '<div class="mw-premium-cell"><span class="metis-status metis-status-' + escapeHtml(status) + '">' + escapeHtml(status.charAt(0).toUpperCase() + status.slice(1)) + '</span></div>'
...
function resetItemForm() {
    ...
    $('#metis-menu-item-button-color').val('mw_primary');
...
function renderItems() {
    ...
    flat.forEach(function(entry, i) {
        ...
        var row = $([
            '<div class="metis-menu-item-row' + rowClass + '" data-index="' + i + '" data-item-id="' + $('<div>').text(item.id).html() + '" style="display:flex;align-items:center;gap:8px;padding:8px 10px;background:#fff;border:1px solid var(--mw-border,#e2e6ea);border-radius:5px;margin-bottom:4px;padding-left:' + (10 + indent) + 'px;">',
            ...
            '    <div style="font-size:11px;color:var(--mw-text-muted,#888);overflow:hidden;text-overflow:ellipsis;white-space:nowrap;">' + $('<div>').text(item.url).html() + (item.target === '_blank' ? ' ↗' : '') + '</div>' + (hasParent ? '<div style="font-size:10px;color:#5f6b86;margin-top:2px;">Sub item</div>' : ''),
            item.as_button ? '    <div style="font-size:10px;color:#2f3c5a;margin-top:2px;">Button: ' + $('<div>').text(String(item.button_color_key || 'mw_primary')).html() + '</div>' : '',
            '  </div>',
    ...
    $list.find('.metis-menu-item-row').each(function() {
        ...
        $row.on('dragover', function(e) { e.preventDefault(); $row.css('outline', '2px solid var(--mw-primary,#0d6efd)'); });
...
$(document).on('click', '#metis-menu-add-item-btn', function() {
    ...
    var colorKey = String($('#metis-menu-item-button-color').val() || 'mw_primary');
    if (editingItemId) {
...
$(document).on('click', '.metis-menu-item-edit', function() {
    ...
    $('#metis-menu-item-button-color').val(String(item.button_color_key || 'mw_primary'));
...
$(document).on('click', '.metis-delete-menu', function() {
    var id = $(this).data('id');
    var name = $(this).closest('.mw-premium-row').find('.mw-premium-cell:first strong').text();
