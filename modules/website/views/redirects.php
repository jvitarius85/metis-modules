...
<div class="mw-page-header">
    <div class="mw-page-header-left">
        <h1 class="mw-page-title">Redirects</h1>
        <p class="mw-subtitle"><?php echo count( $redirects ); ?> redirect<?php echo count( $redirects ) !== 1 ? 's' : ''; ?> configured for website routing.</p>
    </div>
    <div class="mw-page-header-right">
        <button type="button" class="mw-btn mw-btn-primary" id="metis-create-redirect-btn">
...
<div class="metis-table-wrap">
    <?php if ( empty( $redirects ) ) : ?>
        <div class="metis-empty-state">
            ...
            <button type="button" class="mw-btn mw-btn-primary" id="metis-create-redirect-btn-empty">New Redirect</button>
        </div>
    <?php else : ?>
        <div class="mw-premium-table metis-redirects-table">
            <div class="mw-premium-row mw-premium-header">
                <div class="mw-premium-cell">Source</div>
                <div class="mw-premium-cell">Destination</div>
            ...
            <?php foreach ( $redirects as $redirect ) : ?>
                <div class="mw-premium-row">
                    <div class="mw-premium-cell"><strong><?php echo metis_escape_html( (string) ( $redirect['source_path'] ?? '' ) ); ?></strong></div>
                    <div class="mw-premium-cell"><?php echo metis_escape_html( (string) ( $redirect['destination_path'] ?? '' ) ); ?></div>
                    <div class="mw-premium-cell"><?php echo metis_escape_html( (string) ( $redirect['redirect_type'] ?? '301' ) ); ?></div>
                    ...
                    <div class="mw-premium-cell" style="color:var(--mw-text-muted,#6b7280);font-size:13px;">
...
<div id="metis-redirect-modal" class="mw-modal-overlay" style="display:none;" role="dialog" aria-modal="true" aria-label="Redirect Editor">
    <div class="mw-modal" style="max-width:640px;width:95%;">
        <div class="mw-modal-header">
            <h2 class="mw-modal-title" id="metis-redirect-modal-title">New Redirect</h2>
            <button type="button" class="mw-modal-close" id="metis-redirect-modal-close" aria-label="Close">&times;</button>
        </div>
        <div class="mw-modal-body" style="padding:20px;">
            <div class="mw-field" style="margin-bottom:14px;">
                <label class="mw-label">Source Path <span style="color:#dc3545;">*</span></label>
                <input type="text" id="metis-redirect-source" class="mw-input" placeholder="/old-path">
            </div>
            <div class="mw-field" style="margin-bottom:14px;">
                <label class="mw-label">Destination Path <span style="color:#dc3545;">*</span></label>
                <input type="text" id="metis-redirect-destination" class="mw-input" placeholder="/new-path">
            </div>
            <div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(180px,1fr));gap:14px;">
                <div class="mw-field" style="margin:0;">
                    <label class="mw-label">Redirect Type</label>
                    <select id="metis-redirect-type" class="mw-input">
                        <option value="301">301</option>
                        <option value="302">302</option>
                    </select>
                </div>
                <div class="mw-field" style="margin:0;display:flex;align-items:flex-end;">
                    <label class="mw-label" style="display:flex;align-items:center;gap:8px;margin:0;">
                        <input type="checkbox" id="metis-redirect-active" value="1" checked>
                        Active
                    </label>
                </div>
            </div>
            <div class="mw-field" style="margin-top:14px;">
                <label class="mw-label">Notes</label>
                <textarea id="metis-redirect-notes" class="mw-input" rows="4" placeholder="Internal note for why this redirect exists"></textarea>
            </div>
            <input type="hidden" id="metis-redirect-id" value="">
        </div>
        <div class="mw-modal-footer">
            <button type="button" class="mw-btn mw-btn-ghost" id="metis-redirect-cancel-btn">Cancel</button>
            <button type="button" class="mw-btn mw-btn-primary" id="metis-redirect-save-btn">Save Redirect</button>
        </div>
    </div>
</div>
...
function renderRedirectTable(redirects) {
    ...
    $('.mw-page-header .mw-subtitle').first().text(list.length + ' redirect' + (list.length === 1 ? '' : 's') + ' configured for website routing.');
    $wrap.empty();
    ...
    var rows = list.map(function(redirect) {
        ...
        return '<div class="mw-premium-row">'
            + '<div class="mw-premium-cell"><strong>' + escapeHtml(String(redirect.source_path || '')) + '</strong></div>'
            + '<div class="mw-premium-cell">' + escapeHtml(String(redirect.destination_path || '')) + '</div>'
            + '<div class="mw-premium-cell">' + escapeHtml(type) + '</div>'
            + '<div class="mw-premium-cell"><span class="metis-status metis-status-' + (isActive ? 'published' : 'draft') + '">' + (isActive ? 'Active' : 'Inactive') + '</span></div>'
            + '<div class="mw-premium-cell" style="color:var(--mw-text-muted,#6b7280);font-size:13px;">' + escapeHtml(String(redirect.notes || '')) + '</div>'
