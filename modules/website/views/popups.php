...
<div class="mw-page-header">
    <div class="mw-page-header-left">
        <h1 class="mw-page-title">Popups</h1>
        <p class="mw-subtitle"><?php echo count( $popups ); ?> popup<?php echo count( $popups ) !== 1 ? 's' : ''; ?> configured with trigger and display rules.</p>
    </div>
    <div class="mw-page-header-right">
        <button class="mw-btn mw-btn-primary" id="metis-create-popup-btn">
...
<div class="metis-table-wrap">
    <?php if ( empty( $popups ) ) : ?>
        <div class="metis-empty-state">
            ...
            <button class="mw-btn mw-btn-primary" id="metis-create-popup-btn-empty">New Popup</button>
        </div>
    <?php else : ?>
        <div class="mw-premium-table metis-popup-table">
            <div class="mw-premium-row mw-premium-header">
                <div class="mw-premium-cell">Name</div>
                <div class="mw-premium-cell">Trigger</div>
                <div class="mw-premium-cell">Status</div>
                <div class="mw-premium-cell mw-col-right">Actions</div>
            </div>
                <?php foreach ( $popups as $popup ) : ?>
                    <div class="mw-premium-row">
                        <div class="mw-premium-cell"><strong><?php echo metis_escape_html( $popup['name'] ?? '' ); ?></strong></div>
                        <div class="mw-premium-cell metis-table-meta-cell"><?php echo metis_escape_html( $trigger_types[ $popup['trigger_type'] ?? 'click' ] ?? ucfirst( $popup['trigger_type'] ?? '' ) ); ?></div>
                        <div class="mw-premium-cell"><span class="metis-status metis-status-<?php echo metis_escape_attr( $popup['status'] ?? 'draft' ); ?>"><?php echo metis_escape_html( ucfirst( $popup['status'] ?? 'draft' ) ); ?></span></div>
...
function updatePopupSubtitle() {
    var count = $('.metis-popup-table .mw-premium-row').not('.mw-premium-header').length;
    $('.mw-page-header .mw-subtitle').first().text(count + ' popup' + (count === 1 ? '' : 's') + ' configured with trigger and display rules.');
}
...
$(document).on('click', '.metis-delete-popup', function() {
    ...
    metis_confirm('Delete popup "' + name + '"?', function() {
        $.ajax({
            ...
            success: function(r) {
                if (r && r.success) {
                    var $row = $('.metis-delete-popup[data-id="' + String(id || '') + '"]').first().closest('.mw-premium-row');
                    $row.remove();
                    updatePopupSubtitle();
