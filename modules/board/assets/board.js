document.addEventListener('DOMContentLoaded', function () {
    ...
    function removePlaceholderRows(container, placeholderText) {
        ...
        Array.from(container.querySelectorAll('.mw-premium-row')).forEach(function (row) {
    ...
    function upsertMeetingRow(payload, options) {
        ...
        if (!row) {
            ...
            row.className = 'mw-premium-row metis-board-row';
        ...
        row.innerHTML = ''
            + '<div class="mw-premium-cell"><div><strong>' + escHtml(data.title) + '</strong></div><div class="mw-muted">' + escHtml(data.meeting_code) + '</div></div>'
            + '<div class="mw-premium-cell">' + escHtml(data.committee_name || '—') + '</div>'
            + '<div class="mw-premium-cell">' + escHtml(formatDateTimeLabel(data.meeting_date)) + '</div>'
            + '<div class="mw-premium-cell">' + escHtml(data.meeting_type) + '</div>'
            + '<div class="mw-premium-cell"><span class="mw-chip">' + escHtml(data.status) + '</span></div>'
            + '<div class="mw-premium-cell">' + escHtml(String(data.open_actions)) + '</div>'
            + (canManageBoard ? '<div class="mw-premium-cell metis-board-actions-cell"><button type="button" class="mw-btn-xs metis-board-edit-meeting">Edit</button></div>' : '');
    ...
    function upsertCommitteeRow(payload) {
        ...
        let row = Array.from(committeeRowsContainer.querySelectorAll('.mw-premium-row')).find(function (candidate) {
        ...
        row.innerHTML = ''
            + '<div class="mw-premium-cell"><strong>' + escHtml(data.name) + '</strong><div class="mw-muted">' + escHtml(data.committee_code) + '</div></div>'
            + '<div class="mw-premium-cell">' + escHtml(data.chair_name || '—') + '</div>'
            + '<div class="mw-premium-cell">' + escHtml(String(data.meeting_count)) + '</div>'
            + (canManageBoard ? '<div class="mw-premium-cell"><button type="button" class="mw-btn-xs metis-board-edit-committee">Edit</button></div>' : '');

        Array.from(committeeRowsContainer.querySelectorAll('.mw-premium-row')).sort(function (a, b) {
    ...
    function actionResolveControlHtml(payload) {
        if (!canResolveActionItem(payload)) {
            return '<span class="mw-muted">—</span>';
        }
        return '<button type="button" class="mw-btn-xs mw-btn-ghost metis-board-resolve-action" data-action-id="' + escHtml(String(parseId(payload.id))) + '">Resolve</button>';
    }

    function ensureActionPlaceholderRow(container, text) {
        if (!container) return;
        removePlaceholderRows(container, text);
        const hasActionRows = Array.from(container.querySelectorAll('.mw-premium-row')).some(function (row) {
            return parseId(row.dataset.actionId) > 0;
        });
        if (hasActionRows) return;
        const row = document.createElement('div');
        row.className = 'mw-premium-row';
        row.innerHTML = '<div class="mw-premium-cell mw-muted">' + escHtml(String(text || 'No action items.')) + '</div>';
    ...
    function upsertMeetingActionRow(payload) {
        ...
        let row = detailActionRowsContainer.querySelector('.mw-premium-row[data-action-id="' + id + '"]');
        if (!row) {
            row = document.createElement('div');
            row.className = 'mw-premium-row';
        }
        ...
        row.innerHTML = ''
            + '<div class="mw-premium-cell"><strong>' + escHtml(String(payload.title || '')) + '</strong></div>'
            + '<div class="mw-premium-cell">' + escHtml(String(payload.owner_name || 'Unassigned')) + '</div>'
            + '<div class="mw-premium-cell">' + escHtml(formatDateLabel(payload.due_date)) + '</div>'
            + '<div class="mw-premium-cell"><span class="mw-chip">' + escHtml(String(payload.status || 'open')) + '</span></div>'
            + '<div class="mw-premium-cell metis-board-actions-cell">' + actionResolveControlHtml(payload) + '</div>';
    ...
    function upsertAnnouncementRow(payload) {
        ...
        let row = announcementRowsContainer.querySelector('.mw-premium-row[data-announcement-id="' + id + '"]');
        if (!row) {
            row = document.createElement('div');
            row.className = 'mw-premium-row';
        }
        ...
        row.innerHTML = ''
            + '<div class="mw-premium-cell"><strong>' + escHtml(String(payload.title || '')) + '</strong><div class="mw-muted">' + escHtml(String(payload.announcement_code || '')) + '</div></div>'
            + '<div class="mw-premium-cell"><span class="mw-chip">' + escHtml(String(payload.status || 'draft')) + '</span></div>'
            + '<div class="mw-premium-cell">' + escHtml(formatDateTimeLabel(payload.publish_at)) + '</div>'
            + '<div class="mw-premium-cell">' + escHtml(formatDateTimeLabel(payload.updated_at)) + '</div>';
    ...
    function upsertDashboardActionRow(payload) {
        ...
        let row = dashboardActionRowsContainer.querySelector('.mw-premium-row[data-action-id="' + id + '"]');
        if (String(payload.status || 'open') === 'done') {
            if (row) row.remove();
        ...
        row.dataset.ownerPersonId = String(parseId(payload.owner_person_id));
        row.innerHTML = ''
            + '<div class="mw-premium-cell"><strong>' + escHtml(String(payload.title || '')) + '</strong><div class="mw-muted">' + escHtml(String(payload.meeting_code || '')) + '</div></div>'
            + '<div class="mw-premium-cell">' + escHtml(String(payload.owner_name || 'Unassigned')) + '</div>'
            + '<div class="mw-premium-cell">' + escHtml(formatDateLabel(payload.due_date)) + '</div>'
            + '<div class="mw-premium-cell"><span class="mw-chip">' + escHtml(String(payload.status || 'open')) + '</span></div>'
            + '<div class="mw-premium-cell metis-board-actions-cell">' + actionResolveControlHtml(payload) + '</div>';
    ...
    function applySortUi() {
        ...
        sortButtons.forEach(function (btn) {
            ...
            btn.classList.remove('mw-sort-active', 'mw-sort-asc', 'mw-sort-desc');
            if (key === sortKey) {
                btn.classList.add('mw-sort-active');
                btn.classList.add(sortDir === 'asc' ? 'mw-sort-asc' : 'mw-sort-desc');
            }
        });
    }
    ...
    function upsertDecisionRow(payload) {
        ...
        if (!row) {
            row = document.createElement('div');
            row.className = 'mw-premium-row metis-board-decision-row';
        ...
        row.innerHTML = ''
            + '<div class="mw-premium-cell"><strong>' + escHtml(String(payload.title || '')) + '</strong>'
            + (sectionTitle || itemTitle ? '<div class="mw-muted">Section: ' + escHtml(sectionTitle) + (itemTitle ? ' · Item: ' + escHtml(itemTitle) : '') + '</div>' : '')
            + (decisionText ? '<div class="mw-muted">' + escHtml(decisionText) + '</div>' : '')
            + '</div>'
            + '<div class="mw-premium-cell">'
            + (canManageDetail
                ? '<select class="mw-select metis-board-decision-outcome" style="max-width:160px;" disabled>' + outcomeOptions + '</select>'
                : '<span class="mw-chip">' + escHtml(outcome) + '</span>')
            + '</div>'
            + '<div class="mw-premium-cell"><span class="metis-board-decision-count metis-board-decision-for" data-count="' + escHtml(String(parseId(payload.votes_for))) + '">' + escHtml(String(parseId(payload.votes_for))) + '</span></div>'
            + '<div class="mw-premium-cell"><span class="metis-board-decision-count metis-board-decision-against" data-count="' + escHtml(String(parseId(payload.votes_against))) + '">' + escHtml(String(parseId(payload.votes_against))) + '</span></div>'
            + '<div class="mw-premium-cell"><span class="metis-board-decision-count metis-board-decision-abstain" data-count="' + escHtml(String(parseId(payload.votes_abstain))) + '">' + escHtml(String(parseId(payload.votes_abstain))) + '</span></div>'
            + (showVotePanel
                ? '<div class="mw-premium-cell metis-board-decision-vote-panel"><details class="metis-board-vote-collapse"><summary>Member Voting</summary><div class="metis-board-vote-dnd" data-vote-dnd><section class="metis-board-vote-col"><h5>Board Members</h5><div class="metis-board-vote-dropzone" data-vote-bucket="available"></div></section><section class="metis-board-vote-col"><h5>For</h5><div class="metis-board-vote-dropzone" data-vote-bucket="for"></div></section><section class="metis-board-vote-col"><h5>Against</h5><div class="metis-board-vote-dropzone" data-vote-bucket="against"></div></section><section class="metis-board-vote-col"><h5>Abstain</h5><div class="metis-board-vote-dropzone" data-vote-bucket="abstain"></div></section></div><div class="mw-help">Drag board members into vote columns. Votes save automatically.</div></details></div>'
                : '');
        ...
        function minutesDraftFromAgenda() {
            ...
            if (attendanceRows.length > 0) {
                attendanceRows.forEach(function (rowEl) {
                    const member = String(rowEl.querySelector('.mw-premium-cell strong')?.textContent || '').trim();
                    const status = String(rowEl.querySelector('.metis-board-attendance-status')?.value || rowEl.querySelector('.mw-chip')?.textContent || '').trim();
                    if (member === '') return;
            ...
            const actionRows = Array.from(detailWrap.querySelectorAll('.metis-board-tab-panel[data-panel="actions"] .mw-premium-row')).filter(function (rowEl) {
                if (rowEl.classList.contains('mw-premium-header')) return false;
                const cells = Array.from(rowEl.querySelectorAll('.mw-premium-cell'));
            ...
            linkedDocRows.innerHTML = list.map(function (doc) {
                ...
                return '' +
                    '<div class="mw-premium-row">' +
                        ...
                        '<div class="mw-premium-cell">' + (link ? ('<button type="button" class="mw-btn-xs mw-btn-ghost metis-board-preview-linked-doc" data-link="' + escHtml(link) + '">Link</button>') : '<span class="mw-muted">—</span>') + '</div>' +
        ...
        function renderDriveRows(files) {
            ...
            driveRows.innerHTML = list.map(function (file) {
                ...
                if (isFolder && driveModuleUrl !== '') {
                    actions.push('<a class="mw-btn-xs mw-btn-ghost" href="' + escHtml(driveModuleUrl + '?folder_id=' + encodeURIComponent(id)) + '">Link</a>');
                }
                if (canManage && !isFolder) {
                    actions.push('<button type="button" class="mw-btn-xs mw-btn-ghost metis-board-drive-preview-file" data-link="' + escHtml(link) + '" data-name="' + escHtml(name) + '">View</button>');
                    actions.push(
                    ...
                    actions.push('<button type="button" class="mw-btn-xs mw-btn-danger metis-board-drive-delete-file" data-id="' + escHtml(id) + '" data-name="' + escHtml(name) + '">Delete</button>');
                }
                return '' +
                    '<div class="mw-premium-row metis-board-drive-row' + (isFolder ? ' metis-board-drive-row-folder' : '') + '" data-folder-id="' + (isFolder ? escHtml(id) : '') + '">' +
