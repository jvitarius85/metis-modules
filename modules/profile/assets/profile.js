document.addEventListener('DOMContentLoaded', function () {
    ...
    function renderCarddavTokens(tokens) {
        ...
        if (!rows.length) {
            list.innerHTML = '<div class="mw-muted" id="metis-profile-carddav-empty">No CardDAV tokens issued.</div>';
            return;
        }
        list.innerHTML = rows.map(function (row) {
            const id = parseInt(String(row.id || '0'), 10) || 0;
            return '<div class="metis-profile-passkey-item" data-carddav-token-id="' + id + '">' +
                ...
                '<div class="mw-muted">Created: ' + escapeHtml(row.created_at || '') + '</div>' +
                '<div class="mw-muted">Last used: ' + escapeHtml(row.last_used_at || 'Never') + '</div>' +
                '<div class="metis-people-mini-actions"><button type="button" class="mw-btn-xs mw-btn-danger metis-profile-carddav-revoke" data-id="' + id + '">Revoke</button></div>' +
            '</div>';
        }).join('');
    }
