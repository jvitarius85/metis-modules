...
<div class="metis-profile metis-contact-detail" data-person-id="<?php echo metis_escape_attr((string) ($person['id'] ?? 0)); ?>">
    ...
    <div class="mw-space-between" style="margin-bottom:14px;">
        <div>
            <h1 class="mw-page-title" id="metis-profile-title" style="margin-bottom:8px;"><?php echo metis_escape_html($full_name); ?></h1>
            <div class="mw-muted">PID: <span id="metis-profile-pid"><?php echo metis_escape_html((string) ($person['pid'] ?? '')); ?></span></div>
        </div>
    </div>

    <div class="mw-sidebar-layout metis-profile-layout">
        <aside class="mw-sidebar-layout-sidebar metis-profile-layout-sidebar">
            <div class="mw-sidebar-layout-sidebar-inner metis-profile-layout-sidebar-inner">
                <div class="mw-list-sidebar-actions">
                    <div class="mw-list-sidebar-label">Profile</div>
                    <nav class="mw-list-sidebar-nav metis-profile-tabs" aria-label="Profile sections">
                        <button type="button" class="mw-list-sidebar-nav-item metis-profile-tab-btn is-active" data-profile-tab="profile">Profile</button>
                        <button type="button" class="mw-list-sidebar-nav-item metis-profile-tab-btn" data-profile-tab="security">Security</button>
                        <button type="button" class="mw-list-sidebar-nav-item metis-profile-tab-btn" data-profile-tab="notifications">Notifications</button>
                    </nav>
                </div>
            </div>
        </aside>

        <div class="mw-sidebar-layout-content metis-profile-layout-content">
    <div class="metis-profile-tab-panel is-active" data-profile-panel="profile">
        <div class="metis-contact-detail-grid">
            <section class="metis-contact-card metis-profile-card">
                ...
                <div class="mw-muted" id="metis-profile-email-text"><?php echo metis_escape_html((string) ($person['email'] ?? '')); ?></div>
                <div class="mw-muted">Provider: <?php echo metis_escape_html(ucfirst((string) ($person['auth_provider'] ?? 'metis'))); ?></div>
                <div class="mw-muted">Last Updated: <span id="metis-profile-updated"><?php echo metis_escape_html((string) ($person['updated_at'] ?? '')); ?></span></div>
            </section>
            ...
            <section class="metis-contact-card">
                ...
                <form id="metis-profile-form" class="metis-contact-form">
                    <div class="metis-contact-field metis-contact-field-half">
                        ...
                        <input id="metis-profile-first-name" class="mw-input" type="text" value="<?php echo metis_escape_attr((string) ($person['first_name'] ?? '')); ?>" <?php disabled(!$allow_name_edit); ?>>
                    </div>
                    <div class="metis-contact-field metis-contact-field-half">
                        <label for="metis-profile-last-name">Last Name</label>
                        <input id="metis-profile-last-name" class="mw-input" type="text" value="<?php echo metis_escape_attr((string) ($person['last_name'] ?? '')); ?>" <?php disabled(!$allow_name_edit); ?>>
                    </div>
                    <div class="metis-contact-field metis-contact-field-half">
                        <label for="metis-profile-display-name">Display Name</label>
                        <input id="metis-profile-display-name" class="mw-input" type="text" value="<?php echo metis_escape_attr((string) ($person['display_name'] ?? '')); ?>" <?php disabled(!$allow_name_edit); ?> required>
                    </div>
                    <div class="metis-contact-field metis-contact-field-half">
                        <label for="metis-profile-email">Email</label>
                        <input id="metis-profile-email" class="mw-input" type="email" value="<?php echo metis_escape_attr((string) ($person['email'] ?? '')); ?>" readonly disabled>
    ...
    <div class="metis-profile-tab-panel" data-profile-panel="security">
        <div class="metis-contact-detail-grid">
                    ...
                    <?php if (!empty($passkeys)) : ?>
                        <?php foreach ($passkeys as $passkey) : ?>
                            <div class="metis-profile-passkey-item" data-passkey-id="<?php echo metis_escape_attr((string) ($passkey['id'] ?? '')); ?>">
                                ...
                                <div class="mw-muted">Created: <?php echo metis_escape_html((string) ($passkey['created_at'] ?? '')); ?></div>
                                <div class="mw-muted">Last used: <?php echo metis_escape_html((string) ($passkey['last_used_at'] ?? 'Never')); ?></div>
                                <div class="metis-people-mini-actions"><button type="button" class="mw-btn-xs mw-btn-danger metis-profile-passkey-revoke" data-id="<?php echo metis_escape_attr((string) ($passkey['id'] ?? '')); ?>">Revoke</button></div>
                            </div>
                        <?php endforeach; ?>
                    <?php else : ?>
            ...
            <section class="metis-contact-card">
                ...
                <div class="metis-contact-form" style="margin:12px 0;">
                    <div class="metis-contact-field metis-contact-field-full">
                        <label for="metis-profile-carddav-token-label">Create Device Token</label>
                        <input type="text" id="metis-profile-carddav-token-label" class="mw-input" autocomplete="off" placeholder="iPhone, MacBook, Outlook, etc.">
                    </div>
                    <div class="metis-contact-actions">
                        <button type="button" class="mw-btn" id="metis-profile-carddav-generate">Generate CardDAV Token</button>
                    </div>
                </div>
                <div class="mw-callout mw-callout-warning" id="metis-profile-carddav-issued" style="display:none;"></div>
                    ...
                    <?php if (!empty($carddav_tokens)) : ?>
                        <?php foreach ($carddav_tokens as $token_row) : ?>
                            <div class="metis-profile-passkey-item" data-carddav-token-id="<?php echo metis_escape_attr((string) ($token_row['id'] ?? '')); ?>">
                                ...
                                <div class="mw-muted">Created: <?php echo metis_escape_html((string) ($token_row['created_at'] ?? '')); ?></div>
                                <div class="mw-muted">Last used: <?php echo metis_escape_html((string) ($token_row['last_used_at'] ?? 'Never')); ?></div>
                                <div class="metis-people-mini-actions"><button type="button" class="mw-btn-xs mw-btn-danger metis-profile-carddav-revoke" data-id="<?php echo metis_escape_attr((string) ($token_row['id'] ?? '')); ?>">Revoke</button></div>
                            </div>
                        <?php endforeach; ?>
                    <?php else : ?>
                        <div class="mw-muted" id="metis-profile-carddav-empty">No CardDAV tokens issued.</div>
                    <?php endif; ?>
                </div>
            </section>
            ...
            <section class="metis-contact-card">
                <h3>Password</h3>
                <form id="metis-profile-password-form" class="metis-contact-form" data-has-password="<?php echo $has_metis_password ? '1' : '0'; ?>" data-can-set-from-session="<?php echo $can_set_metis_password_from_session ? '1' : '0'; ?>">
                    <div class="metis-contact-field metis-contact-field-full">
                    ...
                    <div class="metis-contact-field metis-contact-field-half">
                        ...
                        <input type="password" id="metis-profile-current-password" class="mw-input" autocomplete="current-password" required>
                    </div>
                    <?php endif; ?>
                    <div class="metis-contact-field metis-contact-field-half">
                        <label for="metis-profile-new-password">New Password</label>
                        <input type="password" id="metis-profile-new-password" class="mw-input" autocomplete="new-password" minlength="12" required>
                    </div>
                    <div class="metis-contact-field metis-contact-field-half">
                        <label for="metis-profile-confirm-password">Confirm Password</label>
                        <input type="password" id="metis-profile-confirm-password" class="mw-input" autocomplete="new-password" minlength="12" required>
                    </div>
                    <div class="metis-contact-actions">
                        <button type="submit" class="mw-btn"><?php echo ($has_metis_password && !$can_set_metis_password_from_session) ? 'Update Metis Password' : 'Set Metis Password'; ?></button>
                    </div>
                </form>

                <?php if ($has_workspace_password) : ?>
                <form id="metis-profile-workspace-password-form" class="metis-contact-form" style="margin-top:24px;padding-top:24px;border-top:1px solid #e5e7eb;">
                    <div class="metis-contact-field metis-contact-field-full">
                    ...
                    <div class="metis-contact-field metis-contact-field-half">
                        ...
                        <input type="password" id="metis-profile-workspace-new-password" class="mw-input" autocomplete="new-password" minlength="12" required>
                    </div>
                    <div class="metis-contact-field metis-contact-field-half">
                        <label for="metis-profile-workspace-confirm-password">Confirm Password</label>
                        <input type="password" id="metis-profile-workspace-confirm-password" class="mw-input" autocomplete="new-password" minlength="12" required>
                    </div>
                    <div class="metis-contact-actions">
                        <button type="submit" class="mw-btn">Update Workspace Password</button>
                    </div>
                </form>
                <?php endif; ?>
        ...
        <div class="metis-contacts-modal-inner metis-profile-modal-inner">
            ...
            <div class="metis-contact-form">
                ...
                <div class="metis-contact-field metis-contact-field-full">
                    ...
                    <input id="metis-profile-avatar-zoom" class="mw-input" type="range" min="1" max="4" step="0.01" value="1">
                </div>
                <div class="metis-contact-actions">
