...
<h1 class="mw-page-title">Deposit Batch <?php echo metis_escape_html( $batch->batch_code ); ?></h1>
<p class="mw-subtitle">Transactions included in this deposit batch.</p>

<!-- BATCH SUMMARY -->
<div class="mw-premium-row mw-batch-summary mw-batch-summary-row">
...
<div class="mw-batch-actions-row">
    ...
    <?php if ( ! empty( $transactions ) ) : ?>
        ...
        ?>
            <div class="mw-premium-row mw-batch-row">
                <div><?php echo metis_escape_html( $date ); ?></div>
                <div><a href="<?php echo metis_escape_url( $tx_url ); ?>"><?php echo metis_escape_html( $donor ); ?></a></div>
                <div><?php echo metis_escape_html( $campaign ); ?></div>
                <div class="mw-col-numeric">
                    $<?php echo number_format( $amount, 2 ); ?>
                    <?php if ( $fee > 0 ) : ?>
                        <div class="mw-muted mw-batch-fee-note">($<?php echo number_format( $fee, 2 ); ?> fee)</div>
                    <?php endif; ?>
                </div>
...
<p class="mw-batch-back-link">
    <a href="<?php echo metis_escape_url( $base_url . '/transactions/' ); ?>" class="mw-btn mw-btn-xs">← Back to Transactions</a>
</p>

<!-- NOTES -->
<h2 class="mw-section-header mw-batch-notes-heading">Batch Notes</h2>
        ...
        <ul class="mw-notes-list">
            <?php foreach ( $batch_notes as $n ) : ?>
                <li class="mw-note-item" data-id="<?php echo (int) $n->id; ?>">
                    <div class="mw-note-meta mw-muted mw-batch-note-meta">
                        <?php echo metis_escape_html( $n->created_at ? date( 'm/d/Y H:i', strtotime( $n->created_at ) ) : '' ); ?>
                    </div>
                    <div class="mw-note-body"><?php echo nl2br( metis_escape_html( $n->note_text ) ); ?></div>
                </li>
            <?php endforeach; ?>
        </ul>
    <?php else : ?>
...
<div class="mw-batch-note-form">
    <input type="text" id="mw-new-note" class="mw-input mw-batch-note-input" placeholder="Add a note…">
...
(function () {
    ...
    document.getElementById('mw-save-note')?.addEventListener('click', function () {
        ...
        .then(res => {
            ...
            li.innerHTML = `<div class="mw-note-meta mw-muted mw-batch-note-meta">Just now</div>
