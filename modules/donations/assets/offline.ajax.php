...
function metis_donations_quick_action_offline_donation_form( array $action = [] ): array {
    ...
    $html = '<form class="mw-offline-donation-form mw-quick-action-form" data-quick-action-form="donations_record_offline_donation">'
        . '<input type="hidden" name="donor_did" value="">'
        . '<div class="mw-offline-donation-grid">'
        . '<label class="mw-offline-field"><span>Date</span><input type="date" name="tran_date" class="mw-input" value="' . metis_escape_attr( date( 'Y-m-d' ) ) . '" required></label>'
        . '<label class="mw-offline-field"><span>Amount</span><input type="number" name="amount" class="mw-input" min="0.01" step="0.01" placeholder="0.00" required></label>'
        . '<label class="mw-offline-field"><span>Campaign</span><select name="campaign_code" class="mw-select" required>' . $campaignOptions . '</select></label>'
        . '<label class="mw-offline-field"><span>Payment Method</span><select name="payment_method" class="mw-select"><option value="ck">Check</option><option value="cash">Cash</option><option value="ach">ACH</option><option value="other">Other</option></select></label>'
        . '<label class="mw-offline-field"><span>Check Number</span><input type="text" name="chk_num" class="mw-input" placeholder="Optional"></label>'
        . '<label class="mw-offline-field"><span>First Name</span><input type="text" name="first_name" class="mw-input" placeholder="Donor first name"></label>'
        . '<label class="mw-offline-field"><span>Last Name</span><input type="text" name="last_name" class="mw-input" placeholder="Donor last name"></label>'
        . '<label class="mw-offline-field"><span>Email</span><input type="email" name="email" class="mw-input" placeholder="donor@example.org"></label>'
        . '<label class="mw-offline-field"><span>Phone</span><input type="text" name="phone" class="mw-input" placeholder="Optional"></label>'
        . '<label class="mw-offline-field mw-offline-field-notes"><span>Notes</span><textarea name="notes" class="mw-input" rows="3" placeholder="Internal notes or source details"></textarea></label>'
        . '</div>'
        . '</form>';
