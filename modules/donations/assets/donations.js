...
function initMetisDonationsModule( context ) {
    ...
    const hasDonationsUi = scope.querySelector( '.metis-donor-view, .metis-donor-row, .metis-tx-row, .metis-campaigns-table' );
    ...
    const donorView = document.querySelector( '.metis-donor-view' );

    if ( donorView ) {
        const rowsContainer = document.getElementById( 'metis-donor-tx-rows' );
        if ( ! rowsContainer ) return;

        const allRows = Array.from( rowsContainer.querySelectorAll( '.metis-donor-tx-row' ) );
        ...
        const prevBtn  = document.getElementById( 'metis-donor-prev' );
        const nextBtn  = document.getElementById( 'metis-donor-next' );
        const statusEl = document.getElementById( 'metis-donor-page-status' );
        const sortBtns = donorView.querySelectorAll( '.metis-sort-btn' );
        ...
        function applySort() {
            ...
            sortBtns.forEach( btn => {
                btn.classList.remove( 'metis-sort-asc', 'metis-sort-desc', 'metis-sort-active' );
                if ( btn.dataset.sort === sortKey ) {
                    btn.classList.add( 'metis-sort-active', sortDir === 'asc' ? 'metis-sort-asc' : 'metis-sort-desc' );
                }
            } );
            applyPagination();
        }
    ...
    const donorRows = Array.from( document.querySelectorAll( '.metis-donor-row' ) );

    if ( donorRows.length ) {
        const search     = document.getElementById( 'metis-donor-search' );
        const givingFilt = document.getElementById( 'metis-donor-giving' );
        const prevBtn    = document.getElementById( 'metis-page-prev' );
        const nextBtn    = document.getElementById( 'metis-page-next' );
        const indicator  = document.getElementById( 'metis-page-indicator' );
    ...
    const offlineDonationForm = document.querySelector( '.metis-offline-donation-form' );
    if ( offlineDonationForm ) {
        const modal         = document.getElementById( 'metis-offline-donation-modal' );
        const openBtn       = document.getElementById( 'metis-open-offline-donation-modal' );
        const closeBtn      = document.getElementById( 'metis-offline-donation-close' );
        const cancelBtn     = document.getElementById( 'metis-offline-donation-cancel' );
        ...
        const lookupInput   = document.getElementById( 'metis-offline-donor-lookup' );
        const donorDidInput = document.getElementById( 'metis-offline-donor-did' );
        const donorStatus   = document.getElementById( 'metis-offline-donor-status' );
        const donorResults  = document.getElementById( 'metis-offline-donor-results' );
        ...
        function openOfflineModal() {
            ...
            if ( window.MetisCore && MetisCore.modal && typeof MetisCore.modal.open === 'function' ) {
                MetisCore.modal.open( 'metis-offline-donation-modal' );
            } else {
        ...
        function closeOfflineModal() {
            ...
            if ( window.MetisCore && MetisCore.modal && typeof MetisCore.modal.close === 'function' ) {
                MetisCore.modal.close( 'metis-offline-donation-modal' );
            } else {
        ...
        function renderLookupResults( matches ) {
            ...
            if ( ! Array.isArray( matches ) || ! matches.length ) {
                ...
                donorResults.innerHTML = '<div class="metis-offline-donor-empty">No matching donors found. Continue entering a new donor.</div>';
                return;
            }
            ...
            donorResults.innerHTML = matches.map( function (item, index) {
                ...
                return '' +
                    '<button type="button" class="metis-offline-donor-result" data-index="' + index + '">' +
                        '<strong>' + Metis.util.escapeHtml( item.name || item.did || 'Donor' ) + '</strong>' +
                        '<span>' + meta.join( ' · ' ) + '</span>' +
                    '</button>';
            } ).join( '' );

            donorResults.querySelectorAll( '.metis-offline-donor-result' ).forEach( function (button) {
        ...
        function runDonorLookup() {
            ...
            donorResults.hidden = false;
            donorResults.innerHTML = '<div class="metis-offline-donor-empty">Searching donors…</div>';
            ...
            fetch( metisAjax.ajax_url, { method: 'POST', body: body, credentials: 'same-origin' } )
                ...
                .catch( function (error) {
                    donorResults.hidden = false;
                    donorResults.innerHTML = '<div class="metis-offline-donor-empty metis-offline-donor-empty-error">' + Metis.util.escapeHtml( error.message || 'Lookup failed.' ) + '</div>';
                } );
        }
    ...
    const txRows = Array.from( document.querySelectorAll( '.metis-tx-row' ) );

    if ( txRows.length ) {
        const tableContainer = document.querySelector( '.metis-tx-table' );
        const statusFilter   = document.getElementById( 'metis-status-filter' );
        const campaignFilter = document.getElementById( 'metis-campaign-filter' );
        const methodFilter   = document.getElementById( 'metis-method-filter' );
        const dateStartInput = document.getElementById( 'metis-date-start' );
        const dateEndInput   = document.getElementById( 'metis-date-end' );
        const searchInput    = document.getElementById( 'metis-search' );
        const quickButtons   = document.querySelectorAll( '.metis-pill-btn' );
        const sortSelect     = document.getElementById( 'metis-sort' );
        const exportBtn      = document.getElementById( 'metis-export-csv' );
        const badgesWrap     = document.getElementById( 'metis-active-filters' );
        const selectAll      = document.getElementById( 'metis-tx-select-all' );
        const txCheckboxes   = Array.from( document.querySelectorAll( '.metis-tx-checkbox' ) );
        const batchCount     = document.getElementById( 'metis-batch-count' );
        const batchTotal     = document.getElementById( 'metis-batch-total' );
        const batchSubmit    = document.getElementById( 'metis-create-batch-btn' );
        ...
        function applyFilters() {
            ...
            txRows.forEach( row => {
                ...
                row.classList.remove( 'metis-tx-row--focused' );
            } );
        ...
        function updateBadges() {
            ...
            badges.forEach( b => {
                ...
                el.type = 'button'; el.className = 'metis-filter-badge'; el.dataset.filter = b.key;
                el.innerHTML = `${ b.label } <span class="metis-badge-x">×</span>`;
        ...
        if ( badgesWrap ) {
            badgesWrap.addEventListener( 'click', function ( e ) {
                const badge = e.target.closest( '.metis-filter-badge' );
                if ( ! badge ) return;
        ...
        if ( exportBtn ) {
            exportBtn.addEventListener( 'click', function () {
                ...
                visible.forEach( row => {
                    csvRows.push( [
                        row.dataset.tid     || '',
                        ( row.querySelector( '.metis-tx-date' )     || {} ).textContent || '',
                        ( row.querySelector( '.metis-tx-donor' )    || {} ).textContent || '',
                        ( row.querySelector( '.metis-tx-campaign' ) || {} ).textContent || '',
                        ( row.querySelector( '.metis-tx-amount' )   || {} ).textContent || '',
                        row.dataset.status    || '',
        ...
        function refreshBatchSummary() {
            ...
            txCheckboxes.forEach( cb => {
                if ( cb.checked ) {
                    count++;
                    total += parseFloat( cb.closest( '.metis-tx-row' )?.dataset.amount || '0' ) || 0;
                }
            } );
        ...
        function focusRow( idx ) {
            const visible = getVisibleRows();
            visible.forEach( r => r.classList.remove( 'metis-tx-row--focused' ) );
            if ( ! visible.length ) { focusedIndex = -1; return; }
            idx = Math.max( 0, Math.min( idx, visible.length - 1 ) );
            visible[ idx ].classList.add( 'metis-tx-row--focused' );
        ...
        scope.addEventListener( 'keydown', function ( e ) {
            ...
            if ( e.key === 'Enter' && focusedIndex >= 0 ) {
                const link = getVisibleRows()[ focusedIndex ]?.querySelector( '.metis-tx-main-link' );
    ...
    const campaignsTable = document.querySelector( '.metis-campaigns-table' );

    if ( campaignsTable ) {

        const searchInput  = document.getElementById( 'metis-campaign-search' );
        const typeFilter   = document.getElementById( 'metis-campaign-type-filter' );
        const statusFilter = document.getElementById( 'metis-campaign-status-filter' );
        const rows         = Array.from( document.querySelectorAll( '.metis-campaign-row[data-name]' ) );
