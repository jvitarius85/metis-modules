...
if ( ! function_exists( "metis_donations_render_form_embed" ) ) {
    ...
    function metis_donations_render_form_embed( array $options = [] ): string {
        ...
        $html .= "<input class=\"mw-input\" type=\"number\" min=\"1\" step=\"1\" name=\"amount\" value=\"" . metis_escape_attr( (string) $amount ) . "\" placeholder=\"Amount\">";
        if ( $show_name ) {
            $html .= "<input class=\"mw-input\" type=\"text\" name=\"name\" placeholder=\"Full name\">";
        }
        if ( $show_email ) {
            $html .= "<input class=\"mw-input\" type=\"email\" name=\"email\" placeholder=\"Email\">";
        }
        if ( $show_phone ) {
            $html .= "<input class=\"mw-input\" type=\"tel\" name=\"phone\" placeholder=\"Phone\">";
        }
        $html .= "</div>";
        $html .= "<button type=\"submit\" class=\"mw-btn\">" . metis_escape_html( $submit ) . "</button>";
        $html .= "</form>";
