...
(function ($) {
    ...
    function updateCampaignRowStatus(campaignId, status) {
        ...
        const $statusChip = $row.children('.mw-premium-cell').eq(3).find('.mw-chip').first();
        ...
        if ($actionsCell.length) {
            ...
            if (nextStatus !== 'archived') {
                $actionsCell.append('<button class="mw-btn-xs metis-newsletter-test-campaign" type="button" data-campaign-id="' + Metis.util.escapeHtml(String(campaignId || '')) + '">Test</button>');
            }
            if (!isSentish) {
                if (nextStatus !== 'queued' && nextStatus !== 'scheduled') {
                    $actionsCell.append('<button class="mw-btn-xs metis-newsletter-edit-campaign" type="button" data-campaign-code="' + Metis.util.escapeHtml(campaignCode) + '" data-campaign-id="' + Metis.util.escapeHtml(String(campaignId || '')) + '">Edit</button>');
                }
                $actionsCell.append('<button class="mw-btn-xs metis-newsletter-queue-campaign" type="button" data-campaign-id="' + Metis.util.escapeHtml(String(campaignId || '')) + '">Send</button>');
                $actionsCell.append('<button class="mw-btn-xs mw-btn-danger metis-newsletter-delete-campaign" type="button" data-campaign-id="' + Metis.util.escapeHtml(String(campaignId || '')) + '">Delete</button>');
            } else if (nextStatus === 'sent') {
                $actionsCell.append('<button class="mw-btn-xs mw-btn-danger metis-newsletter-archive-campaign" type="button" data-campaign-id="' + Metis.util.escapeHtml(String(campaignId || '')) + '">Archive</button>');
            }
        }
    }
    ...
    function removeCampaignRow(campaignId) {
        ...
        $row.remove();
        $rowsWrap.find('.mw-premium-row').filter(function () {
            return !$(this).hasClass('metis-newsletter-row') && !$(this).hasClass('mw-premium-header');
        }).remove();
        if (!$rowsWrap.find('.metis-newsletter-row').length) {
            $rowsWrap.append('<div class="mw-premium-row"><div class="mw-premium-cell mw-muted">No campaigns yet.</div></div>');
        }
    }
        ...
        modal.innerHTML =
            '<div class="mw-modal mw-modal-sm">' +
                '<div class="mw-modal-header">' +
                    '<h2 class="mw-modal-title" id="metis-newsletter-prompt-title">Enter Value</h2>' +
                '</div>' +
                '<div class="mw-modal-body">' +
                    '<div class="mw-field metis-newsletter-prompt-input-wrap">' +
                        '<label id="metis-newsletter-prompt-label" for="metis-newsletter-prompt-input">Value</label>' +
                        '<input id="metis-newsletter-prompt-input" class="mw-input mw-input-wide" type="text">' +
                    '</div>' +
                    '<div class="mw-field metis-newsletter-prompt-options-wrap">' +
                    ...
                    '</div>' +
                '</div>' +
                '<div class="mw-modal-footer metis-newsletter-prompt-footer">' +
                    '<button type="button" class="mw-btn mw-btn-ghost" data-newsletter-prompt-cancel>Cancel</button>' +
                    '<button type="button" class="mw-btn" data-newsletter-prompt-submit>Continue</button>' +
                '</div>' +
            '</div>';
    ...
    function ensureThemeImageSettingsModal() {
        ...
        modal.setAttribute('aria-hidden', 'true');
        modal.innerHTML =
            '<div class="mw-modal mw-modal-sm metis-newsletter-image-settings-modal">' +
                '<div class="mw-modal-header">' +
                    '<h2 class="mw-modal-title">Image Settings</h2>' +
                '</div>' +
                '<div class="mw-modal-body">' +
                    ...
                    '</div>' +
                '</div>' +
                '<div class="mw-modal-footer metis-newsletter-image-settings-footer">' +
                    '<button type="button" class="mw-btn mw-btn-ghost" data-image-settings-cancel>Cancel</button>' +
                    '<button type="button" class="mw-btn" data-image-settings-apply>Apply</button>' +
                '</div>' +
            '</div>';
        document.body.appendChild(modal);
    ...
    function buildThemeSelectx() {
        document.querySelectorAll('#metis-newsletter-theme-card select').forEach(function (select) {
            ...
            trigger.type = 'button';
            trigger.className = 'mw-input metis-theme-selectx-trigger';
