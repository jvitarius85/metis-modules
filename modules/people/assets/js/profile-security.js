    ...
    function ensureCredentialModal() {
        ...
        modal.innerHTML =
            '<div class="metis-modal metis-people-modal-inner">' +
                '<h3 class="metis-modal-title" id="metis-people-credential-title">Temporary Password</h3>' +
                '<div class="metis-form-grid">' +
                    '<div class="metis-field metis-field-full">' +
                        '<div class="metis-muted" id="metis-people-credential-message"></div>' +
                    '</div>' +
                    '<div class="metis-field metis-field-full">' +
                        '<label for="metis-people-credential-password">Temporary Password</label>' +
                        '<input id="metis-people-credential-password" class="metis-input" type="text" readonly autocomplete="off">' +
                    '</div>' +
                    '<div class="metis-form-actions">' +
                        '<button type="button" id="metis-people-credential-copy" class="metis-btn">Copy</button>' +
                        '<button type="button" id="metis-people-credential-close" class="metis-btn metis-btn-ghost">Close</button>' +
                    '</div>' +
                '</div>' +
            '</div>';
            ...
            totpVerify.addEventListener('click', function () {
                ...
                post('metis_people_verify_totp_secret', {
                    ...
                    chips.forEach(function (row) {
                        const chip = row.querySelector('.metis-chip');
                        if (chip) {
                            chip.textContent = 'Configured';
                            chip.classList.add('metis-chip-success');
                        }
                    });
                }).catch(function (err) {
            ...
            openSecurityConfirmModal(
                ...
                post('metis_people_reset_mfa', {
                    ...
                    document.querySelectorAll('#metis-people-totp-setup-open').forEach(function (btn) {
                        ...
                        const chip = row ? row.querySelector('.metis-chip') : null;
                        if (chip) {
                            chip.textContent = 'Not configured';
                            chip.classList.remove('metis-chip-success');
                        }
                    });
                    ...
                    if (passkeysList) {
                        passkeysList.innerHTML = '<div class="metis-muted">No passkeys registered.</div>';
                    }
