    ...
    if (workspaceRoot) {
            ...
            wrap.innerHTML = list.map(function (row) {
                ...
                return '' +
                    '<div class="mw-premium-row">' +
                        ...
                        '</div>' +
                        '<div class="mw-premium-cell">' + entityHtml + '</div>' +
                        '<div class="mw-premium-cell"><span class="mw-chip ' + Metis.util.escapeHtml(statusClass) + '">' + statusLabel + '</span></div>' +
            ...
            wrap.innerHTML = list.map(function (row) {
                ...
                return '' +
                    '<div class="mw-premium-row">' +
                        '<div class="mw-premium-cell">' +
                            '<strong>' + title + '</strong>' +
                            (reason ? '<div class="mw-muted">Reason: ' + reason + '</div>' : '') +
                        '</div>' +
                        '<div class="mw-premium-cell">' + userHtml + '</div>' +
                        '<div class="mw-premium-cell"><span class="mw-chip ' + Metis.util.escapeHtml(statusClass) + '">' + statusLabel + '</span></div>' +
                        '<div class="mw-premium-cell">' + time + '</div>' +
                    '</div>';
            }).join('');
        ...
        if (canManage) {
            ...
            function updateWorkspaceRowFromUser(user) {
                ...
                const cells = row.querySelectorAll('.mw-premium-cell');
                if (cells.length >= 7) {
                    cells[0].innerHTML = '<strong>' + escapeHtml(name) + '</strong>' + (secondaryEmail ? ('<div class="mw-muted">' + escapeHtml(secondaryEmail) + '</div>') : '');
                    ...
                    if (linkedPid) {
                        cells[5].innerHTML = linkedName
                            ? ('<div>' + escapeHtml(linkedName) + '</div><div class="mw-muted">' + escapeHtml(linkedPid) + '</div>')
                            : ('<div class="mw-muted">' + escapeHtml(linkedPid) + '</div>');
                    } else {
                        cells[5].textContent = '—';
                    }
                }
                ...
                if (statusCell) {
                    ...
                    if (isProtected) html += '<span class="mw-chip">Protected</span>';
                    if (isHidden) html += '<span class="mw-chip">Hidden</span>';
                            ...
                            if (cells.length >= 7) {
                                ...
                                if (pid) {
                                    cells[5].innerHTML = displayName
                                        ? ('<div>' + escapeHtml(displayName) + '</div><div class="mw-muted">' + escapeHtml(pid) + '</div>')
                                        : ('<div class="mw-muted">' + escapeHtml(pid) + '</div>');
                                } else {
                                    cells[5].textContent = 'Linked';
                                }
                                cells[6].innerHTML = '<span class="mw-muted">—</span>';
                            }
                            if (personUrl) {
                                row.dataset.personUrl = personUrl;
                                row.classList.add('is-clickable');
            ...
            if (userRowsWrap) {
                ...
                userRowsWrap.addEventListener('click', function (event) {
                    const inMenuAction = event.target.closest('.metis-workspace-actions-menu .mw-btn-xs');
                    if (inMenuAction) {
            ...
            function trimWorkspaceGroupSettingsUi() {
                ...
                if (advancedPanel) {
                    const fields = Array.from(advancedPanel.querySelectorAll('.metis-contact-field'));
                    ...
                    panelOrder.forEach(function (name) {
                        ...
                        if (!panel || !btn) return;
                        const hasVisibleFields = Array.from(panel.querySelectorAll('.metis-contact-field')).some(function (field) {
                ...
                if (permPanel) {
                    [
                        ...
                        if (!el) return;
                        const field = el.closest('.metis-contact-field');
            ...
            function renderExternalMembersGrid(externalMembers) {
                ...
                externals.forEach(function (member) {
                    ...
                    row.innerHTML =
                        ...
                        '<div class="metis-workspace-members-user"><strong>' + title.replace(/</g, '&lt;') + '</strong><span class="mw-muted">' + subtitle.replace(/</g, '&lt;') + '</span></div>' +
