    ...
    if (activityRoot) {
        ...
        function renderActivityRows(rows) {
            ...
            activityRows.innerHTML = list.map(function (row) {
                return '' +
                    '<div class="mw-premium-row" style="display:grid;grid-template-columns:180px 220px 1fr 180px 220px;align-items:center;">' +
                        '<div class="mw-premium-cell">' + Metis.util.escapeHtml(String((row && row.time) || '')) + '</div>' +
                        '<div class="mw-premium-cell">' + Metis.util.escapeHtml(String((row && row.type) || '')) + '</div>' +
                        '<div class="mw-premium-cell">' + Metis.util.escapeHtml(String((row && row.summary) || '')) + '</div>' +
                        '<div class="mw-premium-cell">' + Metis.util.escapeHtml(String((row && row.target) || '—')) + '</div>' +
                        '<div class="mw-premium-cell">' + Metis.util.escapeHtml(String((row && row.actor) || 'System')) + '</div>' +
                    '</div>';
            }).join('');
        }
