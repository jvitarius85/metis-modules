...
if (!metis_people_can_manage()) {
    echo '<div class="mw-alert mw-alert-error">You do not have permission to manage positions.</div>';
    return;
}
...
<div class="metis-people metis-people-positions-page" data-can-manage="1">
    <h1 class="mw-page-title"><?php echo metis_escape_html(metis_current_module_label('People')); ?> Positions</h1>
    <p class="mw-subtitle">Standardize positions by group for consistent assignment.</p>
    ...
    <?php metis_render_sidebar_layout([
        'sidebar' => static function () { ?>
            <div class="mw-list-sidebar-section">
                <div class="mw-list-sidebar-label">Position Groups</div>
                <div class="mw-muted">Board, Staff, Volunteer</div>
            </div>
            <div class="mw-list-sidebar-actions">
                <a href="<?php echo metis_escape_url(metis_people_people_list_url()); ?>" class="mw-btn mw-btn-xs mw-btn-ghost">Back To People</a>
            </div>
        <?php },
        'content' => static function () use ($grouped) { ?>
            <section class="mw-card" style="padding:16px;">
                <?php foreach (['board' => 'Board Positions', 'staff' => 'Staff Positions', 'volunteer' => 'Volunteer Positions'] as $group_key => $group_label) : ?>
                    <div class="metis-contact-field metis-contact-field-full" style="margin-bottom:14px;">
                        ...
                        <div class="metis-people-position-list" data-position-list="<?php echo metis_escape_attr($group_key); ?>">
                            <?php if (!empty($grouped[$group_key])) : ?>
                                ...
                                <?php endforeach; ?>
                            <?php else : ?>
                                <div class="mw-muted">No positions configured.</div>
                            <?php endif; ?>
                        </div>
                        <div class="metis-contact-form" style="margin-top:8px;">
                            <div class="metis-contact-field metis-contact-field-half">
                                <input type="text" class="mw-input" data-position-new-label="<?php echo metis_escape_attr($group_key); ?>" placeholder="New <?php echo metis_escape_attr(strtolower($group_label)); ?>">
                            </div>
                            <div class="metis-contact-field metis-contact-field-half">
                                <button type="button" class="mw-btn" data-position-add="<?php echo metis_escape_attr($group_key); ?>">Add</button>
                            </div>
                        </div>
                    </div>
                <?php endforeach; ?>
