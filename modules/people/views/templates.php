...
<div class="metis-people-ops">
    <h1 class="mw-page-title"><?php echo metis_escape_html( metis_current_module_view_title( 'Role Templates' ) ); ?></h1>
    ...
    <section class="mw-premium-wrap" style="margin-top:14px;">
        ...
        <form id="metis-template-apply-form" class="metis-contact-form">
            ...
            <div class="metis-contact-field metis-contact-field-half"><label for="metis-template-apply-key">Template</label><select id="metis-template-apply-key" class="mw-select" required><option value="">Select template</option><?php foreach ($templates as $template): ?><option value="<?php echo metis_escape_attr((string) $template['template_key']); ?>"><?php echo metis_escape_html((string) $template['template_name']); ?></option><?php endforeach; ?></select></div>
