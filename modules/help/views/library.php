...
if ( ! function_exists( 'metis_render_sidebar_module_layout' ) ) {
    require_once METIS_PATH . 'src/Metis/Core/Runtime/SidebarModuleLayout.php';
}
    ...
    metis_render_sidebar_module_layout(
        [
            ...
            'header_actions' => static function () use ( $homeUrl, $searchUrl, $articlesUrl, $canManage ): void {
                ...
                echo '<a class="mw-btn mw-btn-secondary" href="' . metis_escape_url( $homeUrl ) . '">Help Home</a>';
                echo '<a class="mw-btn mw-btn-secondary" href="' . metis_escape_url( $searchUrl ) . '">Search</a>';
                if ( $canManage ) {
                    echo '<a class="mw-btn" href="' . metis_escape_url( $articlesUrl ) . '">Manage Articles</a>';
                }
                echo '</div>';
            },
            'sidebar' => static function () use ( $tree, $searchQuery, $searchCategory, $categories, $activeCategorySlug, $activeArticleSlug, $searchUrl ): void {
                ...
                echo '<div class="mw-list-sidebar-section">';
                echo '<div class="mw-list-sidebar-label">Search Help</div>';
                ...
                echo '<input id="metis-help-sidebar-query" class="mw-input" type="search" name="q" value="' . metis_escape_attr( $searchQuery ) . '" placeholder="Search help by task, issue, or module">';
                echo '<label class="screen-reader-text" for="metis-help-sidebar-category">Help category</label>';
                echo '<select id="metis-help-sidebar-category" class="mw-input" name="category" aria-label="Help category">';
                ...
                echo '</select>';
                echo '<button class="mw-btn" type="submit">Search Help</button>';
                echo '</form>';
                echo '</div>';

                echo '<div class="mw-list-sidebar-section">';
                echo '<div class="mw-list-sidebar-label">Navigation</div>';
                echo '<div class="mw-list-sidebar-actions">';
                echo '<a class="mw-btn mw-btn-secondary" href="' . metis_escape_url( $searchUrl ) . '">Open Search</a>';
                echo '<a class="mw-btn mw-btn-secondary" href="' . metis_escape_url( function_exists( 'metis_home_url' ) ? (string) metis_home_url( '/admin/help' ) : '/admin/help' ) . '">Help Home</a>';
                echo '</div>';
                echo '</div>';

                echo '<div class="mw-list-sidebar-section">';
                echo '<div class="mw-list-sidebar-label">Help Library</div>';
                ...
                if ( $tree === [] ) {
                    ...
                    foreach ( $tree as $treeCategory ) {
                        ...
                        echo '<a class="mw-list-sidebar-nav-item metis-help-tree-category' . ( $isActiveCategory && $activeArticleSlug === '' ? ' is-active' : '' ) . '" href="' . metis_escape_url( (string) ( $treeCategory['url'] ?? '#' ) ) . '" onclick="event.stopPropagation();">';
                        ...
                        echo '</summary>';
                        echo '<nav class="mw-list-sidebar-nav metis-help-tree-nav" aria-label="' . metis_escape_attr( (string) ( $treeCategory['name'] ?? 'Category' ) ) . ' articles">';
                        foreach ( (array) ( $treeCategory['articles'] ?? [] ) as $treeArticle ) {
                            $articleSlug = (string) ( $treeArticle['slug'] ?? '' );
                            echo '<a class="mw-list-sidebar-nav-item metis-help-tree-article' . ( $activeArticleSlug === $articleSlug ? ' is-active' : '' ) . '" href="' . metis_escape_url( (string) ( $treeArticle['url'] ?? '#' ) ) . '">' . metis_escape_html( (string) ( $treeArticle['title'] ?? '' ) ) . '</a>';
                        }
                        echo '</nav>';
                        echo '</details>';
                    }
            ...
            'content' => static function () use ( $pageKind, $landing, $results, $article, $relatedArticles, $category, $adminContent, $normalizeRows, $renderResultsList, $renderCategoryCards, $homeUrl, $searchUrl, $searchQuery, $searchCategory, $categories, $featuredCategories, $featuredArticles, $treeCategories, $treeArticles ): void {
                if ( $pageKind === 'landing' ) {
                    ...
                    echo '<input id="help-search-home" class="mw-input" type="search" name="q" placeholder="Search help by module, task, or issue">';
                    echo '<button class="mw-btn" type="submit">Search Help</button>';
                    echo '</form>';
                ...
                if ( $pageKind === 'search' ) {
                    ...
                    echo '<input id="help-search-query" class="mw-input" type="search" name="q" value="' . metis_escape_attr( $searchQuery ) . '" placeholder="Search help by module, task, or issue">';
                    echo '<label class="screen-reader-text" for="help-search-category">Filter by category</label>';
                    echo '<select id="help-search-category" class="mw-input" name="category" aria-label="Help category">';
                    echo '<option value="">All categories</option>';
                    ...
                    echo '</select>';
                    echo '<button class="mw-btn" type="submit">Search</button>';
                    echo '</form>';
                    echo '</section>';
                ...
                if ( $pageKind === 'article' ) {
                    ...
                    echo '<button type="button" class="mw-btn" data-help-support-open'
                    ...
                    echo '<div class="mw-modal-backdrop" id="metis-help-support-modal" aria-hidden="true">';
                    echo '<div class="mw-modal metis-help-support-modal" role="dialog" aria-modal="true" aria-labelledby="metis-help-support-title">';
                    echo '<div class="mw-modal-header">';
                    echo '<h2 id="metis-help-support-title" class="mw-modal-title">Contact System Admin</h2>';
                    echo '<button type="button" class="mw-modal-close" data-help-support-close aria-label="Close">&times;</button>';
                    echo '</div>';
                    echo '<div class="mw-modal-body">';
                    ...
                    echo '<textarea id="metis-help-support-message" class="mw-input" name="message" rows="6" placeholder="Describe what you expected to see, what actually happened, and what you were trying to finish." required></textarea>';
                    echo '<p class="metis-help-muted">Your message includes the article and current Help page so the system admin has context.</p>';
                    echo '</form>';
                    echo '</div>';
                    echo '<div class="mw-modal-footer">';
                    echo '<button type="button" class="mw-btn mw-btn-secondary" data-help-support-close>Cancel</button>';
                    echo '<button type="button" class="mw-btn" data-help-support-submit>Send Request</button>';
                    echo '</div>';
                    echo '</div>';
                    echo '</div>';
                    return;
                ...
                echo '<p><a class="mw-btn" href="' . metis_escape_url( $homeUrl ) . '">Back to Help</a></p>';
                echo '</section>';
            },
        ]
    );
