...
$(document).off('dragover.metisImport', '#metis-import-drop-zone').on('dragover.metisImport', '#metis-import-drop-zone', function(e) {
    e.preventDefault();
    $(this).css('border-color', 'var(--mw-primary,#0d6efd)');
});
...
function bindNativeImportFallbacks() {
    ...
    if (zone && !zone.dataset.metisImportDndBound) {
        ...
        zone.addEventListener('dragover', function(e) {
            e.preventDefault();
            zone.style.borderColor = 'var(--mw-primary,#0d6efd)';
        });
...
function renderPreview(preview) {
    ...
    if (preview.site_info && preview.site_info.title) {
        html += '<div style="margin-bottom:16px;padding:12px 16px;background:var(--mw-surface-alt,#f7f8fa);border-radius:6px;font-size:13px;">';
    ...
    stats.forEach(function(s) {
        html += '<div style="text-align:center;padding:12px;background:var(--mw-surface,#fff);border:1px solid var(--mw-border,#e2e6ea);border-radius:6px;">';
        html += '<div style="font-size:24px;font-weight:700;color:var(--mw-primary,#0d6efd);">' + s.count + '</div>';
        html += '<div style="font-size:12px;color:var(--mw-text-muted,#888);">' + s.label + '</div>';
        html += '</div>';
    });
    html += '</div>';
