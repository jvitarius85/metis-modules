...
MetisBlockEditor.prototype._render = function() {
  ...
  if (root) this._bindVResize(root);
};
...
MetisBlockEditor.prototype._shellHtml = function() {
  return [
    ...
    /* ── MEDIA PICKER MODAL ── */
    '  <div class="mube-media-modal" id="mube-media-modal" hidden>',
    '    <div class="mube-media-modal-bg" data-action="close-media-picker"></div>',
    '    <div class="mube-media-modal-panel" role="dialog" aria-label="Choose Image">',
    '      <div class="mube-media-modal-head">',
    '        <span>Media Library</span>',
    '        <input type="search" class="mube-media-search" id="mube-media-search-input" placeholder="Search files\u2026" autocomplete="off">',
    '        <button type="button" class="mube-icon-btn" data-action="close-media-picker" aria-label="Close">\u2715</button>',
    '      </div>',
    '      <div class="mube-media-modal-body" id="mube-media-grid"></div>',
    '      <div class="mube-media-modal-foot">',
    '        <span class="mube-media-status" id="mube-media-status"></span>',
    '        <button type="button" class="mube-btn-ghost" data-action="close-media-picker">Cancel</button>',
    '      </div>',
    '    </div>',
    '  </div>',
...
MetisBlockEditor.prototype._moduleHtml = function(sIdx, cIdx, mod, mIdx) {
  ...
  /* Preview area */
  var previewStyle = style.min_height ? ' style="min-height:' + parseInt(style.min_height,10) + 'px"' : '';
  html += '<div class="mube-mod-preview"' + previewStyle + ' data-sidx="' + sIdx + '" data-cidx="' + cIdx + '" data-midx="' + mIdx + '">';
    ...
    switch (type) {
      ...
      case 'image':
        ...
        if (d.src) {
          ...
          html += '<div class="mube-img-preview mube-img-preview--empty">No image selected</div>';
        }
...
MetisBlockEditor.prototype._applyBind = function(key, val, si, ci, mi) {
  var parts=key.split('.');var entity=parts[0];var prop=parts.slice(1).join('.');
  this._push();
  if (entity==='section') {
    var sec=this._getSection(si); if(!sec)return;
    this._setNestedKey(sec,prop,val); this._patchSectionStyle(si);
  } else if (entity==='column') {
    if (prop==='width') {
      var sec2=this._getSection(si); var col=this._getColumn(si,ci);
      if (sec2&&col) {
        var newW=Math.max(10,Math.min(90,parseFloat(val)||50));
        var diff=newW-col.width;
        var others=(sec2.columns||[]).filter(function(c){return c!==col;});
        if(others.length===1)others[0].width=Math.max(10,others[0].width-diff);
        col.width=newW; this._patchColumnWidths(si);
        var panel=document.getElementById('mube-props-body');
        if(panel){panel.querySelectorAll('[data-bind="column.width"]').forEach(function(el){if(el!==document.activeElement)el.value=String(Math.round(newW));});}
      }
    } else {
      var col2=this._getColumn(si,ci); if(!col2)return;
      this._setNestedKey(col2,prop,val); this._patchColumnStyle(si,ci);
    }
  } else if (entity==='module') {
    var mod=this._getModule(si,ci,mi); if(!mod)return;
    this._setNestedKey(mod,prop,val); this._patchModulePreview(si,ci,mi);
  }
  this._applySelClasses(); this._scheduleAutosave();
};

MetisBlockEditor.prototype._patchModulePreview = function(si,ci,mi) {
  var mod=this._getModule(si,ci,mi); if(!mod)return;
  var sel='[data-kind="section"][data-sidx="'+si+'"] [data-kind="column"][data-cidx="'+ci+'"] [data-kind="module"][data-midx="'+mi+'"] .mube-mod-preview';
  var el=document.querySelector(sel); if(!el)return;
  var nh=this._modulePreview(mod); if(el.innerHTML!==nh)el.innerHTML=nh;
  if(mod.style&&mod.style.min_height) el.style.minHeight=mod.style.min_height+'px';
...
MetisBlockEditor.prototype._patchColumnWidths = function(si) {
  var sec=this._getSection(si); if(!sec)return;
  (sec.columns||[]).forEach(function(col,ci){var el=document.querySelector('[data-kind="section"][data-sidx="'+si+'"] [data-kind="column"][data-cidx="'+ci+'"]'); if(el)el.style.flex='0 0 '+(col.width||(100/sec.columns.length))+'%';});
};

MetisBlockEditor.prototype._patchColumnStyle = function(si,ci) {
  var col=this._getColumn(si,ci); if(!col)return;
  var s=col.style||{}; var el=document.querySelector('[data-kind="section"][data-sidx="'+si+'"] [data-kind="column"][data-cidx="'+ci+'"]'); if(!el)return;
  var a=[]; if(s.bg_color)a.push('background-color:'+s.bg_color); if(s.padding)a.push('padding:'+s.padding); el.style.cssText=a.join(';');
};
...
MetisBlockEditor.prototype._bindEvents = function() {
  ...
  /* ---- Media picker search ---- */
  var mediaSearch = document.getElementById('mube-media-search-input');
  if (mediaSearch) mediaSearch.addEventListener('input', function() {
    var t = self._mediaTarget;
    if (t) self._openMediaPicker(t.si, t.ci, t.mi);
  });
  ...
  /* ---- Media picker interactions (sidebar + modal — outside canvas, not data-action) ---- */
  root.addEventListener('click', function(e) {
    ...
    /* Remove Image button */
    var clearBtn = e.target.closest('.mube-btn-clear-img');
    if (clearBtn) {
      var csi=+clearBtn.dataset.sidx, cci=+clearBtn.dataset.cidx, cmi=+clearBtn.dataset.midx;
      var cmod=self._getModule(csi,cci,cmi);
      if (cmod) { self._push(); cmod.data=cmod.data||{}; cmod.data.src=''; self._patchModulePreview(csi,cci,cmi); self._renderPropsPane(); self._scheduleAutosave(); }
      return;
    }
...
MetisBlockEditor.prototype._openMediaPicker = function(si, ci, mi) {
  ...
  var nonces  = ajaxCfg.action_nonces || {};
  var nonce   = nonces.metis_media_library_list || '';
  var search  = (document.getElementById('mube-media-search-input') || {}).value || '';
  var data = 'action=metis_media_library_list&nonce='+encodeURIComponent(nonce)+'&per_page=60&mime_filter=image&search='+encodeURIComponent(search);
  ...
  xhr.onload = function() {
    try {
      ...
      items = items.filter(function(it){ return it.mime_type && it.mime_type.indexOf('image/')===0; });
      ...
      items.forEach(function(it) {
        html += '<div class="mube-media-item" data-url="'+self._escA(it.url||'')+'" data-name="'+self._escA(it.file_name||'')+'">'+
...
MetisBlockEditor.prototype._closeMediaPicker = function() {
  var modal = document.getElementById('mube-media-modal');
...
/* =========================================================================
   ...
   ========================================================================= */
MetisBlockEditor.prototype._bindVResize = function(root) {
  var self = this;
  root.addEventListener('mousedown', function(e) {
    var handle = e.target.closest('.mube-vresize');
    if (!handle) return;
    e.preventDefault();
    var modEl = handle.closest('[data-kind="module"]');
    if (!modEl) return;
    var si = +modEl.dataset.sidx, ci = +modEl.dataset.cidx, mi = +modEl.dataset.midx;
    var startY = e.clientY;
    var preview = modEl.querySelector('.mube-mod-preview');
    var startH = preview ? preview.offsetHeight : 120;
    var mod = self._getModule(si, ci, mi);
    function onMove(ev) {
      var dy = ev.clientY - startY;
      var newH = Math.max(40, startH + dy);
      if (preview) preview.style.minHeight = newH + 'px';
    }
    function onUp(ev) {
