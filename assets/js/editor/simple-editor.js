(function (window, document) {
    ...
    function resolveEditorThemeColor(value, fallback) {
        ...
        var cssVar = raw.indexOf('mw_') === 0 ? '--' + raw.replace(/_/g, '-') : raw;
    ...
    var boot = document.getElementById('mwpb-editor-bootstrap');
    if (!boot) return;
    var bootSignature = [
    ...
    if (!root) {
        ...
        var inline = document.getElementById('mwpb-inline-root');
    ...
    function hideBootStatus() {
        var bootStatus = document.getElementById('mwpb-editor-boot-status');
    ...
    function bootNewsletterEditor() {
        ...
        var newsletterThemeColorOptions = [
            ['default', 'Default'],
            ['mw_primary', 'Primary'],
            ['mw_primary_dark', 'Primary Dark'],
            ['mw_accent', 'Accent'],
            ['mw_bg', 'Background'],
            ['mw_surface', 'Surface'],
            ['mw_border', 'Border'],
            ['mw_text', 'Text'],
            ['mw_text_muted', 'Muted Text'],
            ['mw_header_bg', 'Header Background'],
            ['mw_row_odd_bg', 'Row Odd Background'],
            ['mw_row_even_bg', 'Row Even Background'],
            ['mw_row_hover_bg', 'Row Hover Background'],
            ['mw_sidebar_bg', 'Sidebar Background'],
            ['mw_sidebar_icon_color', 'Sidebar Icon'],
            ['mw_sidebar_active_color', 'Sidebar Active']
        ];
        ...
        function resolveNewsletterThemeColor(value, fallback) {
            ...
            var aliasMap = {
                ...
                card_bg: '--mw-surface',
                border: '--mw-border',
                card_border: '--mw-border',
                primary: '--mw-primary',
                primary_dark: '--mw-primary-dark',
                header_bg: '--mw-header-bg',
                row_odd_bg: '--mw-row-odd-bg',
                row_even_bg: '--mw-row-even-bg',
                row_hover_bg: '--mw-row-hover-bg',
                sidebar_bg: '--mw-sidebar-bg',
                sidebar_icon_color: '--mw-sidebar-icon-color',
                sidebar_active_color: '--mw-sidebar-active-color'
            };
        ...
        function renderShell() {
            ...
            root.innerHTML = '' +
                ...
                '</div>' +
                '</div>' +
                '<div id="metis-nl-test-modal" class="mw-modal-overlay metis-nl-modal" style="display:none;" role="dialog" aria-modal="true" aria-hidden="true" aria-label="Send Test Email">' +
                '<div class="mw-modal metis-nl-modal__dialog">' +
                '<div class="mw-modal-header"><h2 class="mw-modal-title">Send Test Email</h2><button type="button" class="mw-modal-close" id="metis-nl-test-close" aria-label="Close">&times;</button></div>' +
                ...
                '</div>' +
                '<div class="mw-modal-footer"><button type="button" class="mw-btn mw-btn-ghost" id="metis-nl-test-cancel">Cancel</button><button type="button" class="mw-btn" id="metis-nl-test-send">Send Test</button></div>' +
                '</div>' +
                '</div>' +
                '</div>';
        }
        ...
        function renderStep1() {
            ...
            wrap.innerHTML = '' +
                ...
                '<div class="metis-se-field-row metis-nl-details-row"><label>Lists</label><div class="metis-nl-check-grid">' + (listOptions || '<div class="mw-muted">No active lists available.</div>') + '</div></div>' +
                ...
                if (richAction) {
                    ...
                    if (actionTarget) {
                        ...
                        if (action === 'block') {
                            ...
                            var resolvedColor = resolveEditorThemeColor(actionValue, resolveEditorThemeColor('mw_text', '#1f2330'));
    ...
    function bootStructuredEditorV2() {
        var PAGE_SECTION_TYPES = ['text', 'columns', 'feature_grid', 'cta', 'events', 'spacer', 'posts_list'];
        var POST_SECTION_TYPES = ['text', 'transcript'];
        ...
        function normalizeSection(section, idx) {
            ...
            if (out.type === 'text') {
                ...
                if (!out.content.items.length) out.content.items = [{ icon: '', title: 'Feature', text: '', cta: { label: '', url: '#' } }];
        ...
        function activeSection() {
            ...
            if (state.activeSection < 0) state.activeSection = 0;
        ...
        function renderSectionList() {
            var list = document.getElementById('metis-v2-section-list');
            if (!list) return;
            var html = '';
            state.sections.forEach(function (sec, i) {
                html += '<div class="metis-se-section-item" data-index="' + i + '">' +
                    '<div class="metis-se-section-row">' +
                        '<div class="metis-se-meta-value">' + esc((i + 1) + '. ' + sectionTypeLabel(sec.type) + ' - ' + sectionSummary(sec)) + '</div>' +
                        '<div>' +
                            '<button type="button" class="metis-se-nav-btn" data-v2-list-act="edit">Edit</button> ' +
                            '<button type="button" class="metis-se-nav-btn" data-v2-list-act="up">&uarr;</button> ' +
                            '<button type="button" class="metis-se-nav-btn" data-v2-list-act="down">&darr;</button> ' +
                            '<button type="button" class="metis-se-nav-btn" data-v2-list-act="delete">Delete</button>' +
                        '</div>' +
                    '</div>' +
                '</div>';
            });
        ...
        function renderTop() {
            ...
            var sectionStep = sectionEditorStepNumber();
            var previewStep = previewStepNumber();
            root.innerHTML = '' +
                '<div class="metis-se-shell">' +
                    ...
                    '</div>' +
                    '<div class="metis-se-stage-wrap">' +
                        '<section class="metis-se-stage" data-v2-step="1">' +
                            '<h2 class="metis-se-stage-title">Properties</h2>' +
                            '<div class="metis-se-card"><div class="metis-se-field-grid">' +
                            ...
                            '</div></div>' +
                            '<div class="metis-se-step-divider"></div><div class="metis-se-sections-panel metis-se-sections-panel-full"><div class="metis-se-sections-head">Sections</div><p class="metis-se-sections-copy">' + (isPostContext() ? 'Build the post with intro text, transcript, and outro sections.' : 'Add, reorder (up/down), or delete sections.') + '</p><div class="metis-se-field-row"><label for="metis-v2-new-section-type">Add Section</label><div><select id="metis-v2-new-section-type" class="metis-se-select">' + availableSectionTypes().map(function (type) { return '<option value="' + esc(type) + '">' + esc(sectionTypeLabel(type)) + '</option>'; }).join('') + '</select> <button type="button" id="metis-v2-add-section" class="metis-se-nav-btn">Add</button></div></div><div id="metis-v2-section-list" class="metis-se-section-list"></div></div>' +
                        '</section>' +
                        (isPageContext() ? '<section class="metis-se-stage" data-v2-step="2" hidden><h2 class="metis-se-stage-title">Homepage Hero</h2><div id="metis-v2-hero-editor" class="metis-se-builder-stack"></div></section>' : '') +
                        '<section class="metis-se-stage" data-v2-step="' + sectionStep + '" hidden>' +
                            '<h2 class="metis-se-stage-title">' + (isPostContext() ? 'Content Sections' : 'Section Editor') + '</h2>' +
                            '<div class="metis-se-builder-grid"><aside class="metis-se-builder-left"><div class="metis-se-section-switch"><button id="metis-v2-section-prev" type="button" class="metis-se-nav-btn">&larr;</button><span id="metis-v2-section-label" class="metis-se-active-section-label"></span><button id="metis-v2-section-next" type="button" class="metis-se-nav-btn">&rarr;</button></div><div id="metis-v2-section-settings"></div></aside><div class="metis-se-builder-right metis-se-builder-right--full"><div id="metis-v2-section-content"></div></div></div>' +
                        '</section>' +
                        '<section class="metis-se-stage" data-v2-step="' + previewStep + '" hidden><h2 class="metis-se-stage-title">Preview</h2><div id="metis-v2-preview" class="metis-se-preview"></div></section>' +
                    '</div>' +
                    (isPostContext() ? '<div id="metis-v2-featured-image-modal" class="mw-modal-overlay metis-featured-image-modal" style="display:none;" role="dialog" aria-modal="true" aria-hidden="true" aria-label="Featured Image Picker"><div class="mw-modal metis-featured-image-modal__dialog"><div class="mw-modal-header"><div><h2 class="mw-modal-title">Choose Featured Image</h2><div id="metis-v2-featured-image-count" class="metis-featured-image-modal__count">Loading images...</div></div><button type="button" class="mw-modal-close" id="metis-v2-featured-image-close" aria-label="Close">&times;</button></div><div class="mw-modal-body"><div class="metis-featured-image-modal__toolbar"><input id="metis-v2-featured-image-search" class="metis-se-input" type="search" placeholder="Search images by name or type"><select id="metis-v2-featured-image-mime" class="metis-se-select"><option value=\"\">All image types</option></select></div><div id="metis-v2-featured-image-list" class="metis-media-grid metis-featured-image-modal__grid"></div></div><div class="mw-modal-footer"><button type="button" class="mw-btn" id="metis-v2-featured-image-cancel">Close</button></div></div></div>' : '') +
                    '<div id="metis-v2-inline-image-modal" class="mw-modal-overlay metis-featured-image-modal" style="display:none;" role="dialog" aria-modal="true" aria-hidden="true" aria-label="Inline Image Picker"><div class="mw-modal metis-featured-image-modal__dialog"><div class="mw-modal-header"><div><h2 class="mw-modal-title">Insert Image</h2><div id="metis-v2-inline-image-count" class="metis-featured-image-modal__count">Loading images...</div></div><button type="button" class="mw-modal-close" id="metis-v2-inline-image-close" aria-label="Close">&times;</button></div><div class="mw-modal-body"><div class="metis-featured-image-modal__toolbar"><input id="metis-v2-inline-image-search" class="metis-se-input" type="search" placeholder="Search images by name or type"><select id="metis-v2-inline-image-mime" class="metis-se-select"><option value=\"\">All image types</option></select></div><div id="metis-v2-inline-image-list" class="metis-media-grid metis-featured-image-modal__grid"></div></div><div class="mw-modal-footer"><button type="button" class="mw-btn" id="metis-v2-inline-image-cancel">Close</button></div></div></div>' +
                '</div>';
        }
        ...
        function syncStepUi() {
            ...
            root.querySelectorAll('[data-v2-step]').forEach(function (stage) {
                ...
                stage.hidden = !active;
                stage.style.display = active ? 'flex' : 'none';
            });
            ...
            if (prev) {
                ...
                if (state.step === sectionEditorStepNumber()) prev.textContent = isPageContext() ? 'Back to Hero' : 'Back to Properties';
                else if (state.step === previewStepNumber()) prev.textContent = 'Back to Content';
                else prev.textContent = 'Back to Properties';
            }
            if (next) {
                if (state.step === 1) next.textContent = isPageContext() ? 'Continue to Hero' : 'Continue to Content';
                else if (state.step === sectionEditorStepNumber()) next.textContent = 'Continue to Preview';
                else if (state.step === max) next.textContent = 'Publish';
                else next.textContent = 'Continue to Content';
            }
            if (isPageContext() && state.step === 2) renderHeroEditor();
            if (state.step === sectionEditorStepNumber()) renderStep2Editor();
            if (state.step === previewStepNumber()) updatePreview();
        }
        ...
        function wire() {
            ...
            root.addEventListener('click', function (e) {
                ...
                if (rowBtn) {
                    ...
                    if (act === 'edit') { state.activeSection = idx; state.step = sectionEditorStepNumber(); syncStepUi(); return; }
            ...
            root.addEventListener('change', function (e) {
                ...
                if (target.id === 'metis-v2-homepage') {
                    ...
                    state.hero = normalizeHeroState(state.hero, isHomepage);
                    state.selectedTemplateKey = normalizeSelectedTemplateKey(state.selectedTemplateKey, currentPageType());
                    if (isPageContext() && state.step === 2) {
                        renderHeroEditor();
                    }
                    state.dirty = true;
                    return;
                }
                if (target.id === 'metis-v2-status') { syncPublishedDateField(); renderPostPathPreview(); state.dirty = true; return; }
                if (target.id === 'metis-v2-parent-id' || target.id === 'metis-v2-featured-image-id' || target.id === 'metis-v2-published-date' || target.id === 'metis-v2-excerpt') { state.dirty = true; return; }
