    ...
    function inputForField(field, value) {
        ...
        if (type === 'select') {
            ...
            const optionsHtml = ['<option value=""></option>']
                ...
                .join('');
            return '<select class="mw-select mbe-input" data-key="' + escapeHtml(key) + '">' + optionsHtml + '</select>';
        }

        const htmlType = type === 'number' || type === 'email' || type === 'date' ? type : 'text';
        return '<input class="mw-input mbe-input" data-key="' + escapeHtml(key) + '" type="' + htmlType + '" value="' + escapeHtml(safeValue) + '">';
    }
            ...
            tbody.innerHTML = rows.map((row, rowIndex) => {
                ...
                return '' +
                    '<tr class="mbe-row" data-row-index="' + rowIndex + '">' +
                        ...
                        '</td>' +
                        cells +
                        '<td><button type="button" class="mw-btn-xs mbe-delete-row" data-row-index="' + rowIndex + '">Delete</button></td>' +
                    '</tr>';
            }).join('');
        },
